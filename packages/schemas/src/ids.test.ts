...
describe('branded id schemas [BUG-215]', () => {
  ...
  describe('escape hatches preserve runtime identity', () => {
    it('asProfileId is identity at runtime', () => {
      const id: ProfileId = asProfileId(UUID);
      expect(id).toBe(UUID);
    });

    it('asSubjectId is identity at runtime', () => {
      const id: SubjectId = asSubjectId(UUID);
      expect(id).toBe(UUID);
    });

    it('asSessionId is identity at runtime', () => {
      const id: SessionId = asSessionId(UUID);
      expect(id).toBe(UUID);
    });

    it('asBookId is identity at runtime', () => {
      const id: BookId = asBookId(UUID);
      expect(id).toBe(UUID);
    });

    it('asTopicId is identity at runtime', () => {
