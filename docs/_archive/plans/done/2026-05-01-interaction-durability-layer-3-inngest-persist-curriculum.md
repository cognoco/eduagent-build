...
> **Supersession note (WI-516, 2026-06-23):** Do not execute this plan. The interview flow and `persistCurriculum` finalizer were removed before Layer 3 landed. The draft-status enum values `completing` / `failed` and `onboarding_drafts.failure_code` were added by migration 0046, but their intended Inngest-backed interview consumer no longer exists. They remain only until the deferred Phase-2 retirement of the whole `onboarding_drafts` table; removing them independently would require a risky Postgres enum recreation for a table already scheduled to be dropped.
