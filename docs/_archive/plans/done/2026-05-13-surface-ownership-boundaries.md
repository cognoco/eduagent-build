...
> **Status (2026-05-26):** ALL 10 PRs closed. **Shipped:** PR 1 (query-keys registry, `apps/mobile/src/lib/query-keys.ts`), PR 2 (API service boundary extraction — `routes/progress.ts` flipped to `listProfileSessions` from `services/session/session-crud`; re-export shim in `dashboard.ts` removed; test imports flipped to direct paths), PR 3 (session facade hooks, `use-session-context.ts`), PR 4 (library retention boundary, `use-library-context.ts`), PR 5 (prop-drill cleanup — `ChildQuotaLine`/`EarlyAdopterCard`/`WithdrawalCountdownBanner` no longer fetch; `ChildAccommodationRow` was never created because per-child accommodation rows aren't shown on `ParentHomeScreen` at all, which makes the original violation moot), PR 6 (ReportsList dedupe), PR 7 (subject rename to `SubjectTile`/`SubjectProgressRow`), PR 8 (AST-based ownership guard, `surface-ownership.test.ts`). **Documented deferrals:** PR 9 (payload-narrow queries — deferred with full rationale in `use-session-context.ts` header: warm-cache case is net regression, latency unmeasurable in PR; revisit triggers listed), PR 10 (invalidation precision — one bug fixed: `invalidateProgressSnapshotQueries` was a silent no-op, replaced with predicate-based call + 4 workflow tests; remaining broad-prefix sites carry inline `PR-10 deferred:` comments with closure-missing-context rationale, per plan policy "deferred-with-reason is fine, silently-removed is not"). Plan archived to `docs/_archive/plans/done/`.
...
| `apps/mobile/src/components/home/ChildAccommodationRow.tsx` | ~~Modify~~ **Not created.** | Per-child accommodation rows aren't shown on `ParentHomeScreen` at all; the per-child `useChildLearnerProfile` calls live on dedicated child-surface screens (`child/[profileId]/index.tsx`, `child/[profileId]/mentor-memory.tsx`, `more/accommodation.tsx`) where they're correct. Violation resolved by absence, not by component creation. |
