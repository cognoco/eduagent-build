...
> **Supersession note (WI-516, 2026-06-23):** Do not implement this spec's interview-side Layer 2 or Layer 3. The per-subject interview flow was later deleted, and SUBJECT-09 is closed by that deletion plus the per-topic async topic-probe extraction path (`apps/api/src/services/session/topic-probe-extraction.ts`, `apps/api/src/inngest/functions/topic-probe-extract.ts`). The old `app/interview.ready_to_persist` flow, `routes/interview.ts`, `services/interview.ts`, and `interview-persist-curriculum.ts` no longer exist in source. This archived document also contains stale implementation details: A5's COALESCE-sentinel topic key did not ship; the shipped dedup index is `apps/api/drizzle/0043_topic_dedup_unique_index.sql` on `(book_id, lower(title))`. The old `extractSignals()` topic-returning pseudocode was replaced by `extractSignalsFromExchangeHistory()`, which returns signal fields such as `goals`, `experienceLevel`, `currentKnowledge`, and `interests`, not topic rows. The `draft_status` values `completing` / `failed` and `onboarding_drafts.failure_code` remain only as remnants of migration 0046 until the planned Phase-2 drop of the entire `onboarding_drafts` table; recreating the enum solely to remove those values is intentionally avoided because the whole table is already marked for deferred retirement.
...
| Idempotency-Key collision on replay | Mobile replays a message that the server actually persisted before the disconnect | Server returns 200 + `{ replayed: true, assistantTurnReady, latestExchangeId }`; mobile renders directly when `assistantTurnReady` is true, otherwise refetches and polls until the assistant turn lands | No duplicate persistence; outbox deletes entry on confirm |
| Replay-hit with assistant turn pending | Mobile retries a request whose original handler crashed AFTER the user message persisted but BEFORE the assistant stream completed | Server returns 200 with `assistantTurnReady: false`; mobile shows the user message + a "still thinking…" indicator; polls the conversation tail until the assistant turn lands or a timeout fires | Polling resolves on Inngest-driven push notification (Layer 3) or on next foreground; user can also manually re-engage (their next send creates a new `client_id` and proceeds normally) |
...
- **Replay response:** on KV hit, middleware short-circuits with HTTP 200 `{ replayed: true, clientId, status: 'persisted', assistantTurnReady: boolean, latestExchangeId: string | null }` and `Idempotency-Replay: true` header. Mobile uses `assistantTurnReady` to decide whether to render immediately or refetch and poll until the assistant turn lands.
...
**Race-window tightening — `assistantTurnReady` hint:** the replay response carries `assistantTurnReady: boolean` and `latestExchangeId: string | null` so mobile can distinguish "user message AND assistant reply both persisted" (render immediately) from "user message persisted, assistant reply still pending" (refetch + poll-the-tail until the assistant turn lands, reusing the Layer 3 polling pattern). Server checks: for `session` flow, looks up `session_events` for an `assistant_message` row in the same session with `created_at >= the user row's created_at` matching the `client_id`; for `interview` flow, scans `onboardingDrafts.exchangeHistory[]` for an `assistant` entry after the matching user `client_id`. The lookup is cheap (single indexed query) and degrades safely — if it errors, default to `assistantTurnReady: false` and let mobile poll. Never 500 from the middleware.
...
| OQ-12 | How long should mobile poll for the assistant turn after a replay-hit with `assistantTurnReady: false`? | 30 s with 3 → 6 → 12 s backoff (matches Layer 3 polling), then surface a "Still working — pull to refresh" affordance. Server-side push notification (Layer 3) cuts polling short when delivered. |
