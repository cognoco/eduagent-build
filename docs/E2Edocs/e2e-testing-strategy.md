...
5. **Smoke suite buildout** — **DONE.** All planned smoke and nightly flows written. 36 flows confirmed passing on Android emulator (as of Session 14). 11 flows have known failures (LLM-dependent, flow-design, testID mismatch).
...
9. **Nightly full suite** — NEAR COMPLETE. All 53 flows written. 36 flows confirmed passing (Session 14). 11 flows failing (3 LLM-dependent, 4 flow-design, 3 testID/text mismatch, 1 app logic). 4 flows not yet run (need `launch-devclient.yaml` mechanism). 1 flow partial (settings-toggles, BUG-18). 1 skipped (ExpoGo-only).
...
11. **Session 14 sweep (2026-03-12)** — 26 flows run: 18 PASS, 1 PARTIAL, 11 FAIL. BUG-48 discovered and fixed (parent-redirect timing in `seed-and-sign-in.yaml`). All parent flows now stable. Remaining failures are LLM-dependent (3), flow-design (4), testID/text mismatch (3), and app logic (1). See `e2e-test-results.md` for full breakdown.
