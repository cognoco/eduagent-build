...
5. **Smoke suite buildout** — **DONE.** All planned smoke and nightly flows written. 44 flows confirmed passing on Android emulator (as of Session 16). 2 flows need re-test (LLM connectivity — may now pass with SSE streaming fix). 4 pre-auth flows now runnable via `run-without-seed.sh` (not yet tested on emulator).
...
9. **Nightly full suite** — NEAR COMPLETE. All 54 flows written. 45 flows confirmed passing (Session 16 + BUG-52 fix). 2 flows need re-test (LLM connectivity — analogy-preference-flow, curriculum-review-flow; session-summary now passes after SSE streaming fix). child-paywall FIXED (BUG-52: switch-to-child.yaml). 4 pre-auth flows READY via `run-without-seed.sh` (coppa-flow, profile-creation-consent, consent-pending-gate, sign-up-flow — not yet tested on emulator). 1 flow partial (settings-toggles, BUG-18). 1 skipped (ExpoGo-only). 1 deferred (recall-review, needs coaching-card precompute).
