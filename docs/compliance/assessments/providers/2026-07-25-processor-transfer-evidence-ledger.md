...
| L5 Voyage | Hosted embeddings API; production key present; organization opt-out requires paid billing | US / hosted region unknown | Public DPA identifies AWS and Google | Current stateless embeddings path; zero-day retention begins only after organization opt-out, which remains pending; Files API has a separate 30-day rule but is not used | Public DPA and dated research are indexed; opt-out evidence, organization identity and TIA acceptance remain with OPQ-110 |
...
|---|---|---|
| Mistral adapter uses the global endpoint while compliance research says EU processing requires the EU endpoint | The model register treats Mistral as the EU secondary/vision route. If the endpoint remains global, it cannot be used as proof of EU-resident processing. | WI-2740 owns the correction: switch the adapter/config to `api.eu.mistral.ai` for the EU route or amend the register and transfer package so it no longer claims EU-region processing. |
