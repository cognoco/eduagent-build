...
| 15 | Provider copies (LLM providers, Voyage AI, Clerk, RevenueCat/Stripe, Resend, Sentry, Inngest) | Processing by named sub-processors | **TODO for every provider** — per-provider retention is explicitly out of this document's scope per the task brief ("handled separately"); DPO Actions 7-9 are actively gathering DPA/evidence packs from OpenAI, Mistral, Anthropic, Cerebras, Voyage AI (`DPO exchanges/2026-07-26-action-register-tracker.md` rows 7-9, in flight as of 2026-07-26) | N/A | Explicit deletion calls exist for **Clerk** (`deleteClerkUser`, `apps/api/src/services/clerk-user.ts`, called from `account-deletion.ts`) and a **RevenueCat/Stripe teardown event** (`app/billing.subscription_store_teardown_requested`, dispatched by `account-deletion.ts` and consumed by `billing-subscription-store-teardown.ts`) — both are retried. After retries exhaust, the PII-minimized `app/account.deletion_teardown.failed` and `app/billing.subscription_store_teardown.failed` events are now emitted through stable, awaited `step.sendEvent` calls in those two failure handlers. Inngest durably records, retries, and memoizes the step, so a transport rejection or timeout does not become an application-level swallowed success and replay does not start a second event send. No new payload field or database record is introduced. Manual external-erasure remediation remains necessary, and real chat/pager delivery plus production-console routing are not proved here | Legal/DPO (provider DPAs) + Engineering (the two wired erasure calls) | See G-8: repository-owned dispatch durability is implemented by WI-2994; WI-1916 covers only downstream chat/pager routing and production-console activation |
...
- Both consent paths have `onFailure` Sentry escalation and best-effort
  ...
  account-deletion and subscription-store teardown events named in G-8. Those
  two terminal external-erasure handlers no longer use `safeSend`: WI-2994
  routes them through awaited, stable-name `step.sendEvent` operations. The
  consent-path `safeSend` calls remain best-effort and are outside WI-2994.
...
| G-8 | **Repository-owned dispatch durability implemented by WI-2994:** exhausted Clerk and subscription-store teardown legs emit `app/account.deletion_teardown.failed` and `app/billing.subscription_store_teardown.failed` through awaited `step.sendEvent` operations in `apps/api/src/inngest/functions/account-deletion.ts` and `apps/api/src/inngest/functions/billing-subscription-store-teardown.ts`. Stable step names give Inngest a memoized durable record; deterministic event IDs protect downstream replay. A timeout is not treated as cancellation or swallowed success: the step remains Inngest-owned and retriable until it settles. Focused tests cover immediate rejection, late resolution, late rejection, replay, both event types, and payload minimization. No migration or new database record is needed. The code-owned launch-health mapping is documented in `docs/runbooks/launch-health-alerts.md`. Manual provider remediation is still required; real chat/pager integrations and production-console rule routing remain parked under WI-1916 | Provider copies (§2 row 15) | Low residual — dispatch is durable and duplicate-safe within the existing Inngest substrate, but an operator must still complete external erasure and unproved downstream routing cannot be treated as a delivered page | WI-2994 owns and closes dispatch durability; WI-1916 owns only downstream chat/pager delivery and production-console routing |
...
10. **Partial-external-failure dead-letter drill.** In an isolated test
    ...
    durable-step immediate rejection, late resolution, late rejection, and
    replay paths. Confirm the same stable step key is reused and that a replay
    does not start duplicate remediation. A timeout is not cancellation; the
    awaited Inngest step retains retry/recovery ownership until settlement.
    Confirm the code-owned launch-health mapping in
    `docs/runbooks/launch-health-alerts.md`,
