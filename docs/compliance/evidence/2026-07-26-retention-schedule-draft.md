...
| 13 | Queues (Inngest event payloads; first-party retry/outbox tables) | Durable async execution — deletion orchestration, purges, notifications | **TODO — no in-repo statement of Inngest's own event-payload retention found** (checked `docs/architecture.md`, no match). First-party carriers are purpose-specific: `feedback_retry_queue` keeps feedback text out of Inngest (`support.ts:13-24`); `terminal_deletion_failure_outbox` stores only the existing PII-minimized terminal signal until confirmed dispatch (`terminal-deletion-failure-outbox.ts:10-35`; service `terminal-deletion-failure-outbox.ts:26-70`) | Queue insert | Delete after confirmed dispatch; terminal-deletion rows have no time-based purge because their remediation obligation remains open until delivery | Engineering (first-party carriers); Infra/ops (Inngest plan owner) | No Cloudflare Queue found in `apps/api/wrangler.toml`; Inngest plus the two first-party DB carriers are the durable queue substrates |
...
| 15 | Provider copies (LLM providers, Voyage AI, Clerk, RevenueCat/Stripe, Resend, Sentry, Inngest) | Processing by named sub-processors | **TODO for every provider** — per-provider retention is explicitly out of this document's scope per the task brief ("handled separately"); DPO Actions 7-9 are actively gathering DPA/evidence packs from OpenAI, Mistral, Anthropic, Cerebras, Voyage AI (`DPO exchanges/2026-07-26-action-register-tracker.md` rows 7-9, in flight as of 2026-07-26) | N/A | Explicit deletion calls exist for **Clerk** (`deleteClerkUser`, `apps/api/src/services/clerk-user.ts:251-321`, called from `account-deletion.ts:250-255`) and a **RevenueCat/Stripe teardown event** (`app/billing.subscription_store_teardown_requested`, dispatched `account-deletion.ts:219-234`, consumed by `billing-subscription-store-teardown.ts`). After their retries exhaust, the two handlers first upsert the existing PII-minimized terminal payload into `terminal_deletion_failure_outbox`, then attempt direct dispatch with `safeSendConfirmed` (`account-deletion.ts:90-120`; `billing-subscription-store-teardown.ts:69-100`). Rejection or a two-second timeout leaves the row; timeout stops waiting but does not cancel a late send (`safe-non-core.ts:37-112`). The failure observer retries immediate replay five times and a five-minute sweep recovers stranded rows, always with the same deterministic id. Manual external-erasure remediation remains necessary, and real chat/pager delivery plus production-console routing are not proved here | Legal/DPO (provider DPAs) + Engineering (the two wired erasure calls) | See G-8: event construction, first-party durable dispatch, and replay are wired under WI-2994; downstream chat/pager delivery and production-console routing remain open under WI-1916 |
...
- Both consent paths have `onFailure` Sentry escalation and best-effort
  ...
  The consent-path `safeSend` calls remain best effort. For account-deletion and
  subscription-store teardown, WI-2994 uses `safeSendConfirmed`: rejection or
  a two-second timeout is logged/reported and returns `false`, while timeout
  stops waiting rather than cancelling the underlying send
  (`services/safe-non-core.ts:37-112`). Before that direct attempt, each handler
  idempotently writes the existing PII-minimized payload to the first-party
  `terminal_deletion_failure_outbox` (`account-deletion.ts:90-120`;
  `billing-subscription-store-teardown.ts:69-100`). A successful direct send
  deletes the row. Otherwise `inngest-function-failed-observe` upserts and
  retries it through a named `step.sendEvent`, while the five-minute
  `terminal-deletion-failure-recovery` sweep drains any stranded row
  (`inngest-function-failed-observe.ts:75-145`;
  `terminal-deletion-failure-recovery.ts:15-64`). Every path shares the same
  deterministic event id, so late settlement does not duplicate remediation.
...
| G-8 | **Event construction and direct dispatch attempts landed in WI-2346 (PR #2792); WI-2994 adds first-party durable transport recovery:** exhausted Clerk and subscription-store teardown legs produce `app/account.deletion_teardown.failed` and `app/billing.subscription_store_teardown.failed`, respectively (`apps/api/src/inngest/functions/account-deletion.ts:90-120`; `apps/api/src/inngest/functions/billing-subscription-store-teardown.ts:69-100`). Each first upserts the PII-minimized signal into `terminal_deletion_failure_outbox`, then calls `safeSendConfirmed`; rejection or a two-second timeout is logged/Sentry-reported and leaves the row, while timeout does not cancel a late send (`apps/api/src/services/safe-non-core.ts:37-112`). The repository-owned `inngest-function-failed-observe` performs an immediate five-retry replay, and `terminal-deletion-failure-recovery` scans stranded rows every five minutes. A confirmed direct or replay send acknowledges the row. All paths share the deterministic signal id, preventing duplicate remediation if a timed-out direct attempt settles late. The code-owned launch-health mapping is documented in `docs/runbooks/launch-health-alerts.md:268-302`. Manual provider remediation is still required; real chat/pager integrations and production-console rule routing remain parked under WI-1916 | Provider copies (§2 row 15) | Medium — dispatch now survives transport rejection and timeout through a first-party durable row plus immediate and scheduled recovery, but the GDPR Art 17 failure still requires an operator to finish external erasure, and unproved production routing cannot be treated as a delivered page | WI-2346 closes event construction and best-effort direct dispatch wiring; WI-2994 closes the residual dispatch-durability gap; WI-1916 owns downstream external routing/remediation follow-through |
...
10. **Partial-external-failure dead-letter drill.** In an isolated test
    ...
    `safeSend` rejection and timeout paths and record that they log/report and
    return, while timeout does not cancel a late send. Then confirm the
    first-party `terminal_deletion_failure_outbox` row survives both cases.
    Confirm the repository-owned observer's immediate five-retry replay and
    the five-minute recovery sweep use the same deterministic event id as the
    direct attempt, then acknowledge the row only after confirmed dispatch.
    Confirm the code-owned launch-health mapping in
    `docs/runbooks/launch-health-alerts.md:268-302`,
    but do not treat that as proof of chat/pager delivery or production-console
    rule activation: those downstream integrations remain parked under WI-1916,
    while WI-2994 owns dispatch durability. Complete the manual
