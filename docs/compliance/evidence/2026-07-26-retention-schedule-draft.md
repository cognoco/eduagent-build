...
| G-8 | **Closed for durable event emission by WI-2346 (PR #2792):** exhausted Clerk and subscription-store teardown legs emit `app/account.deletion_teardown.failed` and `app/billing.subscription_store_teardown.failed`, respectively (`apps/api/src/inngest/functions/account-deletion.ts:81-100`; `apps/api/src/inngest/functions/billing-subscription-store-teardown.ts:62-79`). The code-owned launch-health mapping is documented in `docs/runbooks/launch-health-alerts.md:268-280`. Manual provider remediation is still required; real chat/pager integrations and production-console rule routing remain parked under WI-1916, so external delivery is not claimed | Provider copies (§2 row 15) | Medium residual — durable evidence now survives the exhausted workflow, but the GDPR Art 17 failure still requires an operator to finish external erasure and unproved production routing cannot be treated as a delivered page | WI-2346 closes durable emission; WI-1916 remains the post-launch owner for real chat/pager integrations and production-console rule routing |
...
10. **Partial-external-failure dead-letter drill.** In an isolated test
    ...
    Sentry exception, plus the exact PII-minimized durable events
    `app/account.deletion_teardown.failed` and
    `app/billing.subscription_store_teardown.failed` documented by WI-2346 in
    `docs/runbooks/deletion-irreversible-boundary.md` §5. Confirm the code-owned
