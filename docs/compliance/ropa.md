...
| 6 | **Learning sessions & transcripts** | `learning_sessions`, `session_events` (scoped `person_id`) | Deliver the tutoring conversation | Consent | LLM provider(s) via the router; Neon *(stored display name excluded for minors; adult first name may be disclosed; volunteered direct identifiers and explicit first-person special-category disclosures are filtered from textual provider payloads — DPIA §4 A13 / WI-2737; image-pixel residual remains)* | US (LLM) | **Raw transcript purged at 30 days** (`RETENTION_PURGE_ENABLED=true`, verified 2026-06-08) |
...
| 9 | **Semantic embeddings** | vectors from session summaries | Memory recall | Consent | **Voyage AI** receives filtered transcript text; Neon stores the derived pgvector *(WI-2737)* | US | Rebuilt on purge; life of person |
