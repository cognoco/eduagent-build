...
GET /account/export, and the schedule date, grace-period end and initiating person carried into
deletion_audit), or the controller registers self-service exercises, which needs a mechanism that does
...
| Self-service *export*: no record at all — route and service perform no write and no log | `apps/api/src/routes/account.ts`; `apps/api/src/services/identity-v2/export-v2.ts` | 2026-08-05 |
