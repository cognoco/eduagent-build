...
9. **RULED, one item OPEN** — consent-withdrawal bearer-token threat posture reviewed and approved (6.11; WI-2064, OPQ-114); the one mitigate-before-launch disposition (server-side revocation + token expiry for the `cw1` link) needs its linked implementation Work Item created and closed before store submission — see the posture doc's §5/§6 for the decision, owners, and tracking pointer.
