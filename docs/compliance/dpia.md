...
**Controller:** **ZWIZZLY AS**, org.nr **811696072**, Fiskekroken 3B, 0139 Oslo, Norway. **Proposed lead supervisory authority:** Norwegian Datatilsynet, subject to DPO/counsel confirmation of the factual main establishment. **DPO:** `[name / dpo@… — TODO]`.
**Product:** MentoMate — an AI tutoring app for learners aged 13+; under-13 access is unavailable in every country at launch. Any later under-13 product is a separately reviewed phase, not a dormant launch entitlement.
...
**Companion documents:** [`ropa.md`](ropa.md) (data register), [`art9-special-category-position.md`](art9-special-category-position.md), [`breach-response-plan.md`](breach-response-plan.md), [`2026-07-23-13-plus-eea-launch-country-ruling.md`](2026-07-23-13-plus-eea-launch-country-ruling.md).
**Schema source of truth:** the **built, live schema** (`packages/database/src/schema/identity.ts`), designed per [`docs/canon/identity/data-model.md`](../canon/identity/data-model.md). Historical data-flow evidence: [`history/2026-06-07-data-retention-and-erasure-audit.md`](history/2026-06-07-data-retention-and-erasure-audit.md). Built-state verification: [`2026-07-04-launch-compliance-closure-check-early-pass.md`](2026-07-04-launch-compliance-closure-check-early-pass.md).
...
- **Special-category (Art 9) posture** — the service does not solicit or intend
  to use special-category data, but incidental disclosure, inference,
  transmission, and temporary storage are foreseeable in an open-text AI
  service (§6.3;
...
- **Adult self-processing basis + granular purposes (WI-1193, DPIA gate condition 3):** a self-registered adult (no guardian) now has their OWN lawful basis — data-subject consent (Art 6(1)(a), stored in the `<regime>_<who>_consent` naming style as `adult_self_consent`) — and a durable, versioned terms-accepted fact (the consent-policy version in force at acceptance, kept separate from the lawful basis) recorded at signup, where adults previously held no consent record. The single blanket consent purpose is split into independently-recorded, independently-revocable granular purposes (general platform use and a separate LLM-disclosure purpose). An adult withdraws any one purpose through the authenticated self-service endpoint (`PUT /consent/self/withdraw`) without affecting the other; both the withdrawal and the report bind to the authenticated login→person, so an account member cannot act on another profile's consent. The current lawful basis + versioned terms-acceptance + accepted purposes (and any withdrawal) for the caller is retrievable in one query through the authenticated accountability report (`GET /consent/self/accountability`), and the terms-acceptance fact survives a withdrawal, satisfying Art 5(2)/7(1) accountability. Withdrawing a purpose records the consent withdrawal; downstream enforcement that gates processing on a withdrawn purpose (e.g. LLM disclosure) is tracked separately as WI-2372 and blocks the launch gate (WI-1577).
...
| 6.3 | **Incidental special-category disclosure or inference** (Art 9) | Medium | High | The product does not solicit or intentionally use special-category data. A central runtime attribution guard rejects or scrubs clinical characterisations before selected LLM-written durable fields are persisted. The controller proposes child-readable discouragement, broader durable-record suppression, short incidental-content retention, and a separate safeguarding procedure; the applicable Article 9 treatment and residual-risk conclusion require DPO/privacy advice. | Pending advice |
...
| 6.7 | **Data breach at us or a processor** | Low | High | Breach plan + 72h Datatilsynet process (A4); processor DPAs oblige vendor notification; secrets via Doppler; Sentry PII scrubbing; tenant isolation is app-layer only for launch — DB-layer RLS formally accepted as Branch B, not activated, per the signed [`rls-risk-acceptance-memo.md`](rls-risk-acceptance-memo.md) | Low–Med |
...
| 6.9 | **Wrong-regime / wrong-jurisdiction consent** | Low | High | Before launch, replace the current coarse `EU \| US \| ROW` / location-blind path with EEA-country habitual-residence capture and a server/store allowlist; populate the **policy engine** (`regimes`×`policy_cells`×`policy_rules`) with per-country self-consent ages and retain `knowledge_assertions` as the audit trail | Low only after country content and enforcement are production-verified |
| 6.10 | **Over-broad profiling / function creep** | Low | Med | Purpose-fence (§4); profiling limited to teaching personalisation | Low |
| 6.11 | **Non-expiring bearer-token consent-withdrawal link** (P0 email-parent path — leak, forwarding, interception, replay) | Low | Low | Full threat posture reviewed and dispositioned: [`2026-07-17-consent-withdrawal-bearer-token-threat-posture.md`](2026-07-17-consent-withdrawal-bearer-token-threat-posture.md) (WI-2064, OPQ-114). Scope-limited to withdraw/restore of one child, never a data read (`MMT-ADR-0029`); tokens are HMAC-signed and org/person-bound so tampering fails closed. Restore-by-bearer is accepted as shipped; going forward, restore should require an authenticated path, not link possession (product-policy ruling, named separately in the posture doc, not yet reflected in code/ADR). DPO (acting) + eng/security owner approved 2026-07-17. | Low; one mitigate-before-launch item tracked as **WI-2347** (server-side revocation + token expiry — see §9 item 9) |
...
2. **PART-MET — Provider DPAs signed** on business tier + transfer checks done (**OPEN — process, not repo-verifiable**); **no Gemini/Vertex for this app** — **MET in code**, enforced fail-closed + CI-guarded (`router.ts:559,648,1085`) (A11/A12, 6.4).
   - **OpenAI DPA (v.010126, executed 2026-07-24 — Docusign env. C7AE4DF0; signed copy: [`evidence/providers/OpenAI/2026-07-24-ZWIZZLY-AS-signed-openai-dpa.pdf`](evidence/providers/OpenAI/2026-07-24-ZWIZZLY-AS-signed-openai-dpa.pdf)):** standard OpenAI platform DPA, Zwizzly AS ↔ **OpenAI Ireland Ltd** (correct EEA contracting party; §4.1 routes EEA data via Ireland with SCCs/adequacy for onward transfers — feeds §7 TIA). Points the DPO must weigh at sign-off: **(a)** Schedule 1 §5 declares "no sensitive data intended… unless the user includes it unexpectedly in unstructured data" — children's mentor-chat will routinely carry emotional/health-adjacent content in unstructured text; this DPIA (6.3, 6.4) owns that risk, the DPA adds no child-specific safeguards. **(b)** Breach notice is "without undue delay" with no fixed hour count (§2.7) — our 72h Datatilsynet clock (6.7, A4) cannot assume vendor runway. **(c)** Audit rights are annual, at our expense, satisfiable by an audit-report summary (§2.8) — we rely on OpenAI's SOC 2, not independent audit ability. **(d)** DPA §3.3 puts retention/deletion configuration on us — code-verified 2026-07-19: production OpenAI surface is **chat completions only** via raw fetch (`apps/api/src/services/llm/providers/openai.ts:28`), no `store: true`, no files/assistants/responses endpoints, so no dashboard-persisted completions; remaining human check = org-level logging/retention + ZDR eligibility in the OpenAI dashboard (org-vUMJ2DGK2YaiSfdoqoTYkzDZ). **(e)** Training use + liability caps live in the Services Agreement, not the DPA — keep that snapshot with this file. **(f) Coverage gap:** this DPA covers OpenAI-direct traffic only; Cerebras (gpt-oss) is covered separately — per the 2026-07-28 evidence review ([`evidence/providers/README.md`](evidence/providers/README.md)) the self-serve Inference T&Cs incorporate Cerebras's DPA by reference (SCC Modules 1–3 deemed executed); written confirmation from Cerebras still pending. **(g) OPEN — key/org mismatch (live-verified 2026-07-19):** the stg + prd `OPENAI_API_KEY`s in Doppler resolve to `openai-organization: user-mcnev9o6gxeyh3yxgxhof1qp` (a personal user account), NOT the DPA-bound org `org-vUMJ2DGK2YaiSfdoqoTYkzDZ` — so live traffic is currently outside DPA coverage and on consumer terms. Remedy: issue keys under the DPA org, rotate both Doppler configs, re-verify the response header shows the org id. This blocks condition 2 even with the DPA signed.
...
4. **PART-MET — Privacy policy pre-publish TODOs** — controller identity, registered address, and the C3 entity reconciliation are resolved across the policy, DPIA, ROPA, and breach plan. DPO name remains OPEN (A5). A UK representative is dormant because the UK is denylisted by the launch-country ruling; it reopens only if the UK is later enabled. The other 2026-07-04 closure-check policy findings remain separately tracked: **C1** (soften the analytics claim — only Sentry error monitoring exists, no product analytics) and **C4** (add an explicit "profiling is present and lawful" disclosure, Art 13(2)(f)).
...
7. **OPEN — `person_retain.*.retention_period` values must be set** (not placeholder defaults) — counsel fills; enforced by the Phase-F launch-readiness guard (the value-seam test).
...
9. **RULED, one item OPEN** — consent-withdrawal bearer-token threat posture reviewed and approved (6.11; WI-2064, OPQ-114); the one mitigate-before-launch disposition (server-side revocation + token expiry for the `cw1` link) is tracked as **WI-2347**, which must close before store submission — see the posture doc's §5/§6 for the decision, owners, and WI-2347's link.

> **Substrate condition (overarching) — SATISFIED for the early pass (2026-07-10):** the identity-foundation baseline **is built and migrated** — production cutover complete, legacy tables dropped (header note). The required re-confirmation pass against the *built* schema was performed 2026-07-04 (WI-1507 closure check, EARLY pass) and its live `file:line` citations are folded into this v0.2. The store data-safety worksheet was refreshed against identity v2 and the 13+ floor on 2026-07-23. **Still owed before store submission:** the FINAL-GATE re-run against then-current code, country-allowlist implementation evidence, and the production catalog query for leftover legacy tables (feeds WI-1442).
