...
- **Adult self-processing basis + granular purposes (WI-1193, DPIA gate condition 3):** a self-registered adult (no guardian) now has their OWN lawful basis — data-subject consent (Art 6(1)(a)) — and terms-accepted fact recorded at signup, where adults previously held no consent record. The single blanket consent purpose is split into independently-recorded, independently-revocable granular purposes (general platform use and a separate LLM-disclosure purpose). An adult withdraws any one purpose through the authenticated self-service endpoint (`PUT /consent/self/withdraw`) without affecting the other, and the current lawful basis + terms-accepted timestamp + accepted purposes for any person is retrievable in one accountability query, satisfying Art 5(2)/7(1) accountability.
