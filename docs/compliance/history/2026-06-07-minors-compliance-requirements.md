...
**Status:** Historical requirements catalogue. Its UK/US scope, country table, and live/dormant classifications are superseded by the 2026-07-23 [`13+ EEA launch-country ruling`](../2026-07-23-13-plus-eea-launch-country-ruling.md). Retain for audit provenance; do not use it as launch authority.
**Historical scope:** EU/EEA + UK + USA. AI tutoring app, sold to consumers, used by children.

> **2026-06-07 change:** added **A24** (storage limitation, proportionality, and purpose-fencing of the persistent learning memory), prompted by the code-verified `2026-06-07-data-retention-and-erasure-audit.md`. That audit confirmed account-deletion cleanly cascades the whole learning memory, but surfaced three live issues: the Clerk login identity is never deleted in-app (Art 17 gap), verbatim learner answers survive the "30-day transcript" purge indefinitely (misleading-notice risk), and the 30-day purge sits behind an env flag that defaults OFF and must be verified in production.
>
> **2026-06-08 update:** of those three, the Clerk Art 17 gap is **fixed** (commit `9137c7961` — Clerk identity now erased on account deletion, with a break test), the purge flag is **confirmed `true` in prod Doppler** and now documented in `.env.example`, and the misleading-notice risk is **mitigated** by the A5 privacy-policy rewrite (`docs/compliance/privacy-policy.html`, June 2026). Still open: age-out of verbatim fields (A24-b) and the `organizations`/`byok_waitlist` survivors (audit R3).
...
1. Appoint the DPO (A2) → they drive the DPIA (A1). Hand them the code-verified `2026-06-07-data-retention-and-erasure-audit.md` and A24 — the learning-memory retention rule, proportionality paragraph, and purpose-fence are DPIA content.
