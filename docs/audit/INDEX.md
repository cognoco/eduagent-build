...
| [`2026-08-07-built-vs-documented-state-audit.md`](2026-08-07-built-vs-documented-state-audit.md) | Sixteen verified doc-vs-code contradictions in L1 canon, Accepted ADRs and non-terminal `_wip/` trackers, audited at `019c72e36`. Assessment only — every finding is captured, none is fixed. | Drain the seventeen captures — `WI-3147`–`WI-3162`, one per verified built-vs-documented contradiction, plus `WI-3163`, the predicate sweep over the 1,264 candidates this audit could only establish existence for; all at `Stage=Captured` with `Workstream` deliberately unset. Archive this report once they close. |
