...
| ~~**R3**~~ → **R3a** | ~~Erase `byok_waitlist` email on account deletion~~ — **RESOLVED 2026-06-08**: erasure wired into `executeDeletion` (captures the deleted account's email atomically via `DELETE ... RETURNING`, then erases the matching `byok_waitlist` row). Real-DB red→green break test added to `deletion.integration.test.ts`. | eng | ~~MEDIUM~~ → done | No (resolved) |
| **R3b** | Erase/drop the orphaned `organizations` row (T1 backfill writes the owner display-name into `organizations.name` → PII, no FK to accounts, survives deletion). Preferred close = the ratified T1 revert dropping the table; fallback = explicit org-row erasure if the revert slips past launch. | eng | MEDIUM | DPIA-tracked (launch condition) |
