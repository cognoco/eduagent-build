# /goal Spike: GC1 Internal Mock Drain (API) - Codex Edition

> **For agentic workers:** Execute this as one continuous `/goal` session. Use subagents only as parallel workers inside the same session when available; otherwise execute inline with the same batch discipline. The coordinator owns verification, commits, and final status.
...
Baseline from this checkout on 2026-05-14:
...
Recompute before execution. The working tree may have moved.
...
```bash
shadow_count=$(
  rg -n "jest\.(mock|doMock)\(['\"\`]\.\.?/" --glob "*.test.ts" apps/api/ --glob "*.integration.test.ts" tests/integration/ \
...
Convert 5 simple files first. For each file:
...
```bash
pnpm exec jest --config apps/api/jest.config.cjs --findRelatedTests <TARGET_FILE> --no-coverage
```

If the target is under `tests/integration/`, run the relevant integration config instead:

```bash
pnpm exec jest --config tests/integration/jest.config.cjs --findRelatedTests <TARGET_FILE> --no-coverage --maxWorkers=2
```
...
6. Commit the verified batch with the commit skill. Do not push.
...
Current high-count examples from this checkout:
...
Use this only for disjoint single-file work inside the same `/goal` session.
...
5. Continue from the remaining inventory, preserving unrelated dirty files.
