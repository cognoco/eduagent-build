...
NOT TRACKED: docs/audience-matrix.md does not flag this — it lists session/index.tsx:1109 sessionIsOwner as intentional F5 content gating and treats non-owner uniformly as proxy. So alreadyTracked=false.
...
NOT tracked in docs/audience-matrix.md: F1-F14 there are navigation-contract findings; the only consent items (F2) concern shell-level UI consent-overlay interception, not parent-oversight linkage. docs/audit/.../consolidated-triage.md:138 references a DIFFERENT consent vuln (attacker sets parentEmail to bomb addresses), not this oversight dead-end.
...
Tracking status: NOT tracked as a gap. docs/audience-matrix.md (lines 59, 120, 122) and docs/flows/master-directory/account/ACCOUNT-30.md treat owner-gating of export/delete as intended design (F5/F7 are about consolidating duplicated isOwner UI gating into resolveNavigationContract, not about the absence of a per-subject data right). No master-directory flow covers minor-non-owner self-export/erasure.
...
Tracking: acknowledged only in archived backlog (docs/_archive/plans/done/app evolution plan/2026-05-06-hidden-wins-backlog.md:115-117), NOT in the live docs/audience-matrix.md (no entry found). Treat as effectively untracked for the live matrix.
...
alreadyTracked: NO. docs/audience-matrix.md F1-F14 are nav-gating gaps; none cover non-owner self-deletion/leave.
...
6. Not tracked: grep of docs/audience-matrix.md and docs/flows/master-directory/ for transfer/reassign/leave-account/children-persist found only unrelated "handoff" hits (auth redirect, recaps). No flow doc owns ownership-transfer or owner-exit-preserving-children. alreadyTracked=false.
...
Severity adjusted to MEDIUM (not high): the minor is the account owner and retains full self-service control (export/delete via their own owner-gated more/privacy.tsx + account.tsx surfaces), and the 11+ floor with denial-deletes-profile limits the blast radius. The GDPR/parental-rights dead-end is real but bounded, and the false "parent dashboard" copy is a copy bug layered on top. Not tracked in docs/audience-matrix.md (F2 there is about nav-shell consent interception, a different layer).
...
TRACKING: docs/audience-matrix.md acknowledges "push-delivery gaps" as a category (lines 28, 126) but no specific finding F1-F14 documents this guardian-permission-never-requested gap (F13/F14 are reserved/empty). Not explicitly tracked.
...
Severity adjusted to medium (not high): the default device-locale→tutor-prose path works correctly for the 7 overlapping locales, so most users get a reasonable default; the dead-end only bites cs/fr/it learners and anyone wanting to decouple — a real but narrow population. Not tracked in docs/audience-matrix.md (no conversation-language entry found; only the F1-F14 scaffold note).
...
SEVERITY adjusted to medium: true dead-end with no in-app recovery, but pre-launch (no active users per memory) and the destructive workaround (delete + re-register) technically exists. Not tracked in docs/audience-matrix.md F1-F14 nor in docs/flows/master-directory as a logical gap.
...
Not explicitly tracked in docs/flows or docs/audience-matrix.md (the 2026-04-18 quiz-gaps-completion-design treats rounds as terminal by design and documents no resume concept).
