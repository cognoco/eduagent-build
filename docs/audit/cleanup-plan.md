...
| P3 | AUDIT-TESTS-2C — Drain LLM allowlist: migrate ~~3~~ **2** remaining KNOWN_OFFENDERS to HTTP-boundary or provider-registry pattern | todo | | **PR-06** | `apps/api/src/services/session-summary.integration.test.ts`, `apps/api/src/services/quiz/vocabulary.integration.test.ts` | [YELLOW] ~~~10-15 hr~~ ~7-10 hr (scope shrank). **Update (2026-05-09):** PR #180 removed all `jest.mock` from `interview-persist-curriculum.integration.test.ts` — drained automatically. 2 offenders remain (`session-summary`, `vocabulary`). |
