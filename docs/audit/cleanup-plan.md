...
| D-C4-3 | 2026-05-04 | `personaFromBirthYear()` root migration | **Option A: Replace with `computeAgeBracket(birthYear)` returning `'child' \| 'adolescent' \| 'adult'`.** Rename function + type (`Persona` → `AgeBracket`), update 5 caller sites to branch on new labels, update `consent-copy.ts` to accept `AgeBracket`, update 4 test mocks. Thresholds unchanged (< 13, 13-17, 18+). `relearn.tsx` included in same migration (D-C4-4). | Developer readability: eliminates the misleading `'parent'` label (an 18-year-old self-learner is not a parent). No user-facing behavior change — the function only drives age-appropriate copy tone, and actual parents are the only adults exercising the differentiated UI in practice. But code-level vocabulary should match Epic 12's three-axis decomposition (age from `birthYear`, role from `familyLinks`, intent from session state). `isGuardianProfile()` already handles the "has children" question correctly — `computeAgeBracket()` should only answer the age question. |
...
| P2 | AUDIT-MOBILE-2b — Codify brand/animation/celebration hex carve-out in CLAUDE.md "Non-Negotiable Engineering Rules" | todo | | | `CLAUDE.md` (Non-Negotiable Engineering Rules section) | [YELLOW] ~15 min. Resolved by D-C4-2. |
...
| P7 | EPIC-12-ROOT — Migrate `personaFromBirthYear()` to `computeAgeBracket()` across 5 callers: `_layout.tsx` (×2 consent copy), `session-summary/[sessionId].tsx`, `mentor-memory.tsx`, `relearn.tsx`. Rename function, update return values from persona labels to age-bracket labels, update all callers to branch on `'child'\|'adolescent'\|'adult'` instead of `'teen'\|'learner'\|'parent'`. Update 4 test mocks. **Post-land follow-up:** update `.claude/memory/project_persona_removal.md` line 31 (`personaFromBirthYear` → `computeAgeBracket`). | todo | | | `apps/mobile/src/lib/profile.ts`, `apps/mobile/src/lib/consent-copy.ts`, `apps/mobile/src/lib/consent-copy.test.ts`, `apps/mobile/src/app/(app)/_layout.tsx`, `apps/mobile/src/app/(app)/_layout.test.tsx`, `apps/mobile/src/app/session-summary/[sessionId].tsx`, `apps/mobile/src/app/session-summary/[sessionId].test.tsx`, `apps/mobile/src/app/(app)/mentor-memory.tsx`, `apps/mobile/src/app/(app)/mentor-memory.test.tsx`, `apps/mobile/src/app/(app)/topic/relearn.tsx`, `apps/mobile/src/app/(app)/topic/relearn.test.tsx` | [YELLOW-RED] ~1-2 hr. Root fix for all persona-vocabulary fossils. Resolved by D-C4-3. |
...
| Phase | Description | Status | Owner | PR | Files-claimed | Notes |
|---|---|---|---|---|---|---|
| P1 | AUDIT-DEPENDENCY-DRIFT-2-1a-EXECUTE — Apply 1a-DECISION worksheet directive (32 deletes: 23 Bucket B + 7 Bucket C from root, 2 Bucket E from api; align dotenv version; move `react-native-css-interop` to mobile keeping patch at root; align 8 drifted pins to `~`; lockfile regen) | todo | | | `package.json`, `apps/api/package.json`, `apps/mobile/package.json`, `pnpm-lock.yaml` | [YELLOW-RED] agent ~20-30 min, human review 1-2 hr. Shrinks root manifest from 83 to ~53 entries. D-C5-1a + D-C5-2 resolved. Align 8 drifted pins to `~` during execution. |
...
| P3 | AUDIT-PACKAGE-SCRIPTS-2c — Add `apps/api/tsconfig.spec.json`; update `tsconfig.json` references; switch `jest.config.cjs` to spec config | todo | | | `apps/api/tsconfig.spec.json` (new), `apps/api/tsconfig.json`, `apps/api/jest.config.cjs` | [YELLOW] ~30 min. Resolved by D-C6-1 (switch jest to spec). Verify: `pnpm exec nx run api:test` + `pnpm exec nx run api:typecheck`. |
| P4 | AUDIT-PACKAGE-SCRIPTS-2d — Drop the broken `nx.json` `targetDefaults.e2e` block (lines 71-87) | todo | | | `nx.json` | [YELLOW] ~10 min. Resolved by D-C6-2. Verify: `nx graph` still works after removal. |
...
| P6 | D-C7-1 sweep — Verify ~9 Active Work memory entries against `git log --since=2026-04-18`; archive shipped | done | | (D-C7-1 session) | `.claude/memory/MEMORY.md`, `.claude/memory/_archive/project_{parent_visibility_spec,ux_review_pass,open_bugs,multiple_coaching_cards,f8_memory_source_refs,deploy_config_open,prod_approval_gate,template_repo}.md` | Resolved by D-C7-1. 8 archived, 2 Notion issues created, 1 flagged for post-P7 update. |
| P7 | D-MEM-1 reconciliation — Mark `docs/audit/claude-optimization/memory-overlap-flags.md` as superseded for entries the drift audit classified as REINFORCES | todo | | | `docs/audit/claude-optimization/memory-overlap-flags.md` | [YELLOW] ~15 min. Resolved by D-MEM-1. |
