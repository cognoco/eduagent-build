...
**Last updated:** 2026-05-13 UTC (governance evidence reconciliation: PR-15d no-op closure, legacy punchlist supersession, MentoMate naming direction, PR review timing memory — see DEV-009)
**Branch:** `consistency2`
...
| PR-15d | C6 | P3d | Closed 2026-05-12 as no-op after re-grep found zero TS7006/TS2345 matches in `eval-llm/flows/*`; residual TS6307 tracked as paper-trail-only evidence in DEV-009. |
...
| P6 | AUDIT-EXTREFS-2 — EduAgent → MentoMate naming sweep (NOT `@eduagent/*` package names) | todo | | **PR-23** | `docs/**/*.md`, `README.md`, `apps/**/*.{ts,tsx}` (string literals only) | [YELLOW] ~1 hr. Canonical direction is old product name **EduAgent** to current product name **MentoMate**; do not invert this to MentoMate→EduAgent. **Governance:** `.archon/governance-constraints.md` §8 (cross-cutting sweep — i18n locale files in `apps/mobile/src/i18n/locales/` MUST be kept in sync; the i18n staleness pre-commit hook fires when `en.json` is staged. Surface and update each user-facing locale key, don't just edit en.json). |
...
**Status:** `processed-2026-05-13`
**Source:** Review findings I11-I13 on cleanup governance docs and project memory.
**Finding:** Three bookkeeping drifts weakened audit evidence without changing product behavior: (1) PR-15d was closed as a no-op in the C6 phase row but still appeared open in the PR execution summary, (2) the superseded 2026-05-02 punchlist received a new live TS6307 item even though `cleanup-plan.md` is the maintained tracker, and (3) PR check memory lost the explicit instruction that automated review comments can arrive after green checks.

**Root cause:** Evidence was split across active and historical trackers, then one revert removed memory context that is still consistent with the canonical PR Review & CI Protocol. The brand sweep wording also mixed casing around the current product name; use **MentoMate** as the product spelling and **EduAgent → MentoMate** as the canonical rename direction.

**Delta applied:**
- PR Execution Plan row `PR-15d` now records the no-op closure and points residual TS6307 paper trail here instead of leaving the row looking executable.
- Legacy `docs/audit/2026-05-02-artefact-consistency-punchlist.md` is marked historical-only; the stray TS6307 live item was removed from it and preserved in this DEV entry.
- C8 P6 / PR-23 wording now uses `EduAgent → MentoMate` and explicitly says not to invert the direction.
- `.claude/memory/feedback_pr_required_checks.md` restored the automated-review timing instruction as a pointer to the canonical PR protocol.

**Residual paper trail:** While closing PR-15d as no-op, `pnpm exec tsc --noEmit -p apps/api/tsconfig.spec.json` still surfaced residual TS6307 ("File not listed within the file list of project") on `apps/api/eval-llm/flows/*.ts`. This is paper-trail only for now: CI uses jest + ts-jest for these flows and does not consult `tsconfig.spec.json`'s `include`. If it becomes annoying, extend `apps/api/tsconfig.spec.json` `include` to cover `eval-llm/**` and verify with the same re-grep.
