...
| Navigation documentation | Audience matrix still describes the removed in-`LearnerScreen` home branch. | `apps/mobile/src/app/(app)/home.tsx` branches on `navigationContract.home.screen`; `AGENTS.md` is current. | **Correct or retire `docs/compliance/audience-matrix.md`.** |
