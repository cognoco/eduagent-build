...
| Navigation documentation | **Resolved 2026-07-23:** the relocated audience matrix now records that `home.tsx` branches on `navigationContract.home.screen` and that the inline `LearnerScreen` branch was removed. | `docs/compliance/audience-matrix.md`; `apps/mobile/src/app/(app)/home.tsx`; `AGENTS.md`. | **Resolved; matrix retained as historical evidence, not current line-map authority.** |
