...
- **30-day transcript purge — confirmed live (2026-06-08):** the purge sits behind `RETENTION_PURGE_ENABLED`, which defaults OFF and is not in committed config, but is **confirmed `true` in production Doppler** — so transcripts do purge at 30 days (audit item R2, resolved). Residual hardening only: add the flag to committed config docs so it can't silently regress.
