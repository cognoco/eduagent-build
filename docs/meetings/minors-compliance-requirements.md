...
>
> **2026-06-08 update:** of those three, the Clerk Art 17 gap is **fixed** (commit `9137c7961` — Clerk identity now erased on account deletion, with a break test), the purge flag is **confirmed `true` in prod Doppler** and now documented in `.env.example`, and the misleading-notice risk is **mitigated** by the A5 privacy-policy rewrite (`docs/privacy-policy.html`, June 2026). Still open: age-out of verbatim fields (A24-b) and the `organizations`/`byok_waitlist` survivors (audit R3).
