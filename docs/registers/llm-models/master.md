...
|---|---|---|
| **H5 — output moderation** | **CLOSED 2026-08-07** by **`WI-1900` — the work item that extended output moderation from minors-only to adult traffic; it did not build the mechanism, it widened the population and made the adult rate configurable**. Both populations now have an output-content check on the displayed reply, via one mechanism at two settings. **Minors — WI-1365:** synchronous, fail-CLOSED enforcing gate (`services/suitability-gate.ts`), full coverage (1.0), runs LAST over the final candidate reply and replaces it on a `violation`; fail-OPEN-with-alarm when the judge is unavailable. Gated by `JUDGE_ENFORCEMENT_ENABLED` — **actual Doppler values read 2026-08-07: prd `true`, stg UNSET, dev UNSET** (unset resolves to `'false'` via `config.ts`, so the gate is inert wherever the key is absent). **Adults — WI-1900:** post-display, async, sampled check on the same `runSuitabilityJudge` rubric via `app/judge.suitability_requested` → `inngest/functions/judge-suitability.ts`; a blocking-grade verdict raises the existing structured escalation (`app/safety.suitability_blocked`, `mode: 'observed'`), a degraded judge raises `app/safety.suitability_judge_unavailable`. Coverage is config-valued (`JUDGE_ADULT_SUITABILITY_SAMPLING`, default 0.1) and ratchets toward full by configuration, not redesign. Gated by `JUDGE_FRAMEWORK_ENABLED` — **actual Doppler values read 2026-08-07: prd `true`, stg UNSET, dev UNSET**. Adult-path acceptance evidence: eval scenario `JSE02-adult-reply-is-judged` (`apps/api/eval-llm/flows/judge-suitability.ts`) plus the negative-path unit tests in `apps/api/src/inngest/functions/judge-suitability.test.ts`. | **Two residual caveats, deliberate and recorded, not gaps in the gate.** (1) The adult path is **post-display**: fail-closed is unavailable there by construction, so adult coverage is detect-and-escalate, not block — operator-ruled 2026-08-04 (adult residual risk is quality-class; a synchronous adult check fails the no-perceptible-delay requirement at full adult volume). Adult detections are deliberately excluded from `suitabilityBlockedCount` so the operator digest never reports a block that did not happen. (2) At the default 0.1, ~90% of adult replies are unjudged; raising it is a Doppler change, not a code change. **Neither judge flag exists in the `stg` Doppler config**, so staging exercises the disabled path in both cases — pre-prod cannot validate this gate today. |
...
| **Cutover contracts** | Cerebras compliance triplet (ZDR + no-training + executed DPA with ZDR in the text; SCCs + TIA, Cerebras US-only); OpenAI ZDR-for-minors (covers GPT-5 mini secondary + gpt-5.4); Art 28 DPAs. Cerebras controller review completed 2026-07-24: [`2026-07-24-cerebras-processor-transfer-assessment.md`](../../compliance/assessments/providers/2026-07-24-cerebras-processor-transfer-assessment.md). | **Cerebras remains vendor-blocked:** the Trust Center DPA is an unexecuted enterprise template and needs the assessment's eight contractual corrections/confirmations. This now gates continued launch use, because V2 is already live; it is not an engineering gap. |
