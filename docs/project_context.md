...
- **Routes:** `POST /challenge-round/{accept,decline,abort}` — all profile-scoped via `createScopedRepository`. Not yet registered in `AppType`; the mobile hook (`use-challenge-round.ts`) uses raw `fetch` with auth headers rather than the typed Hono client. The offer is not a route call — eligibility comes from `evaluateChallengeReadiness()` (see Trigger, above) via `challengeEligible`, which gates the offer prompt block (see Prompt blocks, above); the LLM then emits `signals.challenge_round_offer` (see Envelope signals, above). The "Too easy" chip otherwise falls through to today's `too_easy` system-prompt dispatch.
...
- **History:** the persistent Challenge mode toggle (`learningMode: 'serious' | 'casual'`) was removed in Phase 0 (PR #325); today's `casual` is the single default tone, and rigor is expressed per-Challenge-Round rather than globally.
...
- **Hardcoded-JSX-literal ratchet (Phase 3):** `scripts/check-i18n-jsx-literals.ts` is a `ts-morph` AST walker that blocks NEW hardcoded English in `JsxText` nodes (`<Text>Add child</Text>`), JSX-children `StringLiteral`/`NoSubstitutionTemplateLiteral` nodes (`<Text>{'Continue'}</Text>`, conditional/`&&`/`??` children), and known user-copy JSX attributes (`label`, `accessibilityLabel`, `title`, `placeholder`, etc.) across `apps/mobile/src/**/*.tsx` — the complement to the `t()`-only orphan-key checker. Forward-only baseline ratchet: existing literals are grandfathered in `scripts/i18n-jsx-literals-baseline.json`; child/text entries are keyed on `{file, kind, text}`, and attribute entries are keyed on `{file, kind, prop, text}` (line-independent, so reformatting doesn't churn). Attribute scanning ignores non-copy props such as `testID`, styles/classes, roles, IDs, routes, image/source paths, metadata, unknown custom props, and translation-key literals. CI step `i18n hardcoded-JSX-literal check` in `ci.yml` runs it on every push; unit-tested in `scripts/check-i18n-jsx-literals.test.ts`. Run `pnpm check:i18n:jsx-literals --accept` to refresh the baseline when adding genuinely non-translatable JSX copy.
