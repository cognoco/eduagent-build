...
- **Model routing by conversation state (escalation rung),** not initial classification. Standard Gemini routing uses Flash for rung 1-2 and Gemini Pro for rung 3+. Entitled Plus or upgraded profiles unlock advanced help from rung 4, but GPT-5.4 is reserved as the default OpenAI candidate for rung 5+; rung 4 uses the cheaper proven path (Claude when available, otherwise Gemini). Family standard profiles are Gemini-only, including fallback.
