...
**Status:** Accepted · 2026-07-21 · amended 2026-07-22 · amended 2026-07-26 · **Scope:** Mentor-notice eligibility, interaction lifecycle, AI contract, persistence authority, rollout, and rollback; plus the product-wide clinical-safety boundary for persisted learning text · **Deciders:** Product sign-off: MentoMate operator; Architect (Jørn Jørgensen), rulings 2026-07-21, 2026-07-22, and 2026-07-26
...
The 2026-07-26 amendment reverses two positions the 2026-07-22 amendment had taken. First, on the response cap: the operator did not ratify a cap-driven `not_yet`, and ruled that attempt lifecycle is owned separately from notice status — a valid `continue` transitions nothing at any learner response, the cap ends only the current re-check attempt, and the notice stays unresolved and re-offerable. Second, on clinical persistence: `refer` extends to user provenance, so user-authored ambiguity is judged — with no producing model to exclude — rather than blocking outright, while a missing or blank LLM producer vendor and migration/backfill ambiguity still fail closed. Sections 2.4 and 4.6 below state the 2026-07-26 contract, not the superseded 2026-07-22 wording; the genuinely fail-closed cases are unchanged.
...
4. A completed re-check is evaluated by an independent server-side judge under explicit producer-vendor exclusion, never by the tutor that produced the exchange. The server accepts only the bounded outcomes `locked_in`, `not_yet`, `dismissed`, `deferred`, or `continue`, each with its corresponding strict reason. A valid `continue` leaves notice lifecycle state unchanged at every learner response, including the third. Attempt lifecycle is owned separately from notice status: on reaching the three-response cap after a valid `continue`, the server ends the current re-check attempt and detaches that attempt's bookkeeping, while the notice itself remains unresolved and eligible for a later re-offer under ordinary eligibility and cooldown rules. Only an unresolved evaluation deterministically terminalizes — for example an unavailable or malformed judgment, a referenced answer event the server cannot find, or a verdict whose reason does not match its outcome — and only at the third learner response, where the safe terminal is `not_yet`. The unresolved set is not closed by this list; every member fails in the same safe direction. Before the third response an unresolved evaluation cannot terminalize.
...
6. `Not now` is not dismissal. It deterministically records `deferred` for the current learning day. A learning day begins at local 04:00 in the learner's stored IANA time zone; before 04:00 belongs to the preceding civil date. If that zone is invalid or unavailable, the server applies the same 04:00 boundary to UTC and derives the learning-day civil date in UTC. The notice may resurface quietly on a later learning day if it remains eligible.
...
6. Every persisted learning-text boundary uses one shared multilingual, normalization-aware safety gate covering all supported Conversation Languages and cross-language phrases. Deterministically protected person-attributed clinical text is blocked. Ambiguous text is referred to an independent judge on a three-way provenance split: learner- and user-authored text refers with judge independence `not-applicable` and no producer vendor, because such text has no model producer to exclude; LLM-authored text refers only with a known, non-blank producer vendor; migration and backfill text never refers. Referred text may proceed only when that judge returns the strict educational-reference allowance; ambiguity alone is never grounds to block user-authored educational text. A missing or blank LLM producer vendor, migration and backfill ambiguity, judge unavailability, malformed output, or an unclear verdict fails closed without sending the protected text to another external service.
