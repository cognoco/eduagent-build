...
- **Subscription DB-row teardown is solved inside this transaction**: the subscription row(s) for the org are deleted before the person/org drops, satisfying both RESTRICT FKs; `subscription_payers` cascade off automatically. The Stripe/RevenueCat **store-level cancellation** (the billing-provider API call) is deliberately *not* part of this erasure transaction — it carries billing-domain side effects (chargeback retain, store entitlement revocation) and is owned by separate billing-cancellation work.
- **Legacy pre-cutover account tables need no erasure step.** Every environment this deletion path runs against was rebuilt by the one-time baseline reset (MMT-ADR-0012), so the pre-cutover `accounts`/`profiles` tables do not physically exist there — there is no legacy PII for the erasure to miss, and a `DELETE FROM accounts` would fail on a nonexistent relation. (Ruled moot by operator, 2026-06-20.)
...
4. **Solve full subscription teardown here, including the store side.** Rejected — the DB-row teardown IS solved here, but the store-cancellation API call (Stripe/RevenueCat) carries billing-domain side effects (chargeback retain, store entitlement revocation) that do not belong inside an erasure transaction; it is separate billing work by design.
