...
**Status:** Accepted · 2026-06-03 · *living; last revised 2026-08-04 — pre-live, this doc is edited in place per the Part II pre-live override* · **Class:** Meta / process — *constitutional* (governs the ADR system itself, not the product) · **Scope:** repo-wide documentation architecture · **Deciders:** Architect (jjoerg) + Claude
...
| *Cross-cutting (unnumbered):* **Agent doctrine** | root `CLAUDE.md` / `AGENTS.md`, `project_context.md` | — | For hierarchy-class content: *"has this line **earned its inline bytes** under the presence dial?"* | dial-governed digest of the hierarchy, per the estate rule `NEX-ADR-0000` §I.2 |

The L0–L4 numbers are sequential; **Decisions is L2**, placed against canon because it is the *why* behind canon's *what*. **Agent doctrine is deliberately unnumbered** — it is orthogonal, spanning every stratum, not a rung in the stack. Its relationship to the hierarchy is governed by the estate rule `NEX-ADR-0000` §I.2 (the presence dial, in the `cognoco/nexus` repo): hierarchy content is never canonical in an agent file, enters it only when it earns front-of-mind presence, and always traces its canonical source; the file's non-hierarchy instruction content (behavior, conventions, commands) is outside that rule's scope.
