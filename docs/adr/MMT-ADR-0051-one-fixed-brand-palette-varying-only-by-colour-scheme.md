...
- Removing that machinery is safe by construction: with no selection set, the applied palette is already the fixed one, so deletion is not a visual change. That is a property worth stating, because it is what makes the cleanup low-risk rather than a re-theming.
