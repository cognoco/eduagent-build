...
- **The parent's resulting edge is conditional on consent status:** for a **consent-capable** teen the parent becomes **admin + Payer + an *optional* Mentorship the teen grants** (inv 19 opt-in) — **never auto-Guardianship** (the teen self-consents; inv 14/19). (The below-consent-age variant, which needs Guardianship + VPC via R13, stays Phase-D-deferred.)
