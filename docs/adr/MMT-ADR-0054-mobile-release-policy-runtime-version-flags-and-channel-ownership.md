...
Delivery follows EAS Update's channel→branch model: updates are published to a **branch**; every install carries a fixed **channel** baked in at build time; and a server-side channel→branch mapping decides which branch each channel serves — installed binaries never change channel. The channels are `production` (store installs), `preview` (internal testing, CI-published), and `development` (dev clients); the `fallback` **branch** is a parked side-track holding a pre-built rollback bundle with the redesigned navigation off, so a broken production navigation can be retreated from in minutes — by remapping the `production` channel to the `fallback` branch server-side — instead of through a multi-day store release.
...
- **A version bump is a release act, never cosmetic.** The app version moves only as part of a deliberate store release. Bumping it removes prior-version installs from the audience of subsequent publishes; they keep working, and the store upgrade is the standard path forward. Serving a superseded runtime again is possible only as a deliberate backport publish (§6).
...
A change is **native-affecting** — and therefore requires a native build, never OTA alone — when it touches any of: `apps/mobile/app.json`, `apps/mobile/package.json`, `apps/mobile/eas.json`, anything under `apps/mobile/plugins/`, `apps/mobile/android/`, `apps/mobile/ios/`, **any file the app config consumes at prebuild** (`apps/mobile/google-services.json` and the icon/splash/adaptive-icon image assets referenced from the app config are the current members of that class), **or any dependency-resolution input whose change can alter the mobile app's native modules** — the root `package.json` (overrides/resolutions), `pnpm-lock.yaml`, and `patches/`. A lockfile or patch change that reaches the mobile dependency graph is a native change even when no `apps/mobile/` file moved; a resolution change provably confined outside that graph is not. Everything else in the mobile app is JavaScript-only and OTA-eligible within its runtime version.
...
- **The fallback machinery is a rollback path in the sense of MMT-ADR-0049.** Deleting the fallback branch or the ability to remap the `production` channel to it, disabling updates in the app config, or removing the flag off-states that make Config F expressible destroys the last cheap rollback path and requires the explicit, loss-naming human confirmation that ADR ratifies. Green readiness gates do not substitute.

### 5. The fallback must be proven before the first production rollout — hard gate

Before the first production store rollout, a **Config F bundle must have been published to the fallback branch through the gated workflow and verified** — credentials present, publish succeeded, bundle carries Config F at the production runtime version. A rehearsal publish reaches no production-channel install (the parked branch serves only builds baked with the `fallback` channel — internal rollback-test installs, if any exist — until the `production` channel is deliberately remapped to it, and the rehearsal never performs that remap), so this gate is cheap; a fallback that has never been exercised is a drawing of a safety net, and the moment its defects surface is by definition the worst one. Ruled hard (not advisory) in the 2026-08-04 sitting: the missing-credential state of the rollback workflow at ruling time was judged exactly the kind of latent defect this gate exists to flush out.
...
- **Each version bump** partitions the OTA audience: superseded runtimes receive nothing further by default. Publishing a **backport** update that targets a superseded runtime remains technically possible and is preserved as a deliberate incident tool under §4's human gates — never a routine obligation; the store upgrade is the standard path for old installs. The required vehicle is a manually dispatched, `production`-environment-approved workflow of the same shape as the fallback publish; until such a workflow exists, no conforming backport path exists and the tool is unavailable rather than exercised outside the gates.
...
4. **Fallback proven (per §5):** the rollback workflow's required credentials are present (its preflight passes), a rehearsal publish to the fallback branch has succeeded, the published bundle is verified to carry Config F at the production runtime version, and the incident action is recorded in the runbook as the server-side channel remap (`eas channel:edit production --branch fallback`) — which the rehearsal deliberately does **not** perform.
...
- The fallback retreat (§5) is scoped to the navigation redesign: remapping the `production` channel to the `fallback` branch serves the Config F bundle to production installs in minutes, provided the §5 rehearsal evidence exists and someone deliberately performs the remap.
...
- Old-version installs go stale rather than broken. This is accepted: the store upgrade path is the standard recovery, no OTA machinery attempts to bridge runtime versions, and a gated backport publish to a superseded runtime exists as an incident option — never a routine support obligation.
...
- **A "semi-hard" fallback gate** (credentials provisioned and preflight green, rehearsal deferred). Considered viable but rejected: the rehearsal is the only step that proves the chain end-to-end, costs ~half an hour, and reaches no production-channel install.
