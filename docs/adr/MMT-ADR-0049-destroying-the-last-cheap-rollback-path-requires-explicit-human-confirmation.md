...
- `.claude/memory/feedback_s6_deferred_irreversible.md` — the source evidence this decision was reconstructed from, recording the ruling of 2026-06-14. It supplies clause 1 ("no agent may start it autonomously"), the plain-terms wording clause 2 requires, clause 3's explicit statement that green gates do not substitute for the confirmation, clause 4's two-decisions-with-a-validation-window instruction, and clause 6's note that the identity data cutover is not flag-reversible at any stage.
- `.claude/memory/feedback_just_do_it.md` — the general no-ask posture that clause 1 carves its narrow exception to, and the reason the exception must be scoped by a property of the change rather than by importance. The source evidence above names this as the one documented exception to it.
- `docs/adr/MMT-ADR-0041-confident-inference-and-reversible-defaults.md` — the adjacent posture on acting without asking. Reversibility is the premise that makes that posture sound, which is precisely the premise this decision identifies as absent at the final stage.
- `docs/plans/v2-plan/2026-06-10-s6-cutover-deletions.md` — the named instance, carrying `status: deferred`, from which this rule was generalised. It is an example of the rule rather than its authority: clause 5 keys on the loss of reversibility, not on any particular migration or stage code.
- `AGENTS.md` § Schema And Deploy Safety — the governed canon for the adjacent case clause 6 sets aside: a migration dropping columns, tables, or types must carry a `## Rollback` section stating whether rollback is possible, what data is lost, and the recovery procedure, and must say so explicitly when rollback is impossible. Enforced by `scripts/check-migration-rollback.sh`.
