...
6. **`/now`'s read-scope covers `person` and `supportership` reads.** The service also serves `person`-scoped and `supporter-hub` requests, reading `person` + `supportership` alongside the derive-on-read operational tables above. This is within the service's intended scope, not a layering violation: the identity tables are the system of record for who a moment concerns and who may see it, and the feed derives from systems of record by design. (An earlier build-plan restriction against these reads predated the identity data model being live and no longer applies.)
