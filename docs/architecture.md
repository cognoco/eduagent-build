...
- Every notice, note, session-analysis Learning Profile field, memory fact/backfill/dedup record, and Needs-Deepening text crosses one shared async multilingual clinical-persistence gate. Unicode-normalized deterministic rules cover every supported Conversation Language and cross-language phrase. Ambiguous text consults an independent judge regardless of producer provenance — user-authored text refers exactly as LLM-authored text does — and proceeds only on a strict educational-reference allowance; unavailable, malformed, `unclear`, missing-producer, and migration/backfill ambiguity fails closed.
