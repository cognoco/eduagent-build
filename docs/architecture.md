...
- The centralized state machine owns offer, local-04:00 learning-day defer in the learner's IANA zone, a UTC-04:00 fallback when that zone is invalid or unavailable, a re-check capped at three learner responses, independently judged `locked_in`/`not_yet`/explicit-dismissal completion, and deterministic `not_yet` at the cap when evaluation would otherwise continue or is unavailable or malformed. It also owns 21-day fade and actor=subject visibility. Clients never create optimistic notice state.
