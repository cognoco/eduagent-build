...
**`apps/api/drizzle/`** — committed migration SQL (90 migrations 0000–0089 as of 2026-05-23) generated by `drizzle-kit` from schema in `packages/database/`. The `meta/_journal.json` tracks the canonical migration order; per-migration `meta/NNNN_snapshot.json` files are Drizzle's diffing baselines. Destructive migrations should ship with a sibling `NNNN_*.rollback.md` per the Schema And Deploy Safety rule in `CLAUDE.md`. Deployment artifacts, not library code.
...
- **Production/Staging**: `drizzle-kit generate` → committed migration SQL in `apps/api/drizzle/` (90 migrations 0000–0089 as of 2026-05-23) → `drizzle-kit migrate` applied in CI/CD pipeline
...
```
# Authoritative source: `apps/api/src/index.ts` route mountings (44 route files as of 2026-05-23).
