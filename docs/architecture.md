...
**`apps/api/drizzle/`** — committed migration SQL (45 migrations 0000–0044 as of 2026-05-01) generated by `drizzle-kit` from schema in `packages/database/`. The `meta/_journal.json` tracks the canonical migration order; per-migration `meta/NNNN_snapshot.json` files are Drizzle's diffing baselines. Destructive migrations should ship with a sibling `NNNN_*.rollback.md` per the Schema And Deploy Safety rule in `CLAUDE.md`. Deployment artifacts, not library code.
