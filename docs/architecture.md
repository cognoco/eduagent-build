...
| **Retention & spaced repetition** | SM-2 as **library/module** (~50 lines pure math). Takes `{ previous_interval, previous_ease_factor, quality_score }` → returns `{ next_interval, next_ease_factor, next_review_date }`. Writes to `retention_cards` at topic grain; `assessments` and progress stay topic-keyed. Consumers are all readers: coaching card ("which topics due/overdue"), notification scheduler ("when is next review"), parent dashboard ("how many topics fading"). Library is the writer, everything else is a reader. Clean interface enables future service extraction. Called through event-driven lifecycle. **Concept-grain mastery capture (MMT-ADR-0017):** An additive `concepts` + `concept_mastery` layer is built to capture every Challenge Round per-concept verdict (solid and weak alike), with the derived note star and tutor-addition signal reading it; capture is gated behind `CONCEPT_CAPTURE_ENABLED` (now enabled, WI-781 — the MMT-ADR-0012 baseline-reset tables and the identity-cutover `profiles`→`person` FK repoint have landed on staging/prod; live production traffic through the gated call site additionally requires `CHALLENGE_ROUND_RUNTIME_ENABLED`). The scheduled spine (`retention_cards`, `assessments`, progress) is not re-keyed. **EVALUATE scoring (Epic 3 extension):** EVALUATE results feed into SM-2 as a new input source, but the math is unchanged. Modified scoring floor: EVALUATE failure = quality 2-3 (not 0-1) — missing a subtle flaw does not equal not knowing the concept. Prevents score tanking on topics the student actually knows. `evaluateDifficultyRung` (integer 1-4) stored alongside SM-2 state on the retention card; persists across sessions, default null (= never evaluated). **Two mastery axes (MMT-ADR-0031):** Challenge Round verification ("proved explanation now", historical `assessments.mastery_challenge_verified_at`, never cleared by decay) and SM-2 retention ("recall held over time", owns decay/due timing) are complementary axes, never aliases — verification may seed the first scheduled review but never exits a topic from the repetition loop, and proof surfaces must present both axes, never one unqualified badge. **Verified learning artifacts (MMT-ADR-0032):** anything rendered as proof of verified learning must carry first-class artifact source (Challenge solid-quote / Challenge-drafted note / learner-authored note / freeform keep) and verification state; only explicitly verified artifacts feed parent/progress proof, provenance references durable evidence metadata (never raw transcripts) and degrades honestly when evidence expires. | Backend |
...
- Every notice, note, session-analysis Learning Profile field, memory fact/backfill/dedup record, and Needs-Deepening text crosses one shared async multilingual clinical-persistence gate. Unicode-normalized deterministic rules cover every supported Conversation Language and cross-language phrase. Ambiguous text consults an independent judge on a three-way provenance split: user-authored text refers with judge independence `not-applicable` and no producer vendor, because such text has no model producer to exclude; LLM-authored text refers only with a known, non-blank producer vendor; migration/backfill text never refers. Referred text proceeds only on a strict educational-reference allowance; unavailable, malformed, `unclear`, a missing or blank LLM producer vendor, and migration/backfill ambiguity fails closed.
- The centralized state machine owns offer, local-04:00 learning-day defer in the learner's IANA zone, a UTC-04:00 fallback when that zone is invalid or unavailable, a re-check capped at three learner responses, independently judged `locked_in`/`not_yet`/explicit-dismissal completion, 21-day fade, and actor=subject visibility. Re-check attempt lifecycle is separate from notice status: a valid `continue` transitions nothing at any response including the third, and reaching the cap after a valid `continue` ends the attempt and detaches its bookkeeping while leaving the notice unresolved and re-offerable; only an unresolved evaluation — for example an unavailable or malformed judgment — terminalizes, and only at the third response, as `not_yet`. Clients never create optimistic notice state.
...
- **The dangerous-procedure reply gate is age-scoped, minors ⊇ adults.** A deterministic, intent-shaped floor (item term **and** actionable how-to structure — not a denylist) neutralizes operational how-to in the tutor reply. **Minors** get the full gate (all controlled/dangerous items); **adults** get a **narrow catastrophic subset only** — CBRN weapons + explosive-device construction — so adult latitude for general chemistry, pharmacology, energetics, weapons history, and non-catastrophic dual-use how-to stays open. This is the product's **first adult-facing content constraint**; its boundary is fixed and widening it is an operator ruling, not an engineering call (`MMT-ADR-0030`; `services/dangerous-procedure-gate.ts`; `docs/registers/safety-guards/master.md` #1).
- **Minor output-gating mode is violation-only + fail-open-with-alarm.** For under-18 traffic the suitability judge can act as a synchronous **enforcing output gate** (backstopping the router content-category refusals), reusing the WI-1154 block-and-replace seam + `sourceReplacement` rail: it blocks-and-replaces **only** on `overall === 'violation'` (never `concern`), never on the `over_blocking` / `topic_drift` categories (over-blocking is a hard failure equal to under-blocking), and a judge that cannot render a verdict **fails OPEN with a structured operator alarm** (fail-closed is reserved for a concrete violation). The mechanism is behind `JUDGE_ENFORCEMENT_ENABLED`, **default off / inert**, and must not flip on until a calibration-gated threshold is set from real minor-traffic verdict data (`MMT-ADR-0016` §3 phase-5 amendment).
