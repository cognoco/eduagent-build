...
- [ ] T4: Verify the cleanup and regression surface — done when: the exact six-specifier structural check passes, GC1 detector tests and CI-shape GC1 pass, Sentry/Inngest/PII companion tests pass, API typecheck/lint pass, the full API unit suite passes, and the relevant real-database integration manifests pass.
