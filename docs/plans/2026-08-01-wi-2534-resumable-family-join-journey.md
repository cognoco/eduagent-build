...
- A pending invite is the pre-journey learner state. Once the authenticated recipient accepts and binds it, journey states are `awaiting_guardian`, `ready_to_join`, `joined`, `declined`, and `withdrawn`. Token expiry is derived live: it retires and removes only an unfinished journey so the learner's active slot can recover, while a committed `joined` row remains resumable after expiry. Declined and withdrawn slots may be safely reissued after their terminal journey residue is removed under the invite-row lock. An expired pending or bound slot is reissuable only after the journey-expiry transition has already retired and removed its unfinished journey; a still-active or joined journey fails closed.
