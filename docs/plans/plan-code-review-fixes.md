...
> **Estimated scope:** ~35 items. Streams 2A-2C and 2E are fully parallel. Stream 2D depends on Phase 1A.

### Stream 2A — API Bug Fixes (PARALLEL)
...
| 2A.3 | 4 | LOW | `apps/api/src/services/coaching-cards.ts:231` | Fix challenge card fallback: uses `profileId` as `topicId`. Replace with correct topic lookup. | |
| 2A.4 | 3 | LOW | `apps/api/src/services/retention-data.ts:462-527` | Add `checkNeedsDeepeningCapacity` call in `startRelearn`. Per FR61-63, max 10 active needs-deepening topics per subject. | |
...
| 2A.7 | 5 | MED | `apps/api/src/services/trial.ts:204-215` | Extend `getTrialWarningMessage` to include 7-day and 2-day milestones (currently only 3, 1, 0). | |
...
### Stream 2B — N+1 & Performance Fixes (PARALLEL)

All independent — different service files, no shared callers.

| ID | Epic | Severity | File | Fix | Size | Verified By |
|----|------|----------|------|-----|------|-------------|
| 2B.1 | 1 | MED | `services/curriculum.ts:1065-1077` | Batch UPDATE with CASE expression instead of per-topic loop in `adaptCurriculumFromPerformance`. | S | |
| 2B.2 | 2 | MED | `services/coaching-cards.ts:306-333` | Replace nested loop in `findContinueBookCard()` with single JOIN query (books -> topics -> sessions). | M | |
...
| 2B.4 | 3 | LOW | `services/retention-data.ts:529-552` | Move `subjectId` filter from JS to SQL WHERE in `getSubjectNeedsDeepening`. | S | |
| 2B.5 | 4 | MED | `services/progress.ts:456-498` | Batch `getContinueSuggestion`: query all subjects/curricula in one pass instead of per-subject loop. | M | |
| 2B.6 | 4 | MED | `services/dashboard.ts:378-394` | Fix `getChildDetail` to query single child profile directly instead of fetching ALL children and filtering. | M | |
| 2B.7 | 4 | MED | `services/interleaved.ts:101-122` | Replace N individual `findFirst` in `selectInterleavedTopics` with single `inArray` batch query. | M | |
| 2B.8 | 5 | MED | `services/billing.ts:756-795` | Replace N individual SELECT+UPDATE in `resetExpiredQuotaCycles` with single batch UPDATE...FROM join. | M | |
| 2B.9 | 2 | LOW | `services/session.ts:501-561` | Add session-scoped cache for static lookups (profile/subject/curriculum) in `prepareExchangeContext`. | M | |

### Stream 2C — New Inngest Crons: Review Reminders & Daily Push (INTERNAL ORDERING)
...
|----|------|----------|------|-------------|------------|-------------|
| 2C.1 | 2+4 | HIGH | **FR42/FR91: `review-due-scan` cron function.** New file. Cron scans `retentionCards` for `nextReviewAt <= now`, groups by profileId, emits `app/retention.review-due` events. | **First** | ~120 | |
| 2C.2 | 4 | HIGH | **FR95: `daily-reminder-scan` cron function.** New file. Cron scans profiles with active streaks, emits daily reminder events. Calls existing `formatDailyReminderBody()`. | **Second** (or parallel with 2C.1) | ~150 | |
| 2C.3 | 3 | LOW | **FR49: Verify SM-2 intervals.** No code change — verify that SM-2 naturally produces 14-day and 42-day intervals. If not, adjust `processRecallResult` parameters. | After 2C.1 | ~0 | |
| 2C.4 | — | — | **Register new functions.** Add `reviewDueScan` and `dailyReminderScan` to `apps/api/src/inngest/index.ts` exports. | After 2C.1+2C.2 | ~5 | |
...
### Stream 2D — Epic 12 API Service Cleanup (DEPENDS ON 1A)
...
|----|----------|------|-----|-------------|
| 2D.1 | HIGH | `apps/api/src/routes/consent-web.ts:279` | Review deep link `mentomate://home` post-persona-removal. ⚠️ Discovery cited wrong URL (`mentomate://parent/dashboard` — does not exist). Actual URL is `mentomate://home`. Verify this is correct for the post-persona routing; also check `mentomate://onboarding` at line 282. | |
| 2D.2 | MED | `apps/api/src/services/profile.ts:179,188,200` | Remove persona age-gate check, remove `inferLegacyPersonaType` from input, compute from birthYear only. | |
| 2D.3 | MED | `apps/api/src/services/session.ts:775` | Simplify to `birthYearFromDateLike()` only — remove personaType-based computation. | |
| 2D.4 | MED | `apps/api/src/services/test-seed.ts:335,340,352,696,765,1154,1213,1297` | Remove 8 `personaType` references. Let personaType be auto-computed from birthYear. | |
| 2D.5 | MED | `apps/api/src/services/profile.test.ts` | Update 22 test references to use birthYear/isOwner patterns instead of personaType. | |
| 2D.6 | MED | `apps/api/src/services/export.ts` + `export.test.ts` | **MISSING FROM ORIGINAL PLAN.** Phase 1 note explicitly says "Expected TS errors in profile.ts, **export.ts**, test-seed.ts until 2D is done" but export.ts was omitted. export.test.ts:27 still has `personaType: 'LEARNER' as const` in mock data. Clean up persona references. GDPR compliance risk if persona data leaks into exports. | |
| 2D.7 | LOW | `apps/api/src/services/billing.test.ts:153` | **MISSING FROM ORIGINAL PLAN.** Mock profile builder includes `personaType: 'LEARNER'`. Remove after migration drops the column. | |

### Stream 2E — Architecture Violation Cleanup (PARALLEL)

Low-priority consistency fixes. All independent.

| ID | Epic | Severity | File | Fix | Verified By |
|----|------|----------|------|-----|-------------|
| 2E.1 | 1 | LOW | `services/interview.ts:1` | Migrate raw ORM writes to scoped repository pattern. | |
| 2E.2 | 2 | LOW | `services/session.ts:139-176, 206-227` | Replace manual `profileId` filtering in `buildBookLearningHistoryContext` + `buildHomeworkLibraryContext` with scoped repo. | |
| 2E.3 | 2 | LOW | `services/session.ts:307-316, 418-424` | Replace direct `db.insert(sessionEvents)` with scoped repo in `insertSessionEvent`. | |
| 2E.4 | 2 | LOW | `services/session.ts:1289-1294` | Document `closeStaleSessions` as intentional cross-profile batch exception. | |
| 2E.5 | 3 | LOW | `services/assessments.ts:309-328` | Replace raw `db.insert()` in `createAssessment` with scoped repo. | |
...
| 2E.7 | 9 | LOW | `routes/billing.ts:131-206` | Guard or remove dormant Stripe checkout endpoints (marked "for mobile" but fully implemented). | |
| 2E.8 | 9 | LOW | `mobile/src/lib/revenuecat.ts:34-46` | Add dev-mode warning when RevenueCat API key not configured (currently silent return). | |
| 2E.9 | — | MED | Multiple service files | **MISSING FROM ORIGINAL PLAN.** 24 direct `console.warn`/`console.error` calls across 12 service files despite `services/logger.ts` existing. Per global instructions: "Silent recovery without escalation is banned" and "console.warn alone is never sufficient." Replace with structured logger. Files: `billing.ts` (3), `llm/router.ts` (5), `notifications.ts` (3), `account.ts` (1), `consent.ts` (1), `memory.ts` (1), `profile.ts` (1), `retention-data.ts` (1), `session.ts` (1), `subject.ts` (1), `vocabulary-extract.ts` (1), `llm/providers/openai.ts` (1). | |
...
> **Estimated scope:** ~47 items (includes 2 reclassified from 2A). All streams are parallel except 3D (depends on 1A + 2D).
>
...
### Stream 3A — Critical UX Dead-Ends (PARALLEL)
...
### Stream 3B — High UX Dead-Ends (PARALLEL)
...
### Stream 3C — Medium UX Dead-Ends (PARALLEL)
...
| 3C.7 | Library | `library.tsx:127-137` | Distinguish "no topics" from "failed to load topics" in empty state message. |
...
### Stream 3D — Epic 12 Mobile Persona Cleanup (DEPENDS ON 1A + 2D)
...
### Stream 3E — Missing Mobile Screens & Features (PARALLEL)
...
### Stream 3F — Accessibility, Voice & Polish (PARALLEL)
...
| 3F.8 | 7 | MED | `library.tsx:235` | Add `generateBookTopics.isPending` to useEffect dependency array. |
...
> **Estimated scope:** ~50 items. All streams fully parallel.

### Stream 4A — Epic 6 Service & Hook Tests (8 HIGH gaps)
...
| 4A.4 | `services/vocabulary-extract.ts` | Unit tests for `extractVocabularyFromTranscript` including malformed LLM JSON. |
...
### Stream 4B — Epic 7-8 Tests (PARALLEL)
...
### Stream 4C — Epic 9 Billing & Webhook Tests (PARALLEL)
...
### Stream 4D — Epic 1-5 Missing Tests (PARALLEL)
...
| 4D.5 | 2 | `services/recall-bridge.ts` | Integration test for full `/sessions/:sessionId/recall-bridge` flow with homework guard. |
| 4D.6 | 3 | `inngest/functions/session-completed.test.ts` | Migrate from manual step extraction to `InngestTestEngine`. Test step-level retries. |
...
### Stream 4E — Epic 10-13 Tests (PARALLEL)
...
| 4E.4 | 12 | `routes/home-cards.test.ts:81,126` | Unskip tests when middleware mock infrastructure is ready. |
...
### Stream 4F — Stale Test & Type Safety Fixes (PARALLEL)

| ID | Epic | File | Fix |
|----|------|------|-----|
| 4F.1 | 2 | `services/session.test.ts` | Replace heavy mocking (8+ modules). Mock for `buildPriorLearningContext` always returns empty — FR40 never exercised. |
...
| 4F.5 | 6 | `components/language/FluencyDrill.test.tsx` | Test timeout callback invocation (currently only tests render + manual submit). |
...
| 3 | 3A + 3B + 3C + 3E + 3F all parallel | 3D waits for 1A + 2D. |
...
### Item Counts by Phase (corrected 2026-04-05)

| Phase | Valid Items | Invalid/Reclassified | Critical | High | Med | Low |
|-------|-------------|---------------------|----------|------|-----|-----|
| Phase 1 | 18 | 0 | 4 | 4 | 7 | 3 |
| Phase 2 | 30 | 8 (6 INVALID + 2 reclassified to P3) | 0 | 3 | 13 | 13 |
| Phase 3 | 47 | 0 (+ 2 received from P2) | 10 | 15 | 19 | 3 |
| Phase 4 | 50+ | 0 | 0 | 8 | 30+ | 12+ |
| **Total** | **145+** | **8** | **14** | **30** | **69+** | **31+** |

*Note: Excludes 5 FIXED/INVALID/DELIBERATE items from discovery doc + 15 truncated Epic 13 items.*
*Phase 2 originally claimed "0 Critical, 5 High, 18 Med, 12 Low = 35 items" — this was wrong. After adversarial review: 6 items INVALID, 2 reclassified to Phase 3, 3 new items added (2D.6, 2D.7, 2E.9).*
