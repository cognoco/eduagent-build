...
| # | PR | Scope | Risk |
|---|----|-------|------|
| 1 | `tier-server-rework` | `TIER_CONFIGS` reshape + `profile_quota_usage` table + `top_up_credits.profile_id` migration + per-profile metering branch (with lazy-provision + owner-top-up fallthrough) + tier-change handlers + reset-cron extension + 402 payload extension + `subscription.ts` comment correction | High — schema + hot path |
| 2 | `client-tier-paywall-removal` | Drop add-child paywall, extend `NavigationSubscriptionContext` with `billingGood`, introduce `isFamilyHubEligible`, swap 5 gating sites | Medium — UX visibility shift + new client billing-state input |
...
async function attemptDecrementInTx(
  ...
  if (!snapshot) {
    ...
    const tier = await getTierForSubscription(tx, subscriptionId);
...
```ts
const tier = await getTierForSubscription(txDb, subscription.id);
if (TIER_CONFIGS[tier].quotaModel === 'per-profile') {
  const role = isOwnerInsert ? 'owner' : 'child';
  // Pass tier through to avoid a second getTierForSubscription query (LOW-1).
...
export async function provisionProfileQuotaUsage(
  ...
  const tier = opts.tier ?? await getTierForSubscription(tx, subscriptionId);
...
Add two fields to the `details` object:

```ts
details: {
  ...existing,
  quotaModel: TIER_CONFIGS[tier].quotaModel,     // 'per-profile' | 'shared-pool'
  profileRole: isOwner ? 'owner' : 'child',      // for child → "ask your parent" CTA
}
```

The client (`QuotaExceededCard.tsx`) already branches on `isOwner` (lines 49+) so no client change is needed here for the existing behavior; the `quotaModel` field future-proofs the upgrade-CTA copy split.
...
### 1F. Tier-change handling (per HIGH-3)
...
The current tier-change handler does not touch `quota_pools` or any per-profile state; after this PR it must reconcile `profile_quota_usage` rows to the new tier's caps. Transitions per spec § Tier-change handling (lines 282-286):
...
This function is called from the tier-change service in the same transaction that updates `subscriptions.tier`. The PR description must name the actual call sites being patched.
...
- 402 payload includes `quotaModel` + `profileRole`.
...
**Goal:** Remove the tier paywall on add-child; introduce `isFamilyHubEligible`; swap 5 display-gating sites. Keep the two pool-data-fetch sites tier-gated. **Add `billingGood` to `NavigationSubscriptionContext`** so the eligibility predicate can correctly gate `past_due`/`cancelled`/`expired` paid users (per CRITICAL-4).

**Files (display-gates — swap):**
- `apps/mobile/src/lib/navigation-contract.ts` — extend `NavigationSubscriptionContext` with `billingGood`; replace `familyPlanOwner` with `isFamilyHubEligible`
- `apps/mobile/src/lib/navigation-contract.ts:245-251` — predicate update site
- `apps/mobile/src/hooks/<the hook that builds the navigation context>` — map server billing status → `billingGood`. Grep `NavigationSubscriptionContext` / `subscription.status === 'ready'` to find the construction site before drafting.
...
**Schema change — `NavigationSubscriptionContext` (per CRITICAL-4):**
...
interface NavigationSubscriptionContext {
  ...
  tier: 'free' | 'plus' | 'family' | 'pro';
  /**
   * True when server-side subscription.status ∈ {'trial','active'}; false when
   * ∈ {'past_due','cancelled','expired'}; null while loading. Distinct from the
   * data-load discriminator `status` above. Family Hub display gating uses
   * `billingGood` for paid tiers; Free is unconditionally billing-good.
   */
  billingGood: boolean | null;
  // ...
}
```

The mapping lives in the hook that builds the context, not in `navigation-contract.ts` (which stays pure). The construction site sets `billingGood = subscription.status === 'trial' || subscription.status === 'active' ? true : subscription.status === undefined ? null : false`.
...
export function isFamilyHubEligible(context: ProfileContext): boolean {
  ...
  // Free is unconditionally billing-good (no billing to gate).
  if (context.subscription.tier === 'free') return true;
  // Paid tiers: gate on billingGood, not the data-load `status`.
  return context.subscription.billingGood === true;
}
```
...
Then update every downstream usage of `familyPlanOwner` in the file to `familyHubEligible`. The Recaps tab gate (V1) now fires on Free/Plus owners with a linked child whose billing is current.
...
test('[V0 hard constraint] V0 + Family + linked child still renders 5 tabs (unchanged)', () => {
  const context = makeContext({
    ...
    linkedChildren: 1,
    subscription: { status: 'ready', tier: 'family', billingGood: true },
  });
...
test('[new state] V0 + Free + linked child renders 5 tabs (rework opens guardian shape to Free)', () => {
  const context = makeContext({
    ...
    activeProfile: makeAdultOwner(),
    linkedChildren: 1,
    subscription: { status: 'ready', tier: 'free', billingGood: true },
  });
  const result = resolveNavigationContract(context);
  expect(result.visibleTabs).toEqual(LEGACY_GUARDIAN_TABS);
});
...
**Tests required:**
- `isFamilyHubEligible`: adult Free owner + linked child → true; adult Free owner + 0 children → false; non-owner → false; `past_due` Plus (`billingGood: false`) → false; Plus + loading (`billingGood: null`) → false.
- Recaps tab visibility (V1 nav): Free adult owner with linked child + `billingGood: true` sees Recaps.
...
**Goal:** Add parent in-app notification when child hits cap. Reuse `QuotaExceededCard` (agent confirmed isOwner-aware copy already exists at lines 49+). **Event dispatch + handler land together in this PR** — no orphan events from PR 1. **New `child_cap_notifications` table** + new `/notifications` route (does not exist today — confirmed by glob).
...
- `apps/api/src/services/billing/metering.ts` — add `safeSend` of `app/billing.profile_quota.exhausted` inside `decrementProfileQuota` when child profile returns `daily_exceeded` or `none`
...
- `apps/mobile/src/i18n/locales/{en,de,es,ja,nb,pl,pt}.json` — 7 locales × new keys (`quota.parent.childCapHit.title`, `.message`, `.dismiss`)
...
if ((result.source === 'daily_exceeded' || result.source === 'none') && snapshot.role === 'child') {
  ...
  await safeSend(
    () => inngest.send({
      ...
      data: {
        subscriptionId, profileId, kind: result.source,
...
The new Inngest function reads the owner profile (via `account_id` join through `subscriptions` → `profiles WHERE is_owner = true`), then inserts the notification row with `onConflictDoNothing` on the dedup index.
...
- `quota.parent.childCapHit.message` — "Their quota resets at midnight. Upgrade for more capacity."
...
1. New Free account → add child → exhaust child's 10/day → confirm "ask your parent" copy, parent sees in-app notification on next session
...
6. Plus owner enters `past_due` → confirm Recaps tab disappears (`billingGood: false`); restored when payment recovers
...
| `apps/api/src/services/billing/subscription-change.ts` | Tier-change reconciliation: Free↔Plus in-place, Plus→Family delete, Family→Plus owner+first-child provision | 1 |
...
| `apps/mobile/src/lib/navigation-contract.ts` | Extend `NavigationSubscriptionContext` with `billingGood`; `familyPlanOwner` → `isFamilyHubEligible` | 2 |
| `apps/mobile/src/hooks/<context builder>` | Map server billing status → `billingGood` | 2 |
...
| `apps/mobile/src/i18n/locales/*.json` (7 files) | Delete invalidated keys, add new ones (`quota.parent.childCapHit.*`, `onboarding.discovery.addChildHint`, `tierFeatures.*`) | 3, 4 |
