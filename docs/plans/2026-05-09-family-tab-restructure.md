...
| **D-FT-3** | Per-child celebrations follow-up (from D1 of more-tab spec) appears on child-detail under the accommodation section, only when the child's accommodation is `short-burst` or `predictable`. Default `big_only`. **Backend prerequisite (verified 2026-05-09 against `apps/mobile/src/hooks/use-settings.ts:87,231` and `apps/api/src/routes/settings.ts`):** `useCelebrationLevel` / `useUpdateCelebrationLevel` and the `/settings/celebration-level` route are account-wide today (no `childProfileId`). To support per-child storage we need: (a) a `celebrationLevel` column on `learning_profiles` (or a per-child override table — implementer's call, but column is simpler), (b) extend the GET/PUT route to accept an optional `childProfileId` query/body parameter, scoped via `assertParentAccess`, (c) a new mutation hook `useUpdateChildCelebrationLevel({ childProfileId, celebrationLevel })`. This work is captured as Step 0a below. |
...
| **D-FT-5** | "Add a child" affordance lives **only in Family tab** (not duplicated in More). **Verified 2026-05-09:** the more-tab spec at `docs/plans/app evolution plan/done/2026-05-09-more-tab-restructure.md` already has D5 superseded inline (`~~**D5**~~ ... Superseded by 2026-05-09-family-tab-restructure.md D-FT-5`). The spec edit has already happened; the remaining work is purely the code change in this PR — remove the "Add a child" row from `apps/mobile/src/app/(app)/more/index.tsx:356-375` and the `child_progress_navigated` track call at `:141-147` (or repoint the source label to `family_tab` if the analytics value still matters). |
| **D-FT-5b** | Other child-related affordances in More tab. Verified 2026-05-09 at `apps/mobile/src/app/(app)/more/index.tsx:201-241`: when the active profile owns linked children, More renders deep-link Pressables to `/(app)/child/<id>` (`accommodation-mode-child-link` for 1 child) and to `FAMILY_HOME_PATH` (`accommodation-mode-family-link` for 2+ children). These were originally added so a parent could reach per-child accommodation without impersonation. Now that child-detail surfaces per-child accommodation directly (D-FT-2) and Family is the single command center (D-FT-1), keep these deep-links as **back-pointers from the More accommodation section** — they help a parent who arrives in More to edit their own accommodation realize "and these are my child's prefs over here." Do not remove. The "Add a child" row is the only More→child surface this PR removes. |
...
**Current state (verified 2026-05-09, full file read):** Imports `useChildLearnerProfile`, `useUpdateAccommodationMode`, `ACCOMMODATION_OPTIONS`, `ReportsListCard`, growth chart, recent sessions, retention signal, subject cards, mentor memory link. **The per-child accommodation picker is already rendered** at lines 781-822 (`ACCOMMODATION_OPTIONS.map((opt) => …)`), and `handleAccommodationChange` at lines 209-226 already calls `updateAccommodation.mutate({ childProfileId: profileId, accommodationMode: mode })`. So D-FT-2 ("edit per-child accommodation without impersonation") is functionally already shipped today. The remaining work is **reordering** (the picker currently sits below subjects, growth chart, mentor memory) and **adding three new cards above it** (live weekly headline, latest monthly highlights, currently-working-on).
...
- Extend props to accept `weeklyHeadline?: { label: string; value: number; comparison: string }` and `currentlyWorkingOn?: string[]` (card cap: render first **3 entries**, then "and N more →" linking to child detail; child-detail "Currently working on" section caps at 10 entries from the helper, no further deep-link).
...
**No spec edit needed.** Verified 2026-05-09: the more-tab spec at `docs/plans/app evolution plan/done/2026-05-09-more-tab-restructure.md` already has D5 superseded inline (line 21 starts with `~~**D5**~~ ... Superseded by 2026-05-09-family-tab-restructure.md D-FT-5`). The narrative state to remember:

- More tab landing keeps: Learning Mode (with celebrations follow-up under short-burst/predictable, for the **active profile only** — i.e., the parent's own learning) + Mentor Memory link (conditional). No "Add a child" row.
- The accommodation section in More keeps editing the active profile's own accommodation only. Per-child accommodation lives on Family → child detail (D-FT-2).
- The deep-link Pressables at `more/index.tsx:201-241` ("Edit \<Name\>'s preferences" / "Open family preferences") are kept as back-pointers from the More accommodation section to the Family tab. See D-FT-5b.

Code edits land in this PR (see step 7).
...
| `learning_profiles.struggles` returns 4+ entries | Heavy practice load | Family card: show first **3** + "and N more →" linking to child detail. Child detail: show all (helper caps at 10). Progress: show first 3 + "and N more" non-link suffix (per sibling spec). | Cap rendering to keep card compact. Full list visible in child detail. |
...
| Per-child celebrations follow-up tries to write before accommodation save resolves | Race on consecutive taps | Both mutations fire independently in parallel | Mutations are not queued client-side (TanStack Query mutations fire concurrently by default). Last-write-wins on the server is acceptable for a setting toggle — no client-side queuing needed. |
...
| Copy sweep misses a locale | Translator left "struggle" in nb.json | User in Norwegian sees old word | Sweep checklist enumerates all 7 locale files. **Forward-only CI guard** (added in the copy-sweep PR per its own spec): a `pnpm` script that fails the build if any file under `apps/mobile/src/i18n/locales/*.json` matches `/\b(struggle\|struggling\|declining\|trouble\|weak)\b/i`. Sweep-on-fix per `feedback_sweep_for_same_bug`. |
...
0. **Schema + data source (shared with Progress spec).** Extend `apps/api/src/services/learner-profile.ts` to expose `getCurrentlyWorkingOn(profileId): Promise<string[]>`. **Verified 2026-05-09:** `learning_profiles.struggles` shape is `{ subject (nullable), topic, lastSeen, attempts, confidence, source }` (`packages/schemas/src/learning-profiles.ts:92-99`) — there is **no `resolvedAt` field**. Helper logic: filter by `lastSeen >= now - 30d` (tighter than the existing 90d archival at `learner-profile.ts:337-344`), optionally filter low-confidence single-shot entries (`confidence === 'low' && attempts < 2`), strip negative-framing prefixes from `topic` field, return `string[]` of bare topic labels. **`subject` handling:** when `subject` is null, render the topic alone; when non-null, render the topic alone too (subject is supplementary context for parents in reports — for the Family-card / child-detail / Progress UI surface, the bare topic is the right granularity). Cap server-side at 10 entries. UI caps: Family card 3 entries + "and N more →"; child-detail full list shows all (up to 10); Progress 3 entries + "and N more" non-link suffix per its sibling spec. Document the chosen window + confidence rule in the PR. **Ownership contract:** the helper itself only reads the row for the passed `profileId` and applies no auth check; ownership is enforced upstream by `assertParentAccess(db, parentProfileId, childProfileId)` on the parent-side dashboard route, or by the active-profile match on the self-side inventory route. Add `currentlyWorkingOn: string[]` to: (a) `dashboardChildSchema` (per-child shape, in `packages/schemas/src/progress.ts`), (b) the inventory response shape consumed by `useChildInventory`, and (c) the demo fixture children in `apps/api/src/routes/dashboard.ts:344-403` so `demoDashboardDataSchema.parse(...)` does not throw after the schema gains a required field. Both Family's dashboard endpoint (`GET /v1/dashboard`, route file `apps/api/src/routes/dashboard.ts:65-79`, response wrapper `dashboardResponseSchema`) and Progress's inventory endpoint read from this helper.

0a. **Backend: per-child celebration level (prerequisite for D-FT-3).** Extend `learning_profiles` with `celebrationLevel` column (enum `'all' | 'big_only' | 'off'`, default `'big_only'`). Migration must add the column and backfill existing rows to the default. Extend `apps/api/src/routes/settings.ts` `/settings/celebration-level` GET/PUT to accept an optional `childProfileId` parameter; when present, scope via `assertParentAccess(db, parentProfileId, childProfileId)` and read/write the child row's `celebrationLevel`; when absent, behave exactly as today (account-wide celebration level for the active profile). Add `useUpdateChildCelebrationLevel({ childProfileId, celebrationLevel })` and `useChildCelebrationLevel(childProfileId)` hooks. **Out of scope for this PR:** unifying account-wide and per-child celebration storage. The active profile's own celebration level continues to live in account-scoped storage; the per-child override is a new column on `learning_profiles` and is read/written separately. (If the implementer finds the existing celebration storage already lives on `learning_profiles` for the active profile, then this is a no-op column rename — verify before adding.)

1. **Backend: extend dashboard endpoint** to include `weeklyHeadline` and `currentlyWorkingOn` per child. **Reuse the `latestSnapshot` already loaded inside `buildChildProgressSummary` at `apps/api/src/services/dashboard.ts:523-550`** — do not call `getLatestSnapshot` a second time; pipe the snapshot into `generateWeeklyReportData(latestSnapshot)` from inside the same function so the per-child compute is one snapshot read, not two. Pull `currentlyWorkingOn` via the helper from step 0. Update `dashboardChildSchema` in `@eduagent/schemas` (and the demo fixture per step 0).
...
3. **Update Family tab landing** (`family.tsx`):
   ...
   - Promote `Add a child` card to always-visible for adult owners. **Surgical change:** keep `FamilySharingToggle` (lines 391-407) inside the `showFamilyManagement = !isDemo && hasChildren` gate — do not drop the whole wrapper. Pull only the `Add a child` Pressable (lines 408-423) out of the wrapper into its own `showAddChild`-gated branch above or below it. Empty state remains the dedicated path for zero-children.
   - Sweep "struggle / declining" copy.
4. **Enhance child-detail** (`child/[profileId]/index.tsx`) — **the per-child accommodation picker already exists at lines 781-822; this step is reordering + new cards, not adding the picker:**
   - Move the accommodation block (lines 721-822 — header + guide toggle + ACCOMMODATION_OPTIONS picker) so it sits below the new live cards but above subjects/growth/recent sessions.
   - Add latest weekly headline card at top (live, sourced from the dashboard child detail response — same snapshot the headline endpoint computes for Family landing).
   - Add latest monthly highlights card (when available; from existing monthly-report data).
   - Add "Currently working on" section (when non-empty; from helper introduced in Step 0).
   - Add per-child celebrations follow-up under accommodation when mode is `short-burst` or `predictable`, using `useUpdateChildCelebrationLevel` from Step 0a (NOT the account-wide `useUpdateCelebrationLevel`).
...
7. **Code edit (not a spec edit) for the More tab.** The more-tab spec at `docs/plans/app evolution plan/done/2026-05-09-more-tab-restructure.md` already has D5 superseded inline; no spec edit needed. In this PR, remove the live "Add a child" row from `apps/mobile/src/app/(app)/more/index.tsx:356-375` and `handleAddChild` at `:99-139` (now unused). Repoint or remove the `child_progress_navigated` track event at `:141-147` — since the deep-link Pressables at `:201-241` still call `handleChildProgressNavigation`, keep the helper but reconsider whether the analytics source label `'more_section'` should change to reflect the surviving call sites (parent reaching child via the More accommodation back-pointer is meaningfully different from "Add a child"). Verify after edit: `more/index.tsx` no longer imports `useFamilySubscription` if `handleAddChild` is the only consumer.
8. **Tests:**
...
- [ ] More-tab live code (`apps/mobile/src/app/(app)/more/index.tsx`) no longer renders "Add a child" — verified by grep for `add-child-link` testID in mobile source. (Sibling spec already supersedes D5 inline at `docs/plans/app evolution plan/done/2026-05-09-more-tab-restructure.md`; no spec edit needed.)
- [ ] Per-child celebration level: migration applied; PUT `/settings/celebration-level?childProfileId=X` updates only the child row; PUT without the param updates only the active profile (no cross-write).
- [ ] Demo dashboard fixture (`apps/api/src/routes/dashboard.ts:344-403`) parses against the updated `dashboardChildSchema` after the new fields are added.
- [ ] `dashboardChildSchema` carries `weeklyHeadline` + `currentlyWorkingOn`. The dashboard endpoint uses the snapshot already loaded in `buildChildProgressSummary` to compute the headline (no second `getLatestSnapshot` call per child).
