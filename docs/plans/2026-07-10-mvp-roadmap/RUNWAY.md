> **Roadmap-of-record (ratified 2026-07-10).** Graduated copy per PLAN.md Phase 5; working files live in `_wip/mvp-roadmap/` (source file: RUNWAY-DRAFT.md).

# MVP Runway — ratified sequencing (roadmap-of-record)
...
**1C. Family/supporter chain** (beta-operator first-session path): WI-1127 (coldstart route) → WI-1135 (cold-start surfaces) → WI-1137 (linking-ceremony screens; person-picker must cover the teen path) → WI-1753 (cross-account existing-teen family join) → WI-2533 (verified guardian attachment for credentialed learners).

> **The 13–16 authority doctrine is provisionally approved and operative; implementation remains gated.** WI-1753 supplies the cross-account join mechanism. Under the MMT-ADR-0010 provisional-approval rider, WI-2037 defines a distinct authenticated-adult ceremony for a credentialed learner below the residence-policy consent age: verify the adult Person and legal relationship/authority at the required assurance/VPC level, then atomically create or confirm global Guardianship and write fresh destination-Organization/purpose grants. Invitation, ordinary email response, admin/Payer status, and learner nomination create no Guardianship; Guardianship creates no Supportership. Final ratification is tracked in OPQ-160 without blocking delivery. WI-2533 supplies the API/mobile implementation and regressions. **Keep WI-1753 at 17+ until WI-2533 lands.** Afterward, a 13–16 learner proceeds only through policy-resolved self-consent or a successful adult ceremony; missing, stale, or unknown authority fails closed.
...
- **Dogfood prod build (WI-1503) → closed beta (WI-1506, families screened for 13+ teens; WI-1655 device-evidence batch runs with it) → store go-live → public launch.**
  - ⚠ **Screening criterion has an open hole (WI-2535, 2026-07-30) — flagged, not rewritten.** "Screened for 13+ teens" assumes a 13+ teen can transact the family model unaided. Because consent authority resolves from exact age × habitual residence and WI-1753 remains 17+-only until WI-2533 lands, a recruited family whose teens are all **13–16** may be unable to exercise the family model. Narrowing the criterion (to 17+, or to learners whose exact age and habitual-residence policy returns `authorizationForm=self`) is a **product call and is not taken here** → operator/PM ruling needed.
