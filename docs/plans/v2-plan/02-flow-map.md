...
>
> Amended 2026-06-13: lost V1 learning/retention flows must be carried forward:
> learner-written reflection + 1.5x bonus, Capitals/Guess Who discovery,
> first-class supporter/adult self-learning, browse-first self-directed paths,
> and compact concrete progress/reward numbers.
...
| `HOME-03`, `HOME-09`, `HOME-11..HOME-13` | Guardian in legacy modes, old deep links, proxy test paths | Tab rendering, ModeSwitcher, own-learning bridge, proxy shell, legacy dashboard | V2 intentionally retires the mode/proxy/tab-shape matrix, but not adult self-learning | S4 scope chip supersedes modes; Support hub carries persistent self-learning doorway; Me becomes equal scope after first real self-learning; S6 deletes after V0 retirement ruling | S4 then S6 | `HOME-12` proxy is dormant; S6 must decide delete vs keep test seam. Do not regress V1's first-class parent/adult own-learning path into a child-stalled fallback only |
...
| `LEARN-01..LEARN-07`, `LEARN-41..LEARN-53`, `LEARN-55` | Learner in session; system stream; session recovery | Ask anything, guided session, voice, summary, crash/offline/reconnect, parking, topic switch, challenge overlays | Session is still the core learning engine; V2 changes how users arrive, not the engine, and must preserve self-explanation | S1/S2 push into existing session paths; V2 wrap-up turn asks for learner-written "Your Words", files it, and awards the 1.5x reflection receipt; S3 adds park-and-return eval gate; S6 may dissolve exit funnel only after that heir is live | Preserve plus S1/S2/S3/S6 | `LEARN-44` parking lot must prove return via S3 evals before session-summary deletion. `LEARN-07` summary cannot be deleted until reflection-for-bonus is in the V2 wrap-up turn |
...
| `LEARN-17`, `LEARN-19`, `LEARN-21..LEARN-24`, `LEARN-27..LEARN-29`, `LEARN-39`, `LEARN-40` | Learner self; proxy read-only in legacy | Progress, vocabulary, bookmarks, transcripts, My Notes, reports, archived transcript | V2 must keep the paper trail and concrete progress signal while killing tab clutter | S3 Journal/notes/recaps/memory + reward history; S2 hub and Subjects tab for subject-local details and compact numbers; preserve progress until S6 | S2/S3/S6 | S3 must be browse-first, not search-only. Self reports, vocabulary browser/list, archived transcripts, and quantified progress numbers need named V2 homes before Progress deletion |
| `PRACTICE-01..PRACTICE-05` | Learner choosing practice, assessment, review, recitation | Practice hub or return-to-practice | Practice is a mode/action, not a top-level V2 tab, and practice points remain earned receipts | S1 mentor bar/light-practice intent plus S2 hub assessment/review entries; keep route until S6 | S1/S2; Preserve | Add route-catalog/intention coverage for Practice. Sup owner enterability without FamilyHome affordance must be decided |
| `QUIZ-01..QUIZ-18` | Learner in practice/quiz; server-driven quiz state | Picker, launch, play, results, history, malformed/error/quit paths | Quiz remains a practice activity, is already-built retention content, and needs deterministic entry | S1 bar + LightPracticeAffordance ("something lighter?") exposes Capitals and Guess Who; S2 subject context where relevant; preserve quiz routes, scores, personal bests, and points | S1/S2; Preserve | `QUIZ-16` home-discovery needs a `/now`/light-practice replacement; challenge banner, Save&Finish, history/detail still need coverage |
...
| `PARENT-01`, `PARENT-02`, `PARENT-24` | Guardian/supporter | Family home open, child count variants, ambient transition | This is the current support landing | S4 Support hub Mentor tab becomes the V1 Family Home successor: household pulse, family summary, one support card per child/person, wins/support-needs, conversation starter, learn-alongside, reports/nudge actions | S4 | Parent transition notice uses SecureStore only; do not document API ack. `PARENT-24` ambient quirks need explicit disposition |
| `PARENT-03..PARENT-06`, `PARENT-08..PARENT-10`, `PARENT-12`, `PARENT-17..PARENT-19` | Supporter/guardian with linked child | Child card, reports, subject/topic/session drill-down, progress/settings modes | V2 person scope must preserve useful structural visibility without artifact leakage | S4 person scope keeps the child-specific Mentor/Subjects/Journal rendering after explicit child selection; Support-hub bottom tabs do not silently switch to this scope. S4 person-scope Subject hub and structural mask; S5 for shared-record/trust layer | S4/S5 | S4 must read live supportee tables through server mask, not copied dashboard data. Report/detail parity is currently implied and must be mapped per flow ID |
| `PARENT-11`, `PARENT-13`, `PARENT-14`, `PARENT-20` | Guardian/supporter | Recaps, weekly report, Learn This Too, Learn Together | These are family bridge and recap flows, not generic learner flows; adult self-learning is also a first-class monetization path | Preserve until S4/S5; re-home into Support hub Journal/shared record and Support hub Subjects/Mentor conversation/co-learning affordances with visibility contract; S4 Support hub includes persistent self-learning doorway | S4/S5 | V1-only Recaps cannot be the whole support story. `PARENT-14` clone-from-child / Learn This Too becomes contextual co-learning and must not be buried as a child-stalled fallback |
...
## V1 Parent Home Reuse Map

This is the explicit "reuse where" map for the current V1 Family Home / parent surfaces. Reuse the **job and signal**, not necessarily the old component shell. The Support hub bottom tabs are scope-preserving; child-specific views below are reached only by tapping a child scope chip or an explicit child row/link.

| V1 element / job | Reuse posture | V2 home |
|---|---|---|
| `ParentHomeScreen` as one monolithic family landing | Split, do not preserve as a screen | Support hub owns the three tab variants; S6 deletes the old route only after heirs are live |
| `ChildCommandCard` per child | Reuse the card job | Support hub -> Mentor: one support card per child/person with status, mentor headline, current focus, wins/support-needs, conversation starter, Learn alongside, Reports, Nudge |
| `FamilySummaryPanel` for 2+ children | Reuse | Support hub -> Mentor top section: household pulse / family summary / per-child representation before any child gets a second item |
| `SingleChildMentorSlot` / `MentorSlot` | Reuse the insight slot | Support hub -> Mentor: "Works for Emma" memory snippet, celebration, or compact progress insight |
| `ConversationStarterCard` + `buildSingleChildPrompts` | Reuse and broaden | Support hub -> Mentor primary "try tonight"; Support hub -> Subjects contextual prompt beside subject/topic; Support hub -> Journal history of starters shown/sent |
| Dashboard child progress summary (`sessionsThisWeek`, streak, mastery deltas, `currentlyWorkingOn`, subject retention) | Reuse with compact presentation | Support hub -> Mentor child cards; Support hub -> Subjects grouped rows; person scope -> Subjects masked structural hub |
| Success/struggle framing | Reuse the positive-only posture | "solid", "coming up", "needs patience"; no "weak/behind/failing" home copy |
| `LearnTogetherSheet` / `AddToMyLearningButton` / `clone-from-child` bridge | Replace implementation, preserve intent | Contextual co-learning: Support hub -> Subjects/Mentor affordance plus person-scope topic doorway; writes only to supporter Me scope; optional teen-safe fillable payoff chip |
| Weekly recaps, session recap detail, reports | Re-home | Support hub -> Journal supporter archive; person scope -> Journal/shared record under S5 visibility contract |
| `NudgeActionSheet` and nudge rate limits | Reuse | Support hub -> Mentor action entry; S5 preserves deliberate send, quiet hours, and 4/24h limits |
| Child-cap/parent notification loop and transition notices | Reuse as attention/status items | Support hub -> Mentor attention/pulse area; server proxy guard remains tracked |
| Child detail subject/topic/session drill-down | Preserve only after explicit child selection | Person scope -> Mentor/Subjects/Journal; Support hub bottom tabs must not jump into first child |
| V1 Recaps tab as a standalone family tab | Do not preserve as top-level tab | Re-home into Support hub -> Journal and S5 shared record |
| ModeSwitcher / family-vs-study mode / proxy shell | Do not reuse | Replaced by scope chip; S6 deletion after V0 retirement ruling |
| Raw child transcripts, private notes, child Journal artifacts, child mentor memory | Do not reuse for supporter UI | Artifact wall: no everyday supporter read path |
| Demo fallback masking zero-child dashboard | Do not reuse as real state | Variant-zero Support hub: add-child anchor + labeled ghost preview only |

## Phase Backlinks

| Phase | Flow coverage it must cite before build |
|---|---|
| S0 backend primitives | `CC-05`, `CC-21`, `LEARN-15`, `LEARN-26`, `LEARN-44`, `LEARN-49..LEARN-51`, `LEARN-55`; plus `/now` source rows that become cards. Unfinished-session/resume priority must preserve the `CC-05` continuation semantics before S1 renders cards |
| S0-R retention gate | `LEARN-15..LEARN-16`, `SUBJECT-14`, `QUIZ-07`, `CC-10`, `CC-21` |
| S1 Mentor home | `AUTH-01`, `AUTH-13`, `AUTH-15`, `ACCOUNT-01`, `ACCOUNT-33..ACCOUNT-36`, `HOME-01`, `HOME-04..HOME-08`, `HOME-10`, `HOME-14..HOME-16`, `SUBJECT-01..SUBJECT-05`, `SUBJECT-07`, `SUBJECT-18..SUBJECT-20`, `SUBJECT-22`, `LEARN-01..LEARN-07`, `LEARN-30`, `LEARN-41..LEARN-53`, `LEARN-55`, `PRACTICE-01..PRACTICE-05`, `QUIZ-01..QUIZ-18`, `DICT-01..DICT-13`, `HOMEWORK-01..HOMEWORK-12`, `BILLING-06`, `BILLING-13`, `QA-01..QA-07`, `QA-10..QA-11`, `QA-13..QA-14`, `CC-01..CC-05`, `CC-09`, `CC-13..CC-16`, `CC-19`, `CC-21`; plus re-routing the home-targeted pushes (`recall_nudge`, `dictation_review`), light-practice discovery, and reward receipts |
| S2 Subject hub | `ACCOUNT-18`, `SUBJECT-06`, `SUBJECT-08`, `SUBJECT-12`, `SUBJECT-14`, `LEARN-08..LEARN-12`, `LEARN-16`, `LEARN-18`, `LEARN-22`, `LEARN-25`, `LEARN-28`, `LEARN-30..LEARN-37`; plus top-level Subjects browse and compact progress numbers |
| Former S2 -> S3 evidence gate (superseded 2026-06-14) | Non-blocking learning criteria for S1/S2 entry success: feed card open, bar intent match, hub reach, topic/session start, homework start, cold-start conversion **plus parent/supporter comprehension**: a linked parent can answer "what did my kid work on this week?" from the app alone in under one minute. Product ruled no observed-cohort data will exist before S3/S4, so this row no longer blocks execution. Current parent surfaces remain covered until S4/S5 heirs replace them |
| S3 Journal and avatar | `AUTH-10`, `ACCOUNT-04`, `ACCOUNT-06..ACCOUNT-15`, `ACCOUNT-28..ACCOUNT-31`, `ACCOUNT-37`, `ACCOUNT-43..ACCOUNT-46`, `ACCOUNT-48..ACCOUNT-49`, `LEARN-17`, `LEARN-19`, `LEARN-21..LEARN-25` (`LEARN-25` = notes/sessions search slice only; structure search stays S2), `LEARN-27..LEARN-29`, `LEARN-39..LEARN-40`, `LEARN-44`, `BILLING-01..BILLING-05`, `BILLING-07..BILLING-12`, `BILLING-14..BILLING-16`, `CC-06..CC-08`, `CC-11..CC-12`, `CC-18`. `ACCOUNT-16..ACCOUNT-17` and `ACCOUNT-50..ACCOUNT-52` remain preserve-only behind existing child/admin settings until S5 classifies admin/rights vs removal; S3 must not place them in Me-scope Journal or supporter scopes |
| S4 Scope chip and Support hub | `AUTH-15`, `ACCOUNT-01..ACCOUNT-05`, `ACCOUNT-33..ACCOUNT-36`, `HOME-02..HOME-04`, `HOME-09..HOME-13`, `PARENT-01..PARENT-06`, `PARENT-08..PARENT-16`, `PARENT-17..PARENT-25`, `BILLING-06`, `BILLING-13`, `QA-08`, `CC-17`, `CC-19..CC-20`; plus persistent supporter/adult self-learning doorway, Me-scope activation, contextual co-learning, and Support-hub scope-preserving Mentor/Subjects/Journal variants. S4 owns the nudge action entry for `PARENT-15`, `PARENT-16`, and `PARENT-25`; S5 owns final trust/visibility/rate-limit ceremony rules |
...
| S6 Cutover and deletions | By deletion subject, matching the S6 plan's task table: exit funnel `LEARN-07` (gated on `LEARN-44` park-return evidence **and** reflection-for-bonus V2 heir); ModeSwitcher + mode shells `HOME-03`, `HOME-09`, `HOME-11`; proxy mode `HOME-12`, `LEARN-52`, `ACCOUNT-30`; `ParentHomeScreen` `HOME-02`, `PARENT-01..PARENT-02`, `PARENT-21`, `PARENT-24`; More tab + My Notes + Recaps + self mentor-memory `ACCOUNT-06..ACCOUNT-15`, `ACCOUNT-28..ACCOUNT-31`, `ACCOUNT-43..ACCOUNT-46`, `ACCOUNT-48..ACCOUNT-49`, `LEARN-27`, `PARENT-11`; Library tab `LEARN-08`, `LEARN-25`, `LEARN-30..LEARN-32`; `child/[profileId]/*` routes `PARENT-03..PARENT-06`, `PARENT-08..PARENT-10`, `PARENT-12..PARENT-13`, `PARENT-17..PARENT-19`, `ACCOUNT-16..ACCOUNT-17` (classification per S5); Progress tab `LEARN-17`, `LEARN-19`, `LEARN-21..LEARN-24`, `LEARN-28..LEARN-29`, `LEARN-39..LEARN-40`; practice-route disposition `PRACTICE-01..PRACTICE-05` (route kept as bar/feed target; reward label re-homed, not dropped); push re-routing for deleted targets (`PARENT-11`, `PARENT-13`); nav-contract retirement `CC-19` + reward readers `CC-03`/`CC-10`; dormant/orphaned rows `AUTH-17`, `SUBJECT-17`, `SUBJECT-21`, `LEARN-13`, `LEARN-14`, `LEARN-20`, `LEARN-38`, `LEARN-54`; old shell rows `HOME-13`, `PARENT-22`, `PARENT-23`; and, per completion rule 4, every flow file in any cited row's Coverage column whose entry path changes. Self memory consent prompt (`ACCOUNT-49`) must be re-homed with Journal memory before deleting `mentor-memory.tsx` |
...
| `CC-03`, `CC-10` | Celebration/rhythm/reward behavior stays scoped: V2 preserves private earned receipts and removes pressure-style streak/gamification; old V0/V1 readers are not removed before V2 heirs preserve their learning value |
...
| Reflection-for-bonus V2 wrap-up | `LEARN-07`, `LEARN-41..LEARN-53`, `CC-21` | S1/S6 with S3 eval gate |
| Supporter/adult self-learning doorway | `HOME-09`, `HOME-11`, `PARENT-14`, `PARENT-20` | S4 |
| Compact quantified progress / earned receipts | `LEARN-17`, `LEARN-19`, `PRACTICE-01`, `QUIZ-01..QUIZ-18`, `CC-03`, `CC-10` | S1/S2/S3/S6 |
