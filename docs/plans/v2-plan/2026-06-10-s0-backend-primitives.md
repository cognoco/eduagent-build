...
- **Producer coverage is incomplete.** Current code only wires `writeActivityMoment()` in the session-filing path. Before S1/S3 depend on ledger moments beyond `session_filed`, add a producer wiring sweep for every S0 `LedgerKind`: `session_filed`, `topic_mastered`, `retention_due`, `needs_deepening_added`, `recap_ready`, `snapshot_ready`, with exact producer files and tests; or explicitly narrow the consumer plans to the kinds that are actually produced.
