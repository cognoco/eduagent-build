...
| `library.tsx` Library **tab** | S2 Subjects list (structure browse) + S3 Journal archive (cross-subject saved-items browse, EU-6) — browse survives; tab dies | S2 + S3 |
...
- [ ] **T6: Delete the Library *tab* (structure browse survives in Subjects; saved-items browse in Journal).**
  ...
  **done when:** (i) heirs live — the S2 Subjects tab list AND the S3 Journal cross-subject *browsable* archive are shipped + measured (not search-only — EU-6); (ii) `git grep -nE "\(app\)/library|LibraryScreen|push.*'/library'" apps/mobile/src` returns zero; `components/library/*` still imported by the S2 hub + S3 archive (grep confirms live consumers — these are NOT orphans); (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; V0/V1 untouched; (iv) orphaned `library.*` *tab* keys removed (keep archive/search keys the Journal uses); `check-i18n-orphan-keys.ts` passes.
