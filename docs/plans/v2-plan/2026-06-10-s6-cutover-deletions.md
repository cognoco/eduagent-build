...
| `progress/milestones.tsx`, `MilestoneCard`, `useProgressMilestones` | DELETE the standalone gallery screen + its read hook; milestone **data/table/detection KEPT** — surfaces as `milestone_reached` moments (S0 + S3). Heir-live precondition: moments shipped + measured | T5 |
