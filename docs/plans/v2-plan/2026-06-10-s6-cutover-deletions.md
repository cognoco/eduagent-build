...
> Synced to spec amendment 2026-06-10 (§2.1/§15.17 motivation end-state, §3.1/§3.2 cold-start in end state).
...
**Approach:** Replacement-live-before-delete, one dying surface per task. Each task asserts the V2 heir is live and measured (S1–S5 flag-shipped + evidence gate cleared) *before* removing the legacy code, runs the existing test suites green on both sides of the deletion (legacy tests deleted with their subjects, V2 tests stay green), and finishes with a project-wide grep proving no orphaned artifact survives (the repo "Clean up ALL artifacts" rule: types, imports, constants, SecureStore keys, commented-out JSX, fallback branches, i18n keys, e2e flows). Destructive steps carry a `## Rollback` posture. **No new surface is built here** — S1–S5 own every heir; S6 only deletes and flips the default.

## Gates / Blocked-by

This plan is GATED. **No task in §Tasks may start until all three gates below are green.** They are independent and checked at plan-start; the V0-retirement gate (c) additionally blocks only the V0-retirement task group (T9–T12), not the earlier deletions.

- **Gate (a) — P3 park-and-return eval coverage EXISTS (the exit-funnel gate, spec §2/§7/§11).** The exit funnel (`session-summary/[sessionId].tsx`, the 3-screen reflection→summary→filing funnel) may NOT dissolve into the mentor wrap-up turn until park-and-return scenarios are in the eval harness (`pnpm eval:llm`). **Verification:** an `apps/api/eval-llm/flows/park-and-return.ts` flow exists and is registered in `apps/api/eval-llm/index.ts`, AND it includes the spec §2 P3 assertion — *a parked item surfaces within its window even while higher-priority cards compete for the ≤3 feed slots* (the EU-3 competition assertion). **As of 2026-06-10 this flow does NOT exist** (verified: `apps/api/eval-llm/flows/` has no `park-and-return*`; the closest are `session-summary.ts` / `session-recap.ts`). This is an **S3 deliverable** (spec §11 S3 row: "park-and-return eval scenarios into `pnpm eval:llm`"). **Gate (a) blocks T1 (exit-funnel dissolution) specifically.**

- **Gate (b) — S2→S3 evidence gate CLEARED (spec §11 evidence gate · §13.6).** The program is **not committed as a unit**. S1+S2 ship behind `MODE_NAV_V2_ENABLED` and are *measured* against the V1 baseline before S3–S6 proceed. The exact discovery/engagement metric + bar is open decision §13.6 (owner: product). **Verification:** product has recorded the S2→S3 evidence-gate decision as PASSED (the metric moved against V1 baseline) — the back half is NOT pre-authorized by the spec. If S3–S5 were never built (the redesign validly stopped at a measured S2), **S6 does not run at all** — there is nothing to cut over to. **Gate (b) blocks the entire plan.**

- **Gate (c) — §13.1 V0-preservation-constraint retirement ruling MADE by product (owner: Zuzana).** S6 **executes** this ruling; it does not pre-empt it. Until product rules on *which S6 evidence threshold* retires the `MODE_NAV_V0_ENABLED` no-regress constraint (spec §13.1, blocks S6 only), the **HARD CONSTRAINT holds: the V0 5-tab production shape (`MODE_NAV_V0_ENABLED=false` flags-off legacy shell AND the V0-on mode shells) must not regress** (spec §7 hard constraint; §14 "Old nav regression risk" row; CLAUDE.md "Nav mode per-environment" + `project_nav_contract_preserve_v0_off`). **Verification:** a recorded product ruling (ADR or decision log) authorizing V0/V1 retirement at the met threshold. **Gate (c) blocks the V0-retirement task group (T9–T12) only.** T1–T8 (heir-backed deletions that do not touch the V0/V1 flag paths) may proceed under gates (a)+(b) while V0 stays flag-isolated.

**Gate sequencing summary:**

| Gate | Blocks | Cleared when |
|---|---|---|
| (a) P3 eval coverage | T1 only | `eval-llm/flows/park-and-return.ts` registered + EU-3 competition assertion present |
| (b) S2→S3 evidence | entire plan | product recorded evidence-gate PASS (metric moved vs V1) |
| (c) §13.1 V0-retirement ruling | T9–T12 only | product ruling authorizing V0/V1 + legacy-contract retirement |

---

## Replacement-live precondition (applies to every deletion task)

Before deleting any legacy surface, its V2 heir must be **live and measured** — shipped in the S1–S5 plans behind `MODE_NAV_V2_ENABLED`, on dev+preview+staging-OTA, and validated by the S2→S3 evidence gate. The heir map (from the spec §7 "what dies" list + `01-codebase-anchors.md` §4 screen-collapse inventory):

| Dying surface (legacy) | V2 heir (must be live first) | Owning phase |
|---|---|---|
| `session-summary/[sessionId].tsx` exit funnel | Mentor wrap-up conversation turn (in `session/index.tsx` / the bar) | S1 + S3 evals |
...
| `ParentHomeScreen.tsx` special shell | Support-hub Mentor feed | S4/S5 |
| `more/*` tab + `my-notes/*` You-tab hodgepodge | Avatar admin sheet (S3) + Journal tab (S3) | S3 |
| `library.tsx` Library **tab** | Journal cross-subject browsable archive (browse survives; tab dies) | S3 (EU-6) |
...
- `apps/mobile/eas.json` (`:11-15` prod, `:21-26` dev, `:37-42` preview); `.github/workflows/ci.yml` OTA env (`:326-327`) — flag plumbing (T9)
...
- **XP backend kill + the "on track" badge** — the XP **write-path side-effect** strip is S0-R (`applyRetentionUpdate()` cleanup, code-only, spec §2.1/§8.3); the calm "on track" badge that replaces streak display **on the new shell** is an S1 surface (spec §2.1/§15.17). S6 does not own or re-do either. (Note: the spec's "backend-only XP" premise is **stale** — the audit found XP is live and UI-wired; see `01-codebase-anchors.md` §6 + S0-R T12 findings F-XP-1/F-XP-2.)
- **Old-shell XP UI removal + `xp_ledger`/`xpStatus` schema drop — gated here by the V0/V1 no-regress constraint (handed off from S0-R T12).** The new shell never wires XP, but the *shipped* V0/V1 XP screens may not be torn out before they retire. Disposition: F-XP-1 (`session-summary/[sessionId].tsx` bonus-XP copy) retires **with the legacy surface S6 deletes that hosts it** (carried by that surface's deletion task, not a separate XP task). F-XP-2 (`practice/` hub `totalXp` label) sits on a surface S6 **keeps** as a bar/feed target — so its XP-label removal + the `xp_ledger` table / `GET /xp` route / `xpStatus` column drop is an **explicit owner-flagged follow-up** (schema migration after the last reader is gone), NOT silently dropped and NOT done while the reader is still live. Quiz XP (F-XP-3, `quiz_rounds.pointsEarned`) is a separate gamification system, dispositioned separately by the spec owner.
...
| `ModeSwitcher.tsx`, `use-mode-switch.ts` (+ tests) | DELETE; remove `<ModeSwitcher/>` mount (`_layout.tsx:602`) | T2 |
...
| `library.tsx` | DELETE Library tab route (browse survives in Journal, S3/EU-6) | T6 |
...
The order is dictated by (1) which heir is live and (2) which deletion unblocks the next. **T1–T8 run under gates (a)+(b) with V0 still flag-isolated. T9–T12 run only after gate (c).** Within each group, the order is a hard dependency chain — earlier deletions remove the *consumers* of the constructs later deletions remove, so no task ever leaves a dangling import.

```
── Group A: heir-backed UI deletions (gates a + b; V0/V1 flags untouched) ──
T1  Exit funnel dissolves          ⟂ GATE (a) P3 evals   — heir: wrap-up turn (S1/S3)
...
T6  Library tab (browse survives)   — heir: Journal archive (S3/EU-6)
T7  child/[profileId]/* proxy routes — heir: chip person-scopes (S4/S5)
T8  navigation-contract home-branch + V1 tab-shape matrix dead code
...
**Why this order:** T2–T7 remove every runtime *reader* of the legacy contract and proxy state; only then (T8) can the contract's tab-shape branches be deleted without a live caller; only after the §13.1 ruling (gate c) do T9–T12 remove the flag scaffolding itself. Deleting the flags before the readers (or the contract before its consumers) would break the build and regress V0 — forbidden until gate (c).

---

## Tasks

> Each `done when:` is a **before/after check**: (i) the V2 heir is live + measured (replacement-live precondition), (ii) the legacy surface has **zero inbound routes / imports** (grep proves it), (iii) the relevant test suites are **green on both sides** of the deletion (legacy tests removed with their subject; V2 tests still green; the must-not-regress floor — V0/V1 for T1–T8 — unchanged), (iv) **zero orphaned artifacts** by a project-wide grep (types, imports, constants, SecureStore keys, commented-out JSX, fallback branches, i18n keys, e2e flows).

### Group A — heir-backed UI deletions (gates a + b; V0/V1 flags untouched)

- [ ] **T1: Dissolve the 3-screen session exit funnel into the mentor wrap-up turn.** ⟂ **GATE (a).**
  Delete `apps/mobile/src/app/session-summary/[sessionId].tsx`, its `_view-models/session-summary-derived.ts`, and co-located tests. Remove the `session-summary` route from any route catalog, `HIDDEN_TAB_ROUTES`, and every `router.push('/session-summary/...')` / `homeHrefForReturnTo` redirect target — repoint each to the S1/S3 wrap-up conversation turn (the bar's end-of-session turn in `session/index.tsx`). Delete the funnel-only helpers (`reflection-starters.ts`, `summary-draft.ts` + its `KEY_PREFIX='summary-draft'` SecureStore key) **only if** grep shows zero non-funnel consumers; if `MentorMemoryCue` / `SessionSummaryLibraryFilingControls` / `FilingFailedBanner` have other consumers, keep them and delete only the funnel screen.
  ...
  **done when:** (i) gate (a) green — `eval-llm/flows/park-and-return.ts` registered with the EU-3 competition assertion; the S1/S3 wrap-up turn is live behind `MODE_NAV_V2_ENABLED` and measured; (ii) `git grep -nE "session-summary|SessionSummaryScreen|session-summary-derived|reflection-starters|summary-draft" apps/mobile/src` returns ONLY (a) the wrap-up turn's filing call sites (if they reuse a kept helper) and (b) nothing referencing the deleted screen/route; zero `router.push.*session-summary`; (iii) the V2 conversation/session suites pass (`cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/session/index.tsx --no-coverage`); the deleted screen's tests are gone, not skipped; the V0/V1 shells are untouched (no nav-flag file changed in this task); (iv) `pnpm check:i18n` shows no orphaned `summary.*` / `reflection.*` keys left in `en.json` (remove them), and `check-i18n-orphan-keys.ts` passes.

- [ ] **T2: Delete the ModeSwitcher + Study/Family mode machinery.**
  Delete `apps/mobile/src/components/chrome/ModeSwitcher.tsx` (+ test) and `apps/mobile/src/lib/use-mode-switch.ts` (+ test). Remove the `<ModeSwitcher />` mount at `_layout.tsx:602`. In `app-context.tsx`, the `derivedMode`/`setMode`/`mode`/`AppMode` machinery loses its UI consumer here; **strip the mode-switch call paths but leave the provider shell until T11** (T11 deletes the provider once the contract that reads `mode` is also gone) — this keeps T2 a clean, build-green deletion that does not yet touch the V0/V1 flags (gate c). Sweep `analytics.ts` mode-switch events, `RequireFamilyContext.tsx` (if its only purpose is mode-gating), and `LearnerScreen.tsx` ModeSwitcher references.
...
- [ ] **T4: Delete `ParentHomeScreen` as a special shell.**
  ...
  **done when:** (i) heir live — the S4/S5 Support-hub Mentor feed is shipped + measured; (ii) `git grep -nE "ParentHomeScreen|FamilyHome" apps/mobile/src` returns zero in source; the `home.tsx` branch renders the V2 feed unconditionally under the V2 flag; (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/home.tsx --no-coverage` passes; V0/V1 untouched; (iv) no orphaned `tabs.familyHub*` / `tabs.children*` keys; `check-i18n-orphan-keys.ts` passes.

- [ ] **T5: Delete the More tab + the You-tab hodgepodge (`my-notes`, legacy `recaps`/`mentor-memory` entries).**
  Delete `apps/mobile/src/app/(app)/more/*` (index, account, privacy, notifications, accommodation, celebrations, help, security-sessions, _layout, and tests). Delete `my-notes/*`, and the standalone `recaps/*` + `mentor-memory.tsx` route entries that the S3 Journal tab + avatar admin sheet now own. Remove `more`/`recaps` from the V1 tab-shape sets (`navigation-contract.ts` / `legacy-navigation-contract.ts`) — branch-level removal only; file deletions are T10. Repoint any `router.push('/more/...')` to the avatar sheet route (S3) and any `/my-notes` / `/recaps` push to the Journal tab.
  **done when:** (i) heir live — S3 Journal tab + avatar admin sheet shipped + measured; (ii) `git grep -nE "/more/|my-notes|MoreScreen|HUB_ITEMS" apps/mobile/src` returns zero inbound route pushes to the deleted screens; the avatar sheet and Journal own every former destination; (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; the deleted screens' tests are gone; V0/V1 untouched; (iv) orphaned `more.*` / `myNotes.*` i18n keys removed; `check-i18n-orphan-keys.ts` passes.
...
- [ ] **T6: Delete the Library *tab* (browse survives in Journal).**
  Delete `apps/mobile/src/app/(app)/library.tsx` and remove `library` from every V1 tab-shape set. **Keep `apps/mobile/src/components/library/*`** (BookCard, SuggestionCard, ShelfRow, LibrarySearchBar) — the S2 hub and the S3 Journal browsable archive (EU-6) reuse them. The tab dies; the browse does not (spec §7 + §5.4 + EU-6). Repoint any `/library` push to the Journal archive (S3).
  **done when:** (i) heir live — the S3 Journal cross-subject *browsable* archive is shipped + measured (not search-only — EU-6); (ii) `git grep -nE "\(app\)/library|LibraryScreen|push.*'/library'" apps/mobile/src` returns zero; `components/library/*` still imported by the S2 hub + S3 archive (grep confirms live consumers — these are NOT orphans); (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; V0/V1 untouched; (iv) orphaned `library.*` *tab* keys removed (keep archive/search keys the Journal uses); `check-i18n-orphan-keys.ts` passes.
...
- [ ] **T7: Delete the `child/[profileId]/*` proxy routes.**
  ...
  **done when:** (i) heir live — S4 scope chip + S5 person-scope structural rendering shipped + measured; (ii) `git grep -nE "\(app\)/child/|/child/\[profileId\]" apps/mobile/src` returns zero inbound pushes; (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; deleted-route tests gone; V0/V1 untouched; (iv) orphaned `child.*` / parent-dashboard i18n keys removed; `check-i18n-orphan-keys.ts` passes.

- [ ] **T8: Remove the now-dead `navigation-contract.ts` home-branch + V1 tab-shape readers (dead-code only — flags still live).**
  After T2–T7, the V1 contract's `home.screen === 'FamilyHome'` branch (`navigation-contract.ts:376-386`), the `STUDY_TABS`/`FAMILY_TABS`/`PROXY_TABS`/`LEGACY_GUARDIAN_TABS` sets (`:145-168`), and the proxy/family gates have zero live consumers under the V2 flag. Delete those branches/sets from `navigation-contract.ts` and update the V2 seam in `use-navigation-contract.ts` so the shell reads only the V2 three-tab set. **Do NOT delete the file or remove the V0/V1 flags yet** — `resolveNavigationContract` may still be referenced by the V1 code path the flags keep alive until gate (c). T8 is dead-branch removal that keeps the V0/V1 *flagged* paths buildable.
...
> **Group B does not start until gate (c) is green.** Until the §13.1 product ruling, the V0 5-tab production shape must not regress; T9–T12 are the *execution* of that ruling, removing the flag scaffolding and the legacy contract entirely.

- [ ] **T9: Flip `MODE_NAV_V2_ENABLED` to the production default.**
  In `apps/mobile/eas.json`, set `build.production.env.EXPO_PUBLIC_ENABLE_MODE_NAV_V2: 'true'` (the prod build now ships V2) and **remove** `EXPO_PUBLIC_ENABLE_MODE_NAV` / `..._V1` from prod (`:13`), dev (`:23-24`), preview (`:39-40`). In `.github/workflows/ci.yml` OTA env (`:326-327`), remove the V0/V1 lines and keep only the V2 line. In `apps/api/src/config.ts`, the `MODE_NAV_V2_ENABLED` env entry stays (it is now the canonical "on" state) — add a comment that V0/V1 are retired. Do this as a **flag-flip commit separate from code deletion** so a regression is reverted by flipping one env value, not by restoring deleted files.
...
| T2–T8 | Delete UI components/routes/dead branches (mode, proxy, parent-home, more, library tab, child routes, nav dead-code) | Yes | None — pure UI logic, no schema, no persisted state | `git revert <task commit>`. The V0/V1 flagged paths are untouched through Group A, so even mid-deletion the legacy shell renders under its flag. |
...
- ModeSwitcher + proxy mode (§7) → T2 + T3. Tab-shape matrix (§7) → T8 + T10. `ParentHomeScreen` special shell (§7) → T4. More tab + You-tab hodgepodge (§7) → T5. 3-screen exit funnel dissolves into wrap-up turn **only after P3 eval coverage** (§7) → T1 ⟂ gate (a). Library *tab* dies, browse survives in Journal (§7, EU-6) → T6 (components/library kept). ~78 redundant front doors / collapse-follows-usage-evidence (§7) → realized across T1–T8 only after gate (b) evidence. HARD CONSTRAINT V0 5-tab `MODE_NAV_V0_ENABLED=false` must not regress UNTIL retirement ruling (§7) → gate (c) blocks T9–T12; T1–T8 leave flags untouched.
- §11 S6 row (cutover & deletions; exit funnel gated on S3 evals; V0 ruling executed) → whole plan; gate (a) = S3 evals, gate (c) = §13.1 ruling.
- §13.1 V0-retirement ruling (owner: Zuzana, blocks S6) → gate (c), executed in T9–T12, never pre-empted.
- §11 evidence gate / §13.6 (S2→S3 not pre-authorized) → gate (b) blocks the entire plan.
- §14 "Old nav regression risk" row (V0 flag-isolated + test-guarded for any S1–S5 PR) → preserved through Group A; the regression-test floor (nav-contract guard/property/totality + V0/V1 e2e) stays green until T10/T12 delete tests *of deleted behavior*.
...
**Deferred-decision scan:** ordered deletion sequence is fully concrete (T1→T12, dependency-justified); each gate has a named verification artifact; the heir-live precondition is a table not a "TBD"; the `summary-draft` key and the `components/library/*` keep-decisions are resolved (keep iff live consumer by grep); the server-route disposition for the funnel is explicitly deferred-with-owner (S5), not silently dropped. No "TBD"/"handle appropriately" remain.

**Anchors verified 2026-06-10:** `feature-flags.ts:30-31`; `legacy-navigation-contract.ts` (whole file read); `app-context.tsx` flags-off short-circuits `:64,77,152`; `_layout.tsx` ModeSwitcher mount `:602`, tab whitelist `:612-749`, proxy chrome `:596-643`; `home.tsx:161-169` branch (via anchors doc); `session-summary/[sessionId].tsx:1-71`; eas.json env `:13,23-24,39-40`; ci.yml OTA `:326-327`; child/`more`/`my-notes`/`recaps` route inventories; nav-contract guard/property/totality/snapshot/acceptance/usage-guard tests; V0/V1 e2e flows. **Gate (a) verified UNMET:** no `eval-llm/flows/park-and-return*` exists today.

**2026-06-10 amendment reconciliation (spec §2.1/§15.17, §3.1/§3.2):**
- *Motivation end-state (§2.1/§15.17):* The end state S6 reaches has no XP counters, streak display, leaderboards, or accumulated-score surfaces **on the new shell** — confirmed. The XP **backend** side-effect strip is owned by S0-R (code-only, spec §8.3); the "on track" badge replacing streak display is an S1 surface. But the spec's "backend-only" premise is **stale** — XP is live and UI-wired (S0-R T12 findings F-XP-1/F-XP-2). So S6 *does* inherit one handoff: the old-shell XP UI removal + `xp_ledger`/`xpStatus` schema drop, **gated by the V0 no-regress constraint** — surfaces S6 deletes carry their XP readers with them (F-XP-1), while the XP label on a *kept* surface (`practice/` → F-XP-2) + the schema drop is an owner-flagged follow-up after the last reader is gone. See the rewritten Out-of-scope bullets above (authoritative).
...
- *V0/V1 no-regress constraint and §13.1 ruling:* **Unaffected** by the 2026-06-10 amendment. Gate (c) (§13.1 retirement ruling, owner: Zuzana) still blocks T9–T12 only; the V0 no-regress hard constraint holds through all of Group A.
