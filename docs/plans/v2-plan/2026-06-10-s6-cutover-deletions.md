...
| `navigation-contract.ts` | DELETE tab-shape matrix + `home.screen` branch. **Matrix only — NOT the file.** The chip owns scope *selection*; this contract still owns *shape* and the gate surface on the V2 path (WI-2905, verified 2026-08-08). Whole-file deletion requires T10's four code-verified checks. | T8, T10 |
...
- [ ] **T10: Delete `legacy-navigation-contract.ts` + the `navigation-contract.ts` shape matrix + their tests.**
  Delete `apps/mobile/src/lib/legacy-navigation-contract.ts` wholesale (`resolveTabShape`, `computeVisibleTabs`, `computeModeVisibleTabs`, `resolveShellVisibleTabs`, `resolveHomeTabPresentation`, the `GUARDIAN_TABS`/`LEARNER_TABS`/`PARENT_PROXY_TABS`/`FAMILY_MODE_TABS`/`STUDY_MODE_TABS` sets). Reduce `navigation-contract.ts` to only what the V2 shell needs.

> **Deleting `navigation-contract.ts` outright is CODE-VERIFIED, never ADR-derived (WI-2905, 2026-08-08).** The former wording — *"or delete it if the V2 chip fully owns shape"* — restated `MMT-ADR-0024`'s title as if it were a fact. It is not, it is a **Proposed** ADR under explicit non-reliance, and the premise is **false in code**. Verified on `main` 2026-08-08: `use-navigation-contract.ts:78-97` calls `resolveNavigationContract(...)` on **every** path including V2 with no short-circuit ahead of it; `:192-203` returns the resolved `contract` alongside `V2_TABS`; and `useNavigationHomeContract()` (`:213`) and `useNavigationDataScopeContract()` (`:232`) have **no V2 branch at all**. Firing this deletion on the ADR's authority would delete live code that the V2 shell itself consumes.
>
> **Do not delete the file on any document's say-so.** Whole-file deletion may proceed only when *all four* of these are demonstrated at execution time, with the command output pasted into the change-set:
>
> 1. **No V2 invocation.** The V2 path no longer calls `resolveNavigationContract` — `resolveNavigationContract` appears nowhere in `use-navigation-contract.ts`, and `git grep -nE "resolveNavigationContract" apps/mobile/src` returns zero outside `navigation-contract.ts` itself.
> 2. **No live import.** `git grep -nE "from '.*navigation-contract'" apps/mobile/src` returns zero (excluding `legacy-navigation-contract`, deleted earlier in this task).
> 3. **No live type or gate consumer.** `git grep -nE "NavigationContract|navigationContract\.gates" apps/mobile/src` returns zero in source — this is the check the old wording missed, since the contract's **gate surface** outlives its tab-shape matrix and is read by screens, not by the chip.
> 4. **Flags actually retired.** Gate (c) has passed and `MODE_NAV_V0_ENABLED`/`MODE_NAV_V1_ENABLED` no longer select any live path (T11's precondition). While either flag can still resolve a legacy shell, `resolveNavigationContract` is live by definition and this file stays.
>
> If any of the four fails, **reduce the file, do not delete it** — that is the default branch of this task, not a fallback. Authority for the reduction comes from this plan's own gates, never from `MMT-ADR-0024` (see the pre-execution checklist above: any hit using that ADR as authority blocks T10/T11).
>
> **These four checks ARE the acceptance contract — "the chip owns shape" is retired as an operative condition.** The phrase is not independently assertable and must not be re-introduced as a gate: tab shape is not owned by `ScopeChip.tsx` even today, since the V2 tab set is a constant in the **hook** (`use-navigation-contract.ts:22`), so a reviewer asked to certify "shape is fully chip-owned" has nothing mechanical to check and would be certifying a narrative. What actually makes deletion safe is that **nothing live still reads the contract** — which is precisely what checks 1–3 measure and check 4 keeps true. Judge the deletion on the four checks alone; if someone proposes deleting on the strength of the phrase instead, that is the exact failure this guard exists to stop. Delete the now-vacuous guard tests of deleted behavior: `navigation-contract.guard.test.ts`, `.property.test.ts`, `.totality.test.ts`, `.snapshot.test.ts`, `.acceptance.test.ts`, `legacy-navigation-contract.test.ts`, and the `__fixtures__/navigation-matrix.ts` fixture — each tests a construct that no longer exists (delete-the-whole-test, not weaken-the-assertion, per CLAUDE.md test rule (b)). Keep any V2-shell test and repoint it to the V2 set.

> **Test deletion is branch-dependent — do not delete tests of a file you are retaining (WI-2905).** The list above assumes the deleted-construct case. On the **reduce** branch — the default whenever any of the four checks above fails — `navigation-contract.ts` survives, and so must the tests that guard the parts of it that survive:
>
> - **`navigation-contract-usage-guard.test.ts` is KEPT on the reduce branch.** It guards *how the retained contract may be consumed*, not the deleted tab-shape matrix. Deleting it while keeping the contract removes the guard on live code and is strictly worse than the pre-cutover state — the usage guard is most valuable precisely when the contract is reduced, since a reduced surface is easier to misuse.
> - The other listed suites are deleted only for constructs actually removed in the same commit. If a suite still has one live subject after the reduction, keep the suite and remove only the cases whose subject is gone.
> - A test is deleted **because its subject is gone**, never because it started failing after a reduction. A failure after reduction is information about the reduction.
