...
This plan is GATED. **No task in §Tasks may start until gates (a), (b), and (d) below are green; gate (c) additionally blocks only the V0-retirement task group (T9–T12).** They are independent and checked at plan-start; the V0-retirement gate (c) additionally blocks only the V0-retirement task group, not the earlier heir-backed deletions.
...
- **Gate (d) — Lost V1 learning/retention flows have V2 heirs LIVE (amended 2026-06-13).** S6 must not delete a legacy host merely because the shell is cleaner. The V2 heirs must preserve the learning/retention value before deletion:
  ...
  **Verification:** S1 T20/T21, S2 T2/T12, S3 T4/T5/T8, S4 T17, and the V2 wrap-up/reflection test are green. **Gate (d) blocks T1, T5, T6, and any reward/progress-reader deletion; if any heir is missing, the deletion is blocked.**
...
| (d) lost-flow heirs live | T1/T5/T6/reward-progression reader deletions | reflection bonus wrap-up + quiz discovery + adult self-learning + browse-first + concrete progress receipts all green |
...
|---|---|---|
| `session-summary/[sessionId].tsx` exit funnel | Mentor wrap-up conversation turn (in `session/index.tsx` / the bar) that asks for learner-written "Your Words", files it, and awards the 1.5x reflection bonus receipt | S1/S6 wrap-up + S3 evals |
...
- [ ] **T5: Delete the More tab + the You-tab hodgepodge (`my-notes`, legacy `recaps`/`mentor-memory` entries).**
  ...
  **done when:** (i) heir live — S3 Journal tab + avatar admin sheet shipped + measured; Journal shows recaps, learner-authored reflection excerpts/markers, notes/bookmarks, mentor memory, and private earned-reward receipt history where present; (ii) `git grep -nE "/more/|my-notes|MoreScreen|HUB_ITEMS" apps/mobile/src` returns zero inbound route pushes to the deleted screens; the avatar sheet and Journal own every former destination; (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; the deleted screens' tests are gone; V0/V1 untouched; (iv) orphaned `more.*` / `myNotes.*` i18n keys removed; `check-i18n-orphan-keys.ts` passes.
...
- [ ] **T6: Delete the Library *tab* (structure browse survives in Subjects; saved-items browse in Journal).**
  ...
  **done when:** (i) heirs live — the S2 Subjects tab list AND the S3 Journal cross-subject *browsable* archive are shipped + measured (not search-only — EU-6), the Subjects tab full list renders before search, and compact progress numbers render on subject rows; (ii) `git grep -nE "\(app\)/library|LibraryScreen|push.*'/library'" apps/mobile/src` returns zero; `components/library/*` still imported by the S2 hub + S3 archive (grep confirms live consumers — these are NOT orphans); (iii) `cd apps/mobile && pnpm exec jest --findRelatedTests src/app/\(app\)/_layout.tsx --no-coverage` passes; V0/V1 untouched; (iv) orphaned `library.*` *tab* keys removed (keep archive/search keys the Journal uses); `check-i18n-orphan-keys.ts` passes.
...
- 2026-06-13 lost-flow preservation ruling → gate (d): reflection-for-bonus, standalone quiz discovery, adult self-learning, self-directed browse, and quantified progress all need live V2 heirs before deleting legacy hosts.
