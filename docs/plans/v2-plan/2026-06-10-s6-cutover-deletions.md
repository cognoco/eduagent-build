...
- **Gate (b) — S2→S3 evidence gate CLEARED (spec §11 evidence gate · §13.6).** The program is **not committed as a unit**. S1+S2 ship behind `MODE_NAV_V2_ENABLED` and are evaluated with the observed-cohort bar before S3–S6 proceed. **Verification:** product has recorded the S2→S3 evidence-gate decision as PASSED in the Bet Sheet / decision log: 3–5 friendly families with a 13+ teen; pass = (a) the teen returns unprompted at least twice in week one and engages a feed/Subject action that is not only the camera, and (b) the parent can answer "what did my kid work on this week?" from the app alone in under one minute. If the teen return criterion fails across the cohort, V2 stops at measured S2, the feed is demoted, and S6 does not run. **Gate (b) blocks the entire plan.**
...
| (b) S2→S3 evidence | entire plan | product recorded observed-cohort evidence-gate PASS |
