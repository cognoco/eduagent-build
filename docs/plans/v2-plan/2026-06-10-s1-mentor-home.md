...
> Synced to spec amendment 2026-06-10 (§2 P5/P6/P7, §2.1, §3.1, §13.7, §15.14–16/19).

**Goal:** Ship the new Mentor-tab home — a deterministic `GET /now` card feed (one anchor + ≤2 module cards, every card a tappable action, all declinable; the anchor visually unique) behind a **local-first intent-matcher** that turns typed/spoken bar text into a zero-LLM deterministic jump on a confident route-catalog match and a mentor turn otherwise (§2 P5), an ever-present pinned input bar with a camera button, a Homework quick-chip, and a voice mic (transcription-only), a learner **cold-start card** that owns the anchor slot until first real state (§3.1), template-rendered ledger-moment cards (no LLM), and the noticing-loop home affordances — the calm "on track" badge replacing streak display, journey-node advancement at the completion moment, and an interim in-conversation micro-celebration (§2 P7 / §2.1) — mounted behind `MODE_NAV_V2_ENABLED` as additive "screen #89", with V0/V1 nav completely untouched.
...
- **The backend-only XP kill (§2.1 supersession) — NOT in S1.** "Backend XP is killed, not wired" rides on the §8.3 retention-gate cleanup (S0-R), since XP status lives on `retention_cards` writers. S1 only stops *displaying* streak/XP on the home (the "on track" badge replaces it, T15); it removes no backend column. Do not touch `retention_cards`, XP status, or any SRS writer here.
...
| `components/mentor/OnTrackBadge.tsx` | Calm "on track" badge — replaces streak display on the home (§2 P7 / §2.1) |
...
- [ ] **T9: Build `NowCardStack` — one anchor + ≤2 modules + overflow affordance + states (P6 budget).**
  ...
  **P6 anchor-uniqueness (compliance posture, not taste — §2 P6, vulnerable-consumer / DSA Art 25/28 / AADC):** no module card — *especially* the quota / 402 / upgrade module — ever shares the anchor's color, size, or focal position. Pass an explicit `variant: 'anchor' | 'module'` to `NowCard`/`LedgerMomentCard` so a module can never be rendered with anchor styling; the quota/upgrade card (`kind === 'quota_exhausted'` / the 402 path, when S0 emits it) is always a `module`, never the anchor slot, and uses a subordinate surface token (never the anchor emphasis token). The anchor is whichever `/now` card S0 ranked first; S1 never promotes a quota card into the anchor slot.
...
- [ ] **T11: Build the `mentor.tsx` page — feed + pinned bar layout + failure-mode orchestration.**
  ...
  - **"On track" badge (§2 P7 / §2.1, T15):** render `OnTrackBadge` in the header region where a streak chip would otherwise sit — the new shell shows **no** streak/XP/counter.
...
- [ ] **T13: Add all S1 i18n keys to `en.json` (same PR — JSX-literal ratchet).**
  ...
  - `mentorHome.onTrack.label` (the calm "on track" badge — replaces streak display, T15 / §2 P7)
...
- [ ] **T15: Replace streak display with the calm "on track" badge on the home (§2 P7 / §2.1).**
  Create `apps/mobile/src/components/mentor/OnTrackBadge.tsx` — a calm, glanceable "on track" badge that **replaces streak display** on the V2 home (spec §2 P7, §2.1 supersession: "the calm 'on track' badge replaces streak display on the new shell"). NO counters, NO streak number, NO XP, NO leaderboard surface (the new shell carries none — §2 P7). The badge reflects a true state (e.g. due-work-cleared / no overdue retention) derived **deterministically** from the already-fetched `/now` feed (e.g. no overdue `retention_due` anchor) — it is a status reflection of real data, never an accumulative token. Copy `mentorHome.onTrack.label`; semantic tokens; persona-unaware; `t()` only. Render it in the `mentor.tsx` header region (T11) where a streak chip would otherwise sit. This is a **display-only** change: S1 removes no backend column — the backend-only XP kill is §2.1's S0-R cleanup (see Scope "out").
  **done when:** `apps/mobile/src/components/mentor/OnTrackBadge.test.tsx` (T15a) asserts: the badge renders the `mentorHome.onTrack.label` copy from a deterministic "on track" input; the component renders **no** numeric counter / streak digit / XP value (assert the rendered tree contains no streak/XP testID and no number-bearing label — guards the §2 P7 "no countable rewards" rule); the component imports no streak/XP source. Real i18n. `cd apps/mobile && pnpm exec jest --findRelatedTests src/components/mentor/OnTrackBadge.tsx --no-coverage` passes; `persona-fossil-guard.test.ts` green.
...
- [ ] **T17: Build the learner `ColdStartCard` — anchor-slot input + three fill-not-fire chips (§3.1).**
  ...
  - **End-of-first-session line (§3.1 teaching durability 3):** the once-only in-character line `mentorHome.coldStart.firstSessionTeach` ("next time, just tell me what you need — anything") is emitted **once**, at the end of the first session — wire the copy key here; the firing point lives in the session wrap-up (S1 supplies the key + the call site if the wrap-up surface is reachable, else leaves a `// S3: emit at first-session wrap-up` annotation rather than inventing a new surface).
...
- **T15a** `components/mentor/OnTrackBadge.test.tsx` — renders the "on track" label; renders no counter/streak/XP digit (§2 P7 no-countable-rewards guard).
...
- **§2 P7 / §2.1 the noticing loop — no counters/streak/XP; "on track" badge replaces streak; journey arc advances at completion; interim in-conversation celebration; credit to learner's own choice** → T15 (`OnTrackBadge` replaces streak, no countable rewards) + T19 (anchor arc advances at the completion moment, truth-caused) + T18 (`MentorCelebration` interim carrier, own-choice copy, one-shot). XP kill = §2.1/S0-R, not duplicated here (Scope "out"); mentor character = S3 (Scope "out").
- **§3.1 learner cold-start — cold-start card owns the anchor slot until first real state; chips fill not fire; equal-weight↔fill coupling; homework dual-path no preamble (latency = acceptance criterion); placeholder rotation; end-of-first-session line** → T17 (`ColdStartCard` + `first-real-state.ts`) + T11 (renders it in the anchor slot when `!hasFirstRealState`). T17a/b assert fill-not-fire, equal-weight coupling, homework dual-path, self-destruct keyed to first real state.
...
- **On-track badge:** `OnTrackBadge` in `apps/mobile/src/components/mentor/` — the §2 P7 calm status surface that replaces streak display; S2/S3 reuse it on the hub/Journal headers, never re-introducing a streak/XP counter on the new shell.
