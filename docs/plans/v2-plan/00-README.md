...
>
> **No-cohort product ruling (2026-06-14).** There will be no S2/S3 observed-cohort evidence data before S4. The former S2->S3 evidence gate is removed as an execution blocker for S3/S4/S5. ~~S4/S5 are still blocked on the identity-foundation flip + convergence~~ (see the 2026-06-28 correction below — the flip is done) and their own acceptance criteria. S6 remains deferred/irreversible and requires explicit human confirmation plus its non-cohort gates (P3 eval coverage, replacement parity, and the §13.1 V0/V1-retirement ruling).
>
> **⚠️ Identity-flip-is-DONE correction (2026-06-28, verified by live DB query — OVERRIDES all "blocked on IF flip" / "post-IF-flip" / "Tier 3 identity-blocked" text below).** The identity-foundation cutover ("IF flip" + convergence + M-REPOINT) is **complete on the live DBs**, not pending. Queried `doppler -c <cfg> -- node` over `DATABASE_URL`: on **stg `ep-fancy-cherry`** (the DB the app serves) legacy `accounts`/`profiles`/`family_links` are **DROPPED**, `login`/`person` are live (`person`=229 rows), and `mentor_activity_ledger`/`subjects` `profile_id` FKs are **already re-pointed to `person`** (subjects 151/151 rows match) — i.e. M-REPOINT is applied, exactly as WI-678 planned; **prd `ep-holy-leaf`** is the same shape (empty). Only the **orphan `dev` DB** still has legacy tables, and nothing reads it. **Consequence:** `S4`/`S5` are **NOT identity-blocked — they are merely *unbuilt*** (backend pre-built; missing mobile screens, the `GET /scopes/coldstart` route, and 5 S5 break-tests). The "Tier 3 — identity-blocked" framing, the `WI-530 → … → IF flip` blocker chain, and the "post-IF-flip" gates in the plan table are **historical**. Caveat: schema *code* still drifts from the DB (`subjects.ts:54` references the dropped `profiles`) — tracked in `WI-1128`. App is **pre-launch, zero users, all DB rows disposable test data**. Full audit + work items: [`03-gap-analysis-2026-06-28.md`](03-gap-analysis-2026-06-28.md).
...
| [`2026-06-10-s4-scope-chip-support-hub.md`](2026-06-10-s4-scope-chip-support-hub.md) | **S4** | code | Yes (identity is **live** now) | ~~post-IF-flip~~ → **unbuilt, buildable now** (flip done 2026-06-28 note) | Scope chip, Support hub with scope-preserving Mentor/Subjects/Journal variants, person scopes, server-enforced structural mask; consumes M-REPOINT's already-complete `profile_id` FK re-point (now applied) and owns only additive `edge_id`; mode/proxy/tab-matrix retire |
| [`2026-06-10-s5-visibility-contract.md`](2026-06-10-s5-visibility-contract.md) | **S5** | code | Yes (identity is **live** now) | ~~post-IF-flip~~ → **unbuilt; depends on S4** | The trust layer: linking ceremony, sealed non-reportable class, render-equivalence, appeal affordance, managed/credentialized tiers, graduation, kid-initiated revocation |
...
**Tier 1 — buildable now (identity-independent):** `S0-R`, `S1`, `S2` (with `S0` already present in code). *(Note: the live identity model is now `person`, not `profiles` — `profiles` is dropped on stg+prd as of the 2026-06-28 correction; these phases remain identity-independent regardless.)*
...
**Tier 3 — ~~identity-blocked~~ → unbuilt, buildable now (identity cutover is DONE — see the 2026-06-28 correction at top):** `S4`, `S5`, then `S6`.
These consume the person/edge model (`docs/canon/identity/`). The W1/W2 schema is **live on stg+prd** (verified 2026-06-28: `profiles` dropped, `person` populated, `profile_id` FKs re-pointed to `person`). So the historical blocker chain (`WI-530` → identity Phase P → baseline reset `MMT-ADR-0012` → W1 → W2 → IF flip + convergence) is **resolved** — S4/S5 no longer gate on the flip; they gate only on their **own build work** (mobile screens, the `GET /scopes/coldstart` route, S5 break-tests) and on S5-depends-on-S4. Ledger ownership played out as planned: M-REPOINT already re-pointed the `profile_id` FK to `person(id)` without renaming the column; S4 still owns only the separate additive nullable `edge_id` FK to `supportership(id)`. The old "no code may stub `person`/`edge` against today's `profiles`/`family_links`" rule is moot — `person`/`edge` are the live tables and `profiles`/`family_links` are gone. *(S6 remains genuinely deferred — it deletes code/shells and needs human sign-off — but no user data is at stake; the app is pre-launch with zero users.)*
...
- ~~**Identity critical path** (the long pole for S4–S6)~~ **— RESOLVED (2026-06-28).** The identity-foundation runway has landed on the live DBs (cutover + M-REPOINT done; see top correction), so it is no longer the long pole. S4–S6's remaining pole is their **own build work**, not the identity timeline. *(The ASCII graph above still shows the historical `identity: WI-530 → … → IF flip` lane; treat that lane as **complete**.)*
...
3. ~~**S4–S6 wait on the identity cutover flip**~~ **The identity cutover is DONE (verified 2026-06-28; see top correction).** S4/S5 are no longer identity-blocked — they are unbuilt. Schedule them on their own build work (mobile screens, `GET /scopes/coldstart`, S5 break-tests) and S5-after-S4; only **S6** remains human-gated (deletes code/shells; no user data at risk — pre-launch, zero users).
