...
| [`2026-06-10-s4-scope-chip-support-hub.md`](2026-06-10-s4-scope-chip-support-hub.md) | **S4** | code | **Yes** | post-IF-flip + convergence | Scope chip, Support hub, person scopes, server-enforced structural mask; ledger `profile_id → person` re-point is M-REPOINT's (IF), S4 adds the new `edge_id` column (additive); mode/proxy/tab-matrix retire |
...
S0-R (retention gate, parallel) ──┘                                                     │
                                                                                        ▼
   identity: WI-530 ─► Phase P ─► baseline reset ─► W1 (schema) ─► W2 (consent/proxy) ─► IF flip + convergence ──► S4 (chip/mask) ─► S5 (trust) ─► S6 (cutover)
                                                                                                                   ▲
