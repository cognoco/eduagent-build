...
> **STATUS (2026-06-27):** IMPLEMENTED. Subjects browse tab, per-subject hub screen, view-models (`subject-hub-state.ts`), notes `origin` discriminator (self/mentor), and all hub sections present. No remaining scope.
...
| `components/subject-hub/SubjectHubChapterSection.tsx` | One chapter disclosure using the existing visual pattern; either refactor `CollapsibleChapter` to accept generic `renderTopic`/header metadata with regression tests, or create `SubjectHubChapterDisclosure`. Topics render via existing `TopicStatusRow` where possible. Header includes derived chapter progress ("N / N mastered"). Tapping a topic opens the sheet (does not navigate). |
...
| **Next-up block** | `useLearningResumeTarget({ subjectId })` — the resume-target read; **same continuation source the `/now` `unfinished_session` card uses** (S0 `session.resume` / resume-target). Fallback: `computeUpNextTopic()` when no active resume target. | Preserve `LearningResumeTarget` fields (`sessionId`, `resumeFromSessionId`, `resumeKind`, `subjectId`, `topicId`) and derive `bookId` only from loaded book/topic data when needed for display. | `use-progress.ts:232-251`; `lib/up-next-topic.ts`; S0 plan ranking input "unfinished_session" |
...
These amendments are source-verified and must be folded into the task execution.

1. **Preflight / sequencing.** If S1 has not landed, S2 must create `subjects.tsx`, add `MODE_NAV_V2_ENABLED`, and register the V2 tab entries per `01-codebase-anchors.md`; otherwise consume the S1 stubs. The old note saying "T7's flag read" is stale; update task references during execution.
2. **Hub data acquisition needs an aggregate query shape.** `started`, `continue-now`, and `sessionCount` require book-session data. `useBookWithTopics()` cannot be called per book from a dynamic loop inside one hook. Use `useQueries` or build a new `useSubjectHubBooksQuery` that fetches all needed `BookWithTopics` plus `useBookSessions(subjectId, bookId)` results per loaded book. Build `inProgressTopicIds`, `sessionCountByTopicId`, and `continueTopicId` from those sessions.
3. **Next-up must preserve the actual resume target.** Current `LearningResumeTarget` carries `sessionId`, `resumeFromSessionId`, and `resumeKind`; it does not provide `bookId` directly. `HubNextUp` should carry the full `LearningResumeTarget` or at least `resumeKind`, `sessionId`, `resumeFromSessionId`, `subjectId`, `topicId`, and a derived `bookId`. Add a test proving an `active_session` resumes by `sessionId`, not only by `topicId`.
4. **Route catalog must switch under V2.** S2 creates `/(app)/subject-hub/[subjectId]`; sibling S0/S1 route-catalog text still points `subject.hub` at legacy shelf. Add a route-expander test that `subject.hub` pushes `/(app)/subject-hub/[subjectId]` under V2 and preserves legacy shelf behavior only where V0/V1 require it.
5. **`CollapsibleChapter` is not a generic container today.** It currently renders its own bare topic rows and hardcoded header semantics. Either create `SubjectHubChapterDisclosure` using the same visual pattern, or first refactor `CollapsibleChapter` to accept header metadata and `renderTopic`, with tests preserving existing Library behavior.
6. **Specify or remove `deriveTopicState()`.** The surface map names it, but T1's required signatures omit it. Either add the exact `deriveTopicState(input): HubTopicState` signature and tests, or state that state derivation is private to `groupHubChapters()` and remove the public name.
...
- [ ] **T4: Collapsible chapter sections on the hub (NOT separate screens) + topic rows with mastery.**
  Create `components/subject-hub/SubjectHubChapterSection.tsx`. Do not assume `components/library/CollapsibleChapter.tsx` is already a generic container; today it owns bare topic rows and header semantics. Either first refactor it to accept header metadata + `renderTopic` with tests preserving Library behavior, or create a `SubjectHubChapterDisclosure` that copies the visual pattern without changing the Library component. Use the existing `TopicStatusRow` for each topic where it can express the needed state; extend its state union only with co-located tests, otherwise render the mastered glyph in the section wrapper. The chapter header also renders `subjectHub.chapter.progress` derived from the already-present `HubChapter.topics` (`masteredCount = topics where state === 'mastered'`, `totalCount = topics.length`), e.g. "4 / 7 mastered"; choose this text count, not a mini progress bar, for S2. Max depth 2 holds: chapter section → topic row; tapping a topic row calls `onOpenTopic(topicId)` (opens the sheet, T5) — it does **not** push a route. Default first chapter expanded, rest collapsed (or all expanded when `chapters.length === 1`).
...
- [ ] **T6: `use-subject-hub.ts` — compose existing hooks into one `SubjectHubData`.**
  Create `hooks/use-subject-hub.ts`. It calls (all existing, no new endpoints unless the aggregate proves necessary): `useBooks(subjectId)`, a stable aggregate/query shape for book topics and sessions (`useQueries` over the loaded books or a new `useSubjectHubBooksQuery`; do not call `useBookWithTopics()` inside a dynamic loop), `useRetentionTopics(subjectId)`, `useLearningResumeTarget({ subjectId })`, and `useSubjectNotes(subjectId)` (T7). Build `inProgressTopicIds`, `sessionCountByTopicId`, and `continueTopicId` from book-session data. It runs the T1 pure functions to produce:
  ```ts
...
- §5.2 collapsible chapter sections **on the hub screen, not separate screens**, topics inside with mastery and a per-chapter mastered/total count → T4 (`SubjectHubChapterDisclosure` or a tested generic `CollapsibleChapter` refactor + `TopicStatusRow`, in-hub, header progress, no route push).
