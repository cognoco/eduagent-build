...
### 5. Database — new table, migration 0142 (additive only)
...
Migration `apps/api/drizzle/0142_wi1777_speaking_practice_attempts.sql`,
...
- [x] T5: DB schema + migration 0142 + repository sub-factory. Test: read-back through `createScopedRepository` (integration test, not run locally — see T6). Discovered during verification: RLS coverage requires the ENABLE + CREATE POLICY statements in the SAME migration (added to `0142_wi1777_speaking_practice_attempts.sql`) and a manifest entry in `apps/api/src/services/database-rls-coverage.ts`'s `PROFILE_SCOPED_TABLES` (a second, API-side RLS guard beyond `packages/database`'s scanner) — both required, neither was "automatic" as originally assumed; both are now green.
...
Migration 0142 only creates a new table — no rollback procedure needed; a
