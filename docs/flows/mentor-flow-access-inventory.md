...
| PARENT-06, PARENT-13 | as PARENT-03 | Reports: monthly + weekly merged list with pinned latest-weekly hero, NEW badge, next-cron-date empty state. Monthly detail marks viewed once (`POST .../reports/:id/view`). Weekly detail (push-driven) marks viewed once server-guarded; empty-week CTA opens NudgeActionSheet in place. Child report view endpoints exist ONLY as dashboard child variants — the self-report twins are broken (LEARN-29, learner-side). |
