> **STATUS: REMAINING-FLOW RERUN UPDATED 2026-06-20** — all 280 flow-plan rows marked from Chrome/Chromium browser sweeps, seeded scenarios, and source-backed checks for dormant/native-only paths. The stale rollup still said 40 pass-with-issues, but the detailed table had 14 remaining issue-marked rows before this rerun. Fresh deterministic reruns cleared the account/settings, auth/consent/profile-lens, curriculum, and parent route packs; Chrome reruns still fail QUIZ-16, QA-02's mentor-audit smoke, and BRIDGE-04 parent bridge entry surfaces. New Cosmo items from this rerun: WI-878, WI-879, and WI-880.
...
| ACCOUNT-07 | Notifications sub-screen | ✅ | Pass | WI-871 | ✅ 2026-06-20 | Fresh rerun passed `more/notifications.test.tsx` in the account/settings pack; WI-871 is closed. |
