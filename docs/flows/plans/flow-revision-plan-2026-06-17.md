...
| AUTH-03 | Sign-up email verification: verify, resend, change email, back; setActive-failure retry preserves sessionId | ✅ | Pass | WI-870 | ✅ 06-19 | Deterministic jest closure proof: `apps/mobile/src/app/(auth)/sign-up.test.tsx` covers verify-complete, verification failure, resend (prepareEmailAddressVerification re-issued), "Use a different email" back-to-form + code clear, "Back to sign in" replace, and both setActive-failure retry paths (verification + OAuth) re-calling setActive with the SAME preserved sessionId. Account-level email sync is covered at the route layer (`apps/api/src/routes/account.test.ts` → PATCH /v1/account/email). 🚫 Blocked: a real verification code arriving by mailbox is a true external boundary, not asserted. |
| AUTH-04 | Sign in with email/password | ✅ | Pass |  |  | Covered in Chrome/browser sweep with seeded scenarios; no product defect found. |
| AUTH-05 | Additional verification | ✅ | Pass | WI-870 | ✅ 06-19 | Deterministic jest closure proof: `apps/mobile/src/app/(auth)/sign-in.test.tsx` covers the second-factor / additional-verification challenge fallback via Clerk-stubbed attempts — first-factor email_code auto-send + complete, second-factor email_code, TOTP (no-prepare → code entry → complete), backup_code entry + success, TOTP-preferred-over-email ordering, unsupported-MFA contact-support + SSO-suggestion help blocks. 🚫 Blocked: real TOTP/phone codes against staging Clerk (live provider boundary). |
| AUTH-06 | Forgot/reset password | ✅ | Pass | WI-870 | ✅ 06-19 | Deterministic jest closure proof: `apps/mobile/src/app/(auth)/forgot-password.test.tsx` covers send-code (reset_password_email_code), transition to reset form, attemptFirstFactor complete, send/reset failures, the #617 client-timeout recovery, and the #511 reset-complete recovery — setActive-failure shows Retry, retry re-calls setActive with the preserved sessionId, permanent failure routes to sign-in. 🚫 Blocked: a real reset code arriving by mailbox is a true external boundary. |
| AUTH-07 | Auth screen navigation | ✅ | Pass |  |  | Covered in Chrome/browser sweep with seeded scenarios; no product defect found. |
| AUTH-08 | OAuth sign in/up | ✅ | Pass | WI-870 | ✅ 06-19 | Deterministic jest closure proof: `apps/mobile/src/app/(auth)/sign-in.test.tsx` + `sign-up.test.tsx` cover OAuth success (Google/Apple/OpenAI → setActive), failure (provider error surfaced), no-session-returned error, incomplete-signIn redirect-to-sign-in with Sentry escalation (BUG-510), missing-fields / incomplete-signUp account-linking messages, the cancel path (setActive-failure → retry UI + "Cancel sign-in"/"Try another method" clears pending state), and the retry path re-calling setActive with the preserved sessionId. 🚫 Blocked: real OAuth provider consent completion in the in-app browser (external provider boundary). |
| AUTH-09 | SSO callback completion/fallback/cancel | ✅ | Pass | WI-870 | ✅ 06-19 | Deterministic jest closure proof: `apps/mobile/src/app/sso-callback.test.tsx` covers maybeCompleteAuthSession called on mount (BUG-261), completion (isSignedIn → replace '/'), the 10s timeout fallback (hidden <10s, shown at 10s), immediate fallback on maybeCompleteAuthSession rejection with Sentry escalation (CR-110), and the cancel/deep-link return (fallback button → replace '/(auth)/sign-in'). 🚫 Blocked: the native OAuth deep-link handoff itself (device/provider boundary). |
...
| ACCOUNT-06 | More hub | ⚠️ | Pass w/ issues | WI-874 | ✅ 2026-06-20 | WI-874: avatar/media render branch now deterministically covered in `apps/mobile/src/components/account/AccountAvatar.test.tsx` (avatarUrl→`<Image>` vs initials fallback vs `?` placeholder vs null-profile→null vs nav). There is no avatar-upload/image-picker UI in the account flow (profiles use initials only), so the only "media" branch is this render path. |
...
| ACCOUNT-08 | Accommodation picker | ⚠️ | Pass w/ issues | WI-875 | ✅ 2026-06-20 | Branch coverage completed (WI-875): the picker no-op/restore edge is now deterministic — re-selecting the already-active mode fires no PATCH (`more/accommodation.test.tsx`); the child-editor consent-withdrawn → restore path is covered by the WI-263 block (`child/[profileId]/index.test.tsx`). |
| ACCOUNT-09 | Account security | ⚠️ | Pass w/ issues | WI-874 | ✅ 2026-06-20 | WI-874: age-gate boundary branches are deterministically covered — `packages/schemas/src/age.test.ts` exercises `computeAgeBracket`/`isAdultOwner`/`isUnambiguouslyAdult` at ages 12/13/17/18/19 and for null/undefined birthYear; `apps/mobile/src/lib/navigation-contract.test.ts` covers the `showAccountSecurity`/`showAddChild` gates incl. the #807 null-birthYear regression; `more/account.test.tsx` covers the owner-shows / non-owner-hides security-section wiring. No standalone screen-level age branch remains untested. |
| ACCOUNT-10 | Export my data | ⚠️ | Pass w/ issues | WI-874 | ✅ 2026-06-20 | WI-874: the export delivery branch is now deterministically covered in `more/privacy.test.tsx` — the native `Share.share` path (pre-existing) plus the new `Platform.OS === 'web'` Blob/anchor download path (asserts `mentomate-data-export.json` download, `createObjectURL`/`revokeObjectURL`, no fall-through to Share) and the `if (!doc) return` web guard. The "regional eligibility" decision is server-owned (no client branch); the live provider event remains an external boundary. |
| ACCOUNT-11 | Delete account: initial → confirming | ⚠️ | Pass w/ issues | WI-874 | ✅ 2026-06-20 | WI-874: the confirmation/provider branch is deterministically covered in `apps/mobile/src/app/delete-account.test.tsx` — initial→confirming→scheduled state machine, typed-`DELETE` enable gate, family-pool + App/Play-Store subscription advisories, double-tap race guard, mutation error path, and the owner-only redirect gate. **Blocked (external boundary):** the deletion-confirmation email is dispatched server-side via Inngest/email provider — there is no client code path to exercise, so live email delivery stays out of unit scope. |
...
| ACCOUNT-17 | Child memory consent prompt | ✅ | Pass | WI-871 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/components/memory-consent-prompt.test.tsx`. Real email/deep-link delivery stays external (out of unit scope). |
| ACCOUNT-18 | Subject analogy preference | ✅ | Pass |  |  | Covered in Chrome/browser sweep with seeded scenarios; no product defect found. |
| ACCOUNT-19 | Consent request during underage profile creation | ✅ | Pass | WI-871 | ✅ 06-20 | Deterministic jest: `apps/api/src/services/consent.test.ts` (consent request/URL + auth gate); mobile auth gate in `apps/mobile/src/app/consent.test.tsx`. Live email delivery is the only true boundary, out of unit scope. |
...
| SUBJECT-06 | Broad subject → pick-book | ✅ | Pass | WI-858 | ✅ 2026-06-19 | WI-858 resolved: deterministic Jest proof refreshed and inventory de-staled. Broad routing → pick-book asserted in `create-subject.test.tsx` ("routes broad subjects to the picker screen") + `j09` e2e; filing, shelf→book stack seeding, shelf-entry routing, and `_layout` initialRouteName covered in `(app)/pick-book/[subjectId].test.tsx`, `(app)/pick-book/_layout.test.tsx`, `(app)/shelf/[subjectId]/index.test.tsx`. Stale inventory test column (Practice-hub `practice-subject-picker.yaml`; PARKED pick-book Maestro flows) corrected. |
...
| LEARN-50 | Challenge Round mobile surface | ✅ | Pass | WI-864 | ✅ 2026-06-20 | WI-864: docs/status aligned (drafted-note grounding runs server-side at emission via `buildValidatedDraft`/`validateNoteDraft`, `session-exchange.ts:556`, with a `body:null` fallback; flag `CHALLENGE_ROUND_RUNTIME_ENABLED` default-false, API-side, no flag flip in scope). Deterministic proof added: mobile drafted-note skip + ungrounded-fallback render (`session/index.test.tsx`), concept-capture parked guard (`concept-capture.test.ts`); existing coverage cited for flag-off, schema grounding, answer-event validation, conservative mastery, needs-deepening persistence. |
...
| PARENT-16 | Nudge rate limit: 4 per recipient child per rolling 24h, counted on `toProfileId` regardless of sender, `pg_advisory_xact_lock`… | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/api/src/services/nudge.integration.test.ts` `[PARENT-16]` pins the real invariants against a live stg DB — parent B blocked after child received 4 from parent A (counted on `toProfileId`, any sender), and the `pg_advisory_xact_lock` concurrency BREAK (≤1 of 5 concurrent succeeds at count=3). Unit `nudge.test.ts` `[PARENT-16]` + route `nudges.test.ts` cover the 4-cap threshold, no-insert-on-limit, and 429 surface. Integration suite is CI-routed (api/db-schema change classes), not run by default jest. |
| PARENT-17 | Child curriculum overview | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/app/(app)/child/[profileId]/curriculum.test.tsx` `[PARENT-17]` — happy path + not-linked gate (pre-existing) plus added loading / error-with-retry / empty-no-subjects / missing-profileId fallback states. |
...
| PARENT-22 | Family-route blocked gate | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/components/guards/RequireFamilyContext.test.tsx` `[PARENT-22]` — study-mode child route blocked + switch-to-family CTA for a family-capable guardian, driven end-to-end through the real `useEnterFamilyMode → setMode → PATCH /app-context` (success navigation + server-rejection error surface; no internal mocks). |
...
| PARENT-24 | Parent-home ambient layer: household-pulse subtitle, `ParentTransitionNotice` | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/components/home/ParentHomeScreen.test.tsx` `[PARENT-24]` — household-pulse subtitle when children are active, greeting-subtitle fallback when none, and `ParentTransitionNotice` appearing once ≥1 child is linked. Notice variants additionally unit-tested in `ParentTransitionNotice.test.tsx`. |
| PARENT-25 | Progress-tab nudge entry: guardian viewing linked child w/ `nudgeRecommended` → `progress-nudge-cta` opens NudgeActionSheet | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/app/(app)/progress/index.test.tsx` `[PARENT-25]` — guardian viewing a linked child with `nudgeRecommended` taps `progress-nudge-cta`, mounting NudgeActionSheet with the child name/id and a working onClose. Sheet internals (templates/send/error) covered separately by `NudgeActionSheet.test.tsx`. |
...
| QA-02 | Post-auth comprehensive smoke | ❌ | Fail — gated | WI-857 / WI-878 / WI-879 | ✅ 2026-07-06 | Manual-preview Chrome rerun got past the prior export stall: mentor-audit smoke ran 21 checks with 13 pass / 8 fail. Failures split between BRIDGE-04 entry surfaces (WI-878) and registry readiness drift against current Mentor/Family shells (WI-879). Manifest reconciled: post-auth-comprehensive-devclient.yaml carries `blocked` and points to Chrome equivalent (`registry-smoke.spec.ts`, `--project=mentor-audit-registry-smoke`) and WI-878/WI-879 gate; YAML preserved pending M1-COMPREHENSIVE rewrite and no longer counted as live Maestro coverage. QA-02 cannot close until WI-878 and WI-879 are resolved. |
...
| QA-08 | Parent add-child regression | ✅ | Pass | WI-872 | ✅ 06-20 | Deterministic jest: `apps/mobile/src/app/create-profile.test.tsx` `[QA-08]` — `?for=child` child-referent copy/title/placeholder, and the parent-adds-child submit path shows the confirmation alert without switching the active profile (does not hijack the parent session). Profile-limit upgrade-alert branch covered by the BILLING-14 path in the same file. |
| QA-09 | Consent email URL regression | ✅ | Pass | WI-871 | ✅ 06-20 | Deterministic jest: `apps/api/src/services/consent.test.ts` → "[QA-09] generated consent email URL shape" locks the approval-link URL builder. Live email send is external. |
...
| CC-07 | Accommodation badge surfaces: non-deletable badge on mentor-memory; selector on child settings + self settings; role-gated capt… | ⚠️ | Pass w/ issues | WI-875 | ✅ 2026-06-20 | Branch coverage completed (WI-875): the mentor-memory badge adult/`older` bracket branch ("Accommodation mode: …") and the child-settings `none`-mode subtitle now have deterministic tests (`mentor-memory.test.tsx`, `child/[profileId]/index.test.tsx`), alongside the existing role-gated `setByParent` caption and self-picker coverage. |
