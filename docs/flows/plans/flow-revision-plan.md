...
| 5  | Core Learning Sessions   | 12 | ❌ | 5 pass, 2 pass-w/issues, 5 fail. Targeted WSL web passed guided learning, first-session entry, read-only transcript, coach-band UI, and voice-speed controls visibility; WSL full web failed Ask/freeform/session-summary path before completion; targeted book-session probe reached an assistant response but then crashed before chips/feedback; Library empty-state subject CTA routed home instead of create-subject. |
...
| 9  | Practice Hub + Quiz      | 18 | ❌ | 14 pass, 1 pass-w/issues, 3 fail. WSL full web and targeted probes covered practice hub, history/detail, review shortcut, empty state, quiz launch/play/results branches, recitation, and answer-check/dispute guards; remaining failures have bug links. |
...
| 11 | Homework                 |  8 | ❌ | 7 pass-w/issues, 1 fail. WSL web UX crawl and targeted camera guards covered homework start, fallback, gallery, image payload, and navigation at partial depth; create-subject-from-homework failed and has a bug link. |
...
| 18 | Regression Smoke         | 12 | ❌ | 9 pass, 2 pass-w/issues, 1 fail. Hosted/WSL web passed quick smoke, post-auth learner UX smoke, parent add-child, tab leak, consent deny confirmation, and quiz full-flow regression after its fix-branch rerun; focused-book generation still fails. |
| **Total** | | **184** | ❌ | 99✅ 49⚠️ pass-w/issues 17❌ 13 blocked 6 removed 0 untested. Hosted Playwright smoke passed 10/10; WSL full Playwright web ran 31 tests with 19 passed, 1 flaky setup, and 11 failed; targeted WSL web probes added settings/account/billing/learning/practice/dictation/homework/retention/progress/subject-onboarding/parent-dashboard/core-session coverage without using the emulator. |
