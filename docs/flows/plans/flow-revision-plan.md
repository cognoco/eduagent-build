...
| AUTH-07 | Auth screen navigation (sign-in ↔ sign-up ↔ forgot) | ❌ | Fail | https://www.notion.so/3608bce91f7c81928ecdf32a6e8c115c | ✅ 05-14 | Product fix: moved the sign-up return-to-sign-in row directly below the primary CTA and added focused Jest coverage for CTA/link ordering. Hosted Playwright web smoke run 25852959340 passed sign-in → sign-up → sign-in → forgot-password → sign-in navigation. 2026-05-14 lane rerun attempt on Android/dev-client did not reach the auth flow because Metro/dev-client ended at "Error loading app" / `timeout`; row not promoted until a device flow rerun completes. |
...
| ID | Flow | Tested | Result | Bugs | Doc Updated | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| ACCOUNT-01 | Create first profile | ✅ | Pass | https://www.notion.so/3608bce91f7c816d83dbd583835d897a | | 2026-05-14 targeted Playwright web: J-12 created a first profile from the pre-profile gate and landed on learner home. Command: `pnpm exec playwright test -c apps/mobile/playwright.config.ts apps/mobile/e2e-web/flows/journeys/j12-pre-profile-create-profile.spec.ts --project=later-phases --workers=1 --no-deps`. |
...
| ACCOUNT-24 | Post-approval landing | ✅ | Pass | https://www.notion.so/3608bce91f7c813998ccf07b435b221a | | 2026-05-14 targeted Playwright web: J-13 approved the pending consent token, used Check again, and reached the post-approval destination. Command: `pnpm exec playwright test -c apps/mobile/playwright.config.ts apps/mobile/e2e-web/flows/journeys/j13-consent-pending-parent-approval.spec.ts --project=later-phases --workers=1 --no-deps`. |
...
| SUBJECT-06 | Broad subject → pick a book | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c8146bf41ddde4aeab0dc | | 2026-05-15 focused mobile checks: broad subject routes to picker/session fallback instead of staying on `Preparing your first lesson...`; J-09 web preview reached session chat after subject creation. Dedicated broad-book web rerun still pending, so coverage is partial. |
...
| ACCOUNT-04 | Profile switching | ✅ | Pass | https://www.notion.so/3608bce91f7c81f49152df527f941d9b | | 2026-05-14 targeted Windows Playwright web role-transitions: setup + J-04/J-05/J-06/J-07 passed after the harness opened child progress via the explicit `parent-home-child-progress-*` action. API BUG-62 route test passed for restricted-consent child detail redaction. |
...
| ID | Flow | Tested | Result | Bugs | Doc Updated | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| LEARN-01 | Freeform chat (Ask intent card) | ❌ | Fail | https://www.notion.so/3608bce91f7c81edb885f511f70f86d4 | | 2026-05-15 web rerun reached chat and summary instead of the old library fallback, but the submitted summary stayed pending for ~90s and no Continue action appeared. Still red until the summary submit path completes or recovers from the slow server response. |
...
| LEARN-04 | Core learning loop | ❌ | Fail | https://www.notion.so/3608bce91f7c8161bfedede760ae265c | | 2026-05-15 Playwright web: Library → shelf → book → session no longer crashes and first assistant response renders with feedback controls, but the second message hit the in-session `Lost connection` state before the next response/chips. |
| LEARN-05 | Coach bubble visual variants (light/dark) | ⚠️ | Pass w/ issues |  | Playwright web: inventory UI guard passed | 2026-05-14: web inventory pass seeded active learner and verified the coach band renders with CTA, dismisses cleanly, and leaves the home intent cards responsive. Light/dark/native visual variants were not exhaustively compared on device, so keep a native visual polish pass queued. |
...
| LEARN-07 | Session summary (submit / skip) | ❌ | Fail | https://www.notion.so/3608bce91f7c81edb885f511f70f86d4 | | 2026-05-15 Playwright web: J-08 reached Session Complete and filled a summary, but Submit stayed in the pending spinner state for ~90s and the Continue button never appeared. |
| SUBJECT-02 | Create subject from library empty state | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c817eb19cc45799cfe32e | | 2026-05-15 focused mobile check verifies the no-subject Library empty-state action now routes to `/create-subject`; no dedicated web rerun of the Library-empty entry point yet, so marked partial. |
...
| CC-01 | Conversation-stage chips + feedback gating | ❌ | Fail | https://www.notion.so/3608bce91f7c8161bfedede760ae265c | | 2026-05-15 Playwright web: first assistant response now exposes feedback controls, but second response entered `Lost connection` before `quick-chip-*` returned. Focused Jest verifies bookmark/feedback rendering no longer crashes. |
...
| ID | Flow | Tested | Result | Bugs | Doc Updated | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| LEARN-13 | Recall check | ✅ | Pass | https://www.notion.so/3608bce91f7c8184a549effe9d0e1b21 | | 2026-05-15 Playwright web J-22: seeded `failed-recall-3x` direct `/topic/recall-test?...` opened `recall-test-screen`. |
| LEARN-14 | Failed recall remediation | ✅ | Pass | https://www.notion.so/3608bce91f7c8184a549effe9d0e1b21 | | 2026-05-15 Playwright web J-22: "I don't remember" displayed `remediation-card` and the relearn action. |
| LEARN-15 | Relearn flow (same / different method) | ✅ | Pass | https://www.notion.so/3608bce91f7c816fb3b0c5cc44e970b7 | | 2026-05-15 Playwright web J-22: seeded `retention-due` direct `/topic/relearn` reached a relearn review phase. |
| LEARN-16 | Retention review (library + retention surfaces) | ❌ | Fail | https://www.notion.so/3608bce91f7c8169bb93d6d7b75443c2 | | 2026-05-15 Playwright web J-22: `/progress/{subjectId}` now loads the subject page, but the Memory check card shows `We couldn't load what needs review right now` / `progress-subject-retention-error` instead of `progress-subject-retention-card`. |
...
| QUIZ-05 | Mid-round quit with confirm | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c8147b974e7a9f276f80e | | 2026-05-15 focused Jest verifies confirm quit now replaces to quiz index. End-user rerun is still blocked by the Android/dev-client bundle not loading in Maestro, so coverage is partial. |
...
| QUIZ-08 | Quota / consent / forbidden typed errors | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c8107bae5eb4b90d1e7ee | | 2026-05-15 focused Jest verifies ForbiddenError-shaped and CONSENT_REQUIRED launch failures suppress Retry. End-user rerun is still blocked by the Android/dev-client bundle not loading in Maestro, so coverage is partial. |
...
| QUIZ-11 | Malformed-round guard (BUG-812 / F-015) | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c81afbfefec4fe77c058d | | 2026-05-15 focused Jest verifies malformed fallback Back to quiz home replaces directly to quiz index. End-user rerun is still blocked by the Android/dev-client bundle not loading in Maestro, so coverage is partial. |
...
| HOMEWORK-05 | Gallery import | ⚠️ | Pass w/ issues |  | Jest camera guard passed | 2026-05-14: targeted camera test passed `opens the preview when a gallery image is selected`, exercising the gallery button -> preview path with mocked image picker. Real browser/native gallery picker was not exercised. |
| HOMEWORK-06 | Image pass-through to multimodal LLM | ⚠️ | Pass w/ issues |  | Jest camera guard passed | 2026-05-14: targeted camera test passed session navigation with captured image metadata (`imageUri` path covered with OCR result and serialized homework payload). Real multimodal LLM call was not executed in web inventory. |
| SUBJECT-04 | Create subject from homework branch | ⚠️ | Pass w/ issues | https://www.notion.so/3608bce91f7c8116a36cd5e8216ba8db | | 2026-05-15 targeted camera test `creates a new subject before continuing` passes. Native camera end-user coverage was not rerun, so coverage is partial. |
...
| AUTH-11 | Session-expired forced sign-out | ❌ | Fail | https://www.notion.so/3608bce91f7c81af964ed95a3afc3a93 | ✅ 05-14 | Product fix: session-expired notice now survives auth screen remounts and remains available for five minutes; focused Jest covers the forced-sign-out notice, remount retention, and expiry window. 2026-05-14 lane Android/dev-client rerun attempt was blocked before auth by Metro/dev-client "Error loading app" / `timeout`; row not promoted until a flow rerun completes. |
...
| ACCOUNT-12 | Scheduled deletion state: keep account / cancel deletion | ✅ | Pass | https://www.notion.so/3608bce91f7c81b89765e17ec7a18ef3 | | 2026-05-14 targeted Playwright web (`j19-small-isolated-fixes.spec.ts -g "ACCOUNT-12"`) with `account-deletion-scheduled` seed verified `/delete-account` renders `delete-account-scheduled`, hides `delete-account-confirm`, shows Keep account / sign-out controls, and sends the cancel-deletion request from Keep account. |
...
| ACCOUNT-25 | Parent consent management for a child | ✅ | Pass | https://www.notion.so/3608bce91f7c814eaad1e8fc171d8a3e | | 2026-05-14 targeted Playwright web: J-21 opened a seeded child detail, withdrew consent, saw the grace-period deletion scheduled banner, canceled deletion, and saw the withdraw control restored. Command: `pnpm exec playwright test -c apps/mobile/playwright.config.ts apps/mobile/e2e-web/flows/journeys/j21-parent-consent-management.spec.ts --project=later-phases --workers=1`. |
...
| PARENT-03 | Child detail drill-down | ✅ | Pass | https://www.notion.so/3608bce91f7c81f49152df527f941d9b | | 2026-05-14 targeted Windows Playwright web role-transitions: setup + J-04/J-05/J-06/J-07 passed. Parent home card header still opens child detail/settings; child progress journeys now use the explicit Progress action and can drill into the seeded session recap/back chain. |
| PARENT-04 | Child subject → topic drill-down | ✅ | Pass | none | | 2026-05-14 targeted Playwright web rerun on lane branch: J-16 found the seeded parent progress session card, opened session detail, continued to topic detail, and unwound back to progress. |
| PARENT-05 | Child session / transcript drill-down | ✅ | Pass | none | | 2026-05-14 targeted Playwright web rerun on lane branch: J-16/J-17 both reached the seeded child session detail/recap path from parent progress after the session card rendered. |
...
| PARENT-08 | Subject raw-input audit | ✅ | Pass | none | | 2026-05-14 targeted branch-local Playwright web rerun: `parent-with-children` opened child detail from Home, rendered `subject-card-{subjectId}`, and showed `subject-raw-input-{subjectId}` with `fractions homework`. |
...
| PARENT-11 | Child-session recap (narrative + clipboard + chip) | ✅ | Pass | none | | 2026-05-14 targeted Playwright web rerun on lane branch: J-17 opened the seeded recap, confirmed narrative content was available, and showed the copy-conversation prompt toast. |
| PARENT-12 | Child-subject detail retention badges (data-gated) | ✅ | Pass | none | | 2026-05-14 targeted branch-local Playwright web rerun: `parent-subject-with-retention` opened the child subject page from the parent child detail card and rendered `subject-retention-badge`. |
...
| PARENT-13 | Child weekly report detail (push-driven) | ✅ | Pass | none | | 2026-05-14 targeted branch-local Playwright web rerun: `parent-with-weekly-report` opened child reports, tapped `weekly-report-card-{reportId}`, and rendered `child-weekly-report-hero` plus the sessions metric. |
...
| BILLING-05 | Account/Profile -> Subscription: manage billing deep link | ✅ | Pass | https://www.notion.so/3608bce91f7c812eadd0e0e0c6001e04 | | 2026-05-14 targeted Playwright web (`j19-small-isolated-fixes.spec.ts -g "BILLING-05"`) with `trial-active` seed verified `/subscription` renders `manage-billing-web-info` and Manage billing copy for an active trial. |
...
| CC-11 | i18n / `t()` cross-cutting string layer | ⚠️ | Pass w/ issues |  | Jest i18n guards passed | 2026-05-14: targeted `src/i18n/index.test.ts` and `src/app/uppercase.test.ts` passed 82 tests, covering i18n initialization/fallback behavior and copy casing guards. This is code-level cross-cutting coverage rather than a full locale-by-locale end-user crawl. |
| CC-12 | FeedbackProvider + shake-to-feedback on gate screens | ⚠️ | Pass w/ issues |  | Playwright web: feedback sheet passed | 2026-05-14: web inventory pass opened feedback from the learner home early-adopter CTA, selected a category, entered text, submitted through a mocked successful API response, saw the thank-you state, and closed the sheet. Native shake gesture is not available on web and still needs device confirmation. |
| CC-13 | Streaming error classification + stream-fallback guard | ⚠️ | Pass w/ issues |  | Playwright web + Jest guards passed | 2026-05-14: web inventory pass sent a freeform chat message and confirmed the thinking animation resolves to an assistant bubble. Targeted `src/lib/sse.test.ts` passed, including app-level fallback/done handling and structured stream error classification; broader `use-sessions` guard run timed out, so treat as web + lower-level guard coverage rather than a full end-user error-injection pass. |
...
| QA-06 | Focused-book generation regression | ✅ | Pass | https://www.notion.so/3608bce91f7c8111b454e9393446c856 | | 2026-05-15 focused mobile checks verify ambiguous/focused-book creation falls back into `/session` instead of staying on `Preparing your first lesson...`; J-09 Playwright web also reached `chat-input` after first-curriculum generation. |
...
| QA-11 | Quiz full flow regression | ✅ | Pass | https://www.notion.so/3608bce91f7c81d0be00f2319683ea1e | | 2026-05-14 WSL full Playwright web: J-10 Practice → Quiz → play → results flow failed before completion. 2026-05-14 fix branch: J-10 spec now handles optional challenge start and free-text quiz questions; rerun passed with staging API via Doppler after precreating the NativeWind web cache file. |
...
| 2  | First Profile + Consent  | 12 | ❌ | 6 pass, 6 blocked. Targeted web reruns passed first profile, language setup, consent gates, and post-approval landing; underage/profile setup variants remain blocked by harness/account setup gaps. |
| 3  | Subject Onboarding       | 12 | ⚠️ | 5 pass, 1 pass-w/issues, 6 removed. Targeted web reruns passed learner-home create-subject, subject resolution correction controls, language setup through first session, read-only book/curriculum viewing, and assessment start; broad subject → Pick a Book now routes to picker/session fallback but still has partial dedicated broad-book coverage. Legacy interview/curriculum/accommodation/interests onboarding rows were removed before this sweep. |
| 4  | Learner Home + Resume    |  6 | ⚠️ | 5 pass, 1 pass-w/issues. HOME-01/HOME-06/HOME-08, profile switching, and API resume-target continue passed web; More/settings coverage remains partial. |
| 5  | Core Learning Sessions   | 12 | ❌ | 5 pass, 3 pass-w/issues, 4 fail. Targeted WSL web passed guided learning, first-session entry, read-only transcript, coach-band UI, and voice-speed controls visibility; reruns improved the Ask/freeform/session-summary and book-session paths to partial coverage, while remaining failures are still tracked with bug links. |
| 6  | Library, Books, Topics   |  7 | ✅ | 7 pass. WSL full web passed Library → shelf → book → start learning; targeted web pass covered manage-subject, topic-detail, subject settings, and library-search controls. |
| 7  | Retention & Recall       |  4 | ❌ | 3 pass, 1 fail. Focused reruns restored most recall/relearn routes; one retention surface remains failed and linked for follow-up. |
...
| 9  | Practice Hub + Quiz      | 18 | ⚠️ | 14 pass, 4 pass-w/issues. WSL full web and targeted probes covered practice hub, history/detail, review shortcut, empty state, quiz launch/play/results branches, recitation, and answer-check/dispute guards; prior quiz failures are now non-blocking partial-coverage notes. |
| 10 | Dictation                | 10 | ⚠️ | 3 pass, 7 pass-w/issues. Targeted WSL web probes covered choice, text-preview, surprise generation, playback controls, exit modal cancel, completion, and result recording; photo review/remediation/perfect-score branches are partial/mock-backed and still need native/gallery/live-LLM coverage. |
| 11 | Homework                 |  8 | ⚠️ | 8 pass-w/issues. WSL web UX crawl and targeted camera guards covered homework start, fallback, gallery, image payload, navigation, and homework-to-subject recovery at partial depth. |
| 12 | Account / Settings       | 11 | ❌ | 10 pass-w/issues, 1 fail. WSL web probes rendered account, privacy, notifications, learning preference/accommodation, mentor language, and sign-out controls; session-expired forced sign-out remains failed pending rerun after its product fix. |
| 13 | Account Deletion         |  2 | ⚠️ | 1 pass, 1 pass-w/issues. WSL web probe rendered Delete account; targeted web pass covered scheduled deletion recovery. |
| 14 | Parent Setup + Children  |  8 | ⚠️ | 6 pass, 2 pass-w/issues. WSL full web passed add-first-child CTA; targeted probes covered add-child/profile gating and parent consent/memory paths, with generic child creation still partial. |
| 15 | Parent Dashboard         | 17 | ⚠️ | 14 pass, 3 pass-w/issues. Targeted WSL web passed multi-child dashboard, parent library, reports, accommodation rows, raw-input audit, guided-practice tooltip, retention badges, weekly report detail, and child-topic Understanding/review-status cards. |
| 16 | Billing                  | 13 | ⚠️ | 7 pass, 6 pass-w/issues. WSL web probes covered current plan, trial/usage, family-pool visibility, restore/BYOK visibility, Manage billing web info, top-up fallback, and static tier comparison. |
| 17 | Cross-Cutting Final Pass | 11 | ⚠️ | 6 pass, 5 pass-w/issues. WSL full web passed goBackOrReplace, opaque web backgrounds, and profile-as-lens fallback; targeted guards covered animation, i18n, feedback, streaming, envelope stripping, stale-send, HMR guards, and FlatList refs at partial depth where noted. |
| 18 | Regression Smoke         | 12 | ⚠️ | 10 pass, 2 pass-w/issues. Hosted/WSL web passed quick smoke, post-auth learner UX smoke, parent add-child, tab leak, consent deny confirmation, focused-book generation fallback, and quiz full-flow regression after its fix-branch rerun. |
| **Total** | | **184** | ❌ | 103✅ 55⚠️ pass-w/issues 7❌ 13 blocked 6 removed 0 untested. Hosted Playwright smoke passed 10/10; WSL full Playwright web ran 31 tests with 19 passed, 1 flaky setup, and 11 failed before focused reruns; targeted WSL web probes added settings/account/billing/learning/practice/dictation/homework/retention/progress/subject-onboarding/parent-dashboard/core-session coverage without using the emulator. |

### Blocker Taxonomy

`Blocked` now means a concrete account/service/native/harness blocker prevents judgment. There are no generic untested rows left in this first-status inventory.

| Queue bucket | Count | What it means | Next action |
| --- | ---: | --- | --- |
| untested | 0 | Every inventory row now has an explicit first status. | None for first-status inventory; follow-up work should focus on fixing/rerunning Fail rows and deepening Pass w/ issues coverage. |
