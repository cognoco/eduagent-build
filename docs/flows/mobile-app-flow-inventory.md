...
| SUBJECT-09 | Interview onboarding. Routes straight to tutoring when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true` (or to language-setup for language subjects); legacy mode routes to `/(app)/onboarding/interests-context` (SUBJECT-18) when the LLM extracts interests, then on to analogy-preference | `/(app)/onboarding/interview` | `e2e/flows/onboarding/create-subject.yaml` |
| SUBJECT-10 | Analogy-preference onboarding. Status (2026-05-05): bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true` (see `docs/specs/2026-05-05-subject-onboarding-fast-path.md`). Still reachable from legacy routing when the flag is off; deletion gated on Phase 4 + Settings affordance follow-up. Hidden on language subjects (`pedagogyMode === 'four_strands'`) per BUG-939 | `/(app)/onboarding/analogy-preference` | `e2e/flows/onboarding/analogy-preference-flow.yaml` |
| SUBJECT-11 | Curriculum review. Status (2026-05-05): bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true` (see `docs/specs/2026-05-05-subject-onboarding-fast-path.md`). Still reachable from legacy routing when the flag is off; deletion gated on Phase 4 + Settings affordance follow-up. | `/(app)/onboarding/curriculum-review` | `e2e/flows/onboarding/curriculum-review-flow.yaml` |
...
| SUBJECT-15 | Accommodation-mode onboarding step (none / dyslexia / adhd / etc.). Status (2026-05-05): bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true` (see `docs/specs/2026-05-05-subject-onboarding-fast-path.md`). Still reachable from legacy routing when the flag is off; deletion gated on Phase 4 + Settings affordance follow-up. | `/(app)/onboarding/accommodations` (inserted between analogy-preference and curriculum review) | Partial: implicitly exercised by `analogy-preference-flow.yaml` chain |
...
| SUBJECT-18 | Interests-context picker. Status (2026-05-05): bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true` (see `docs/specs/2026-05-05-subject-onboarding-fast-path.md`). Still reachable from legacy routing when the flag is off; deletion gated on Phase 4 + Settings affordance follow-up. Inserted by interview when the LLM returns interests. Each interest classified as free-time / school / both (default both for untouched). Continue/skip routes to analogy-preference | `/(app)/onboarding/interests-context` | `e2e/flows/onboarding/onboarding-extras-flow.yaml` |
