...
Each row describes prod-active behavior unless tagged. When a row says **Production behavior** / **Dev/staging behavior**, it is calling out a flag-gated split where the user-facing experience differs by environment. The most common case is `FEATURE_FLAGS.ONBOARDING_FAST_PATH` — defaults **off in production**, **on in dev/staging** — so production users walk through the full preference chain, dev/staging users skip it. Other tags used inline:
...
| SUBJECT-09 | Interview onboarding. **Production behavior:** runs the full 4-turn interview after subject create, then routes through interests-context (when LLM extracts interests) → analogy-preference → accommodations → curriculum-review (`ONBOARDING_FAST_PATH` defaults off in prod). **Dev/staging behavior:** routes straight to tutoring after the interview (or to language-setup for language subjects), bypassing all four preference screens (`ONBOARDING_FAST_PATH=true` by default). Audit Section A + Slice 1 PRs 5c/5e/5h flip the prod default and delete the bypassed screens | `/(app)/onboarding/interview` | `e2e/flows/onboarding/create-subject.yaml` |
| SUBJECT-10 | Analogy-preference onboarding. **Production behavior:** prod-active step in the onboarding chain after the interview; hidden on language subjects (`pedagogyMode === 'four_strands'`) per BUG-939. **Dev/staging behavior:** bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true`. Deletion tracked in audit Section D + Slice 1 PR 5h (settings replacement already lives in `subject/[subjectId].tsx` per `useAnalogyDomain()`) | `/(app)/onboarding/analogy-preference` | `e2e/flows/onboarding/analogy-preference-flow.yaml` |
| SUBJECT-11 | Curriculum review. **Production behavior:** prod-active step at the end of the onboarding chain; learner can challenge / skip / add topics or ask why topics are ordered this way. **Dev/staging behavior:** bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true`. Deletion tracked in audit Section D + Slice 1 PR 5h (no settings replacement needed — curriculum is editable in Library) | `/(app)/onboarding/curriculum-review` | `e2e/flows/onboarding/curriculum-review-flow.yaml` |
...
| SUBJECT-15 | Accommodation-mode onboarding step (none / dyslexia / adhd / etc.). **Production behavior:** prod-active step inserted between analogy-preference and curriculum-review. **Dev/staging behavior:** bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true`. Deletion tracked in audit Section D + Slice 1 PR 5h (settings replacement lives in `more.tsx` Accommodation section, profile-level — same endpoint as the onboarding screen) | `/(app)/onboarding/accommodations` (inserted between analogy-preference and curriculum review) | Partial: implicitly exercised by `analogy-preference-flow.yaml` chain |
...
| SUBJECT-18 | Interests-context picker. **Production behavior:** prod-active when the interview LLM returns interests; each interest is classified as free-time / school / both (default both for untouched); continue/skip routes to analogy-preference. **Dev/staging behavior:** bypassed when `FEATURE_FLAGS.ONBOARDING_FAST_PATH=true`. Deletion tracked in audit Section D + Slice 1 PR 5h (settings replacement lives in `mentor-memory.tsx` via `TellMentorInput`) | `/(app)/onboarding/interests-context` | `e2e/flows/onboarding/onboarding-extras-flow.yaml` |
...
| LEARN-26 | First-curriculum session entry (the post-onboarding wall — see "Path 0" in `docs/flows/learning-path-flows.md`). `POST /subjects/:subjectId/sessions/first-curriculum` polls up to `FIRST_CURRICULUM_SESSION_WAIT_MS = 25_000` ms for both first materialized topic AND interview-extracted signals before creating a `learning_sessions` row. `firstCurriculumSessionStartSchema` (`packages/schemas/src/sessions.ts` lines 248–251) accepts only optional `bookId` — no `topicId`, no `rawInput`. Server picks the first topic by `sortOrder` (`findFirstAvailableTopicId` in `apps/api/src/services/session/session-crud.ts:249`), so topic-grain learner intent is dropped here. **Audit Section A (pre-warm, PR 5d) shrinks the 25s wall; Section J (topic matching, PR 5i) closes the intent-drop**. No learner-tappable surface — entry comes from `interview.tsx` and `language-setup.tsx` after submit. Error path on topic timeout returns a 504-style "still preparing your subject" error to the screen | API-driven from `/(app)/onboarding/interview` and `/(app)/onboarding/language-setup` | Code-only — covered indirectly by `onboarding/create-subject.yaml`; no dedicated flow asserts the 25s polling behavior or the topic-grain drop |
