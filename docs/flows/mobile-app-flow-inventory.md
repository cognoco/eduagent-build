...
## What changed through the 2026-05-14 snapshot
...
15. **More tab reorganization (commit ea32d358 plus later More hub split).** More is now a hub with nested sub-screens. The landing screen links to Learning preferences, Mentor memory, Account, Notifications, Privacy & Data, and Help. App language, change password, and Subscription live under `/(app)/more/account`; Export my data and Delete account live under `/(app)/more/privacy`; accommodation controls live under `/(app)/more/learning-preferences` -> `/(app)/more/accommodation`. See `docs/flows/e2e-flow-coverage-audit-2026-05-13.md` for the E2E drift this caused.
...
| ACCOUNT-06 | More is a hub, not one long settings page. User opens More, then chooses Learning preferences, Mentor memory, Account/Profile, Notifications, Privacy & Data, or Help. Account/Profile opens profile, password, app-language, and subscription controls; Privacy & Data opens privacy policy, terms, export, deletion, and withdrawal-archive controls; Learning preferences opens the accommodation picker. Eligible adult owners also see add-child and family-breakdown sharing controls on the hub. | `/(app)/more`, `/(app)/more/account`, `/(app)/more/privacy`, `/(app)/more/notifications`, `/(app)/more/learning-preferences`, `/(app)/more/accommodation`, `/(app)/more/help` | `e2e/flows/account/more-tab-navigation.yaml` — exists, but must be re-run against this current hub/sub-screen journey |
| ACCOUNT-07 | User opens More, taps Notifications, and manages push-notification and weekly-digest switches on the notifications sub-screen. Back returns to the More hub. | `/(app)/more` -> `/(app)/more/notifications` | `e2e/flows/account/settings-toggles.yaml` — exists, but must target the notifications sub-screen |
| ACCOUNT-08 | User opens More, taps Learning preferences, sees the current accommodation summary, then taps the accommodation row. The accommodation screen lets them select accommodation modes; some selected modes reveal celebration-level choices (`all`, `big_only`, `off`) inline below that mode. | `/(app)/more` -> `/(app)/more/learning-preferences` -> `/(app)/more/accommodation` | Partial: `e2e/flows/account/settings-toggles.yaml` covers related settings, but no dedicated current accommodation-flow YAML exists yet |
| ACCOUNT-09 | User opens More, taps Account/Profile, then uses the Change password row rendered by `AccountSecurity`. Owners see the row; non-owner profiles do not. | `/(app)/more` -> `/(app)/more/account` via `change-password-row` | `e2e/flows/account/change-password.yaml` — exists, but must enter through Account/Profile |
| ACCOUNT-10 | User opens More, taps Privacy & Data, then taps Export my data. Native platforms open the share sheet with the JSON export; web downloads `mentomate-data-export.json`. Errors show an export alert. | `/(app)/more` -> `/(app)/more/privacy` -> `more-row-export` | `e2e/flows/account/export-data.yaml` — exists, but must enter through Privacy & Data |
| ACCOUNT-11 | User opens More, taps Privacy & Data, then taps Delete account. The delete-account screen walks through warning, exact `DELETE` typed confirmation, family/subscription advisory copy, and scheduled deletion. | `/(app)/more` -> `/(app)/more/privacy` -> `/delete-account` | `e2e/flows/account/delete-account.yaml`, `e2e/flows/account/account-lifecycle.yaml` — exist, but must enter through Privacy & Data |
| ACCOUNT-12 | If deletion is already scheduled, user lands on the scheduled state of `/delete-account`, taps Keep account (`delete-account-keep`), the cancellation mutation runs, and the app returns to More with the account intact. | `/delete-account` scheduled state, normally entered from `/(app)/more/privacy` | `e2e/flows/account/delete-account-scheduled.yaml`, plus partial coverage in `e2e/flows/account/account-lifecycle.yaml`; update to confirm the current return path |
| ACCOUNT-13 | Privacy policy | `/(app)/more/privacy` -> `/privacy` | Partial: exercised in account lifecycle and More navigation flows |
...
| BILLING-05 | Owner opens Subscription from Account/Profile and taps Manage billing. The app deep-links out to App Store / Play subscriptions when the platform provides a management URL. | `/(app)/more` -> `/(app)/more/account` -> `/(app)/subscription` -> App Store / Play subscriptions | Partial: surfaced in subscription-details flow |
| BILLING-06 | A child profile without entitlement reaches the subscription screen state that renders `ChildPaywall`. From there the child can use notify-parent rather than purchasing directly; the subscription row itself is hidden while the parent is impersonating a child. | child profile with no entitlement on `/(app)/subscription`; no visible child More subscription row | `e2e/flows/billing/child-paywall.yaml` — exists, but should use a valid direct/deep-link or seeded child-paywall entry |
...
| BILLING-08 | Family owner opens Subscription from Account/Profile. When `useFamilySubscription` returns data for `tier === 'family'`, the screen renders `family-pool-section` with family usage details; Family static comparison is also available in PLANS for Family users. | `/(app)/more` -> `/(app)/more/account` -> `/(app)/subscription` family usage section | `e2e/flows/billing/family-pool.yaml` — exists, but must enter through Account/Profile |
...
| BILLING-11 | Trial user opens Subscription from Account/Profile. The screen shows `trial-banner` above Current Plan with "Trial active" copy, optional `subscription.trialEndsAt`, and a status badge that reads Trial. | `/(app)/more` -> `/(app)/more/account` -> `/(app)/subscription` `status === 'trial'` branch | `e2e/flows/billing/subscription-details.yaml` — exists, but must enter through Account/Profile |
| BILLING-12 | Pro or Family customer opens Subscription from Account/Profile and views PLANS. If RevenueCat offerings are unavailable, `getTiersToCompare(currentTier)` appends a read-only comparison card for the other approved tier, with no public upsell action for store-unapproved SKUs. | `/(app)/more` -> `/(app)/more/account` -> `/(app)/subscription` static PLANS block | `e2e/flows/billing/static-comparison-family.yaml`, `e2e/flows/billing/static-comparison-pro.yaml` — exist, but must enter through Account/Profile |
