...
| Mentor/family routes | Not surfaced from the Study shell. Adults can switch to Mentor/Family when eligible, but Study remains their own learning space. Optional setup affordances must not trap the Study user. |
...
| ACCOUNT-03, ACCOUNT-05 | Adult owners only | Add-child and family-plan gating are not required to study. If shown in Study, they are optional setup affordances and must never trap the user. The mentor setup details live in the mentor inventory. |
