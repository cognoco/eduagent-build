...
| **db-migrations** | `apps/api/drizzle/**` | `db:migrate:dev`, `nx run @eduagent/database:test` | `test:api:integration` | Migrate before deploy; rollback section if dropping. Emits `--github-output database=true`; the `Database package tests (db-migrations — WI-1164)` ci.yml step runs the suite on PRs |
