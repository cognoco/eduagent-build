...
| **scripts-typecheck** | `scripts/**/*.ts`, checker inputs, `apps/api/{src,eval-llm}/**`, `packages/*/src/**` | `typecheck:scripts` | — | WI-3061: `scripts/` belongs to no `tsc --build` project reference, so its suites compiled nowhere. Roots come from `scripts/jest.config.cjs`; pre-existing debt is a `(file, code, count)` ratchet in `scripts/scripts-typecheck-baseline.json` — repay debt then `pnpm typecheck:scripts --accept`. No TS2554 is baselined, so wrong-arity calls fail anywhere |
