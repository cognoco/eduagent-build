...
| **integration-typecheck** | `tests/integration/**`, checker inputs, CI | `typecheck:integration` | `test:integration` | Roots come directly from `tests/integration/jest.config.cjs`; hosted CI runs the checker for every non-doc change because suites import sources across packages |
| **scripts-typecheck** | `scripts/**/*.ts`, checker inputs, `apps/api/{src,eval-llm}/**`, `packages/*/src/**` | `typecheck:scripts` | — | WI-3061: `scripts/` belongs to no `tsc --build` project reference, so its suites compiled nowhere. Roots come from `scripts/jest.config.cjs`; pre-existing debt is a `(file, code, count)` ratchet in `scripts/scripts-typecheck-baseline.json`. Gates on NEW/RISING diagnostics only; repaid debt prints a cleanup notice and still passes (same shape as `check-i18n-jsx-literals.ts`), tidied with `pnpm typecheck:scripts --accept`. No TS2554 is baselined, so wrong-arity calls fail anywhere |
