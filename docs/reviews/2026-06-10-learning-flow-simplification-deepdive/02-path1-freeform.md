...
> Cluster scope: Freeform "Ask Anything" — first-message subject classification (CFLF), the filing machinery (≥5-exchange close-path auto-file, MMT-ADR-0019), and the freeform note CTA / "cannot-save" trap. · Analyst: path1 · Date 2026-06-10 · Sources verified at HEAD of `new-llm` (branch confirmed via `git branch --show-current`).
