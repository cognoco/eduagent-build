...
| Column | Notes |
|---|---|
| `profile_id` | **Nullable.** Null for pre-signup events (`app_opened`, `signup_started`). Set once a profile exists. |
...
|---|---|---|---|
| `app_opened` | Yes — funnel denominator | Client → `POST /activation-events` | Dedup: 1 row/device/UTC day. |
...
- `profile_id IS NULL` is **expected** for `app_opened` and `signup_started` — they fire pre-account. Do not treat as a bug.
