...
Uses **`appVersion` policy** for `runtimeVersion`. The runtime version is the app `version` alone (e.g. `"1.0.1"`), taken from `version` in app.json — store build numbers (`android.versionCode`/`ios.buildNumber`) are **not** part of the key, so incrementing a build number does not block incompatible OTAs; only bumping `version` does. Matching is **exact** — a version bump creates a new, disjoint OTA audience, so bumping the version is a release act, never cosmetic. Ratified as policy (with the OTA/native boundary, flag tuples, and channel-ownership rules) by [`MMT-ADR-0054`](adr/MMT-ADR-0054-mobile-release-policy-runtime-version-flags-and-channel-ownership.md).
