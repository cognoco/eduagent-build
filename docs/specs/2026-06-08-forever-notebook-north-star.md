...
> **STATUS UPDATE (2026-07-14): LIVING NORTH STAR.** Slice 1 is partial/live rather than parked: capture and presence/correction read sides shipped. Concept-targeted review, note-correctness nudges, and Slices 2–5 remain roadmap.
...
| — Unified note + saved-from-mentor surface | One learner-content place, two **origins** made visually obvious — *My notes* (the learner's own words, editable) vs *Saved from my mentor* (the mentor's words: definition/example, authoritative, read-only) — across two **views** (in-context on the topic + a global list). Consolidates today's split `topic_notes` / `bookmarks` UI. | Parked — folded here 2026-06-09 from the Library-restructure exploration (`project_library_home_subject_hub_direction`). Deferred *because* the shipped [chat-notes-bookmarks](../_archive/v0-v1-superseded/specs/2026-05-12-chat-notes-bookmarks.md) spec deliberately chose "no new global saved-item abstraction" and kept the two co-discoverable-but-separate; a unified store is a north-star call, not a standalone Library tweak. Honors invariants 3 (the restatement is the kept artifact) + 4 (layer/attribute, never overwrite — *My notes* and *Saved from my mentor* are distinct attributed origins, never merged into one undifferentiated blob). |
