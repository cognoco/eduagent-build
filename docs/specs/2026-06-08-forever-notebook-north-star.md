...
- **Learn** — capture happens passively, as a by-product of tutoring. The learner never "takes notes"; knowledge settles into the record. The notebook you'd keep forever is the one you never maintain.
...
"Forever" is the whole game: the same notebook that held the 11-year-old's multiplication tables holds the adult's compound-interest understanding — and can show the line between them. **Not storage — a second memory that teaches you back.**
...
| 2 — Relevance nudge | The "pure-pull-goes-blind" fix: connection-triggered push, never calendar | Roadmap — invariant 5 |
| 3 — Two-axis confidence | Display staleness × source-agreement as two legible signals, not one bar | Roadmap — invariant 6 |
| 4 — Restatement trajectory | Append-only per-concept evaluation log + "March-you vs now-you" view | Roadmap — invariants 3, 4 (slice-1 table split leaves the seam) |
| 5 — Consented connection graph | Offered cross-subject edges; learner answers train the graph | Roadmap — invariant 7 |
| — Return-as-promotion / archive tier | The two-tier demote-not-delete memory; requires an archive tier that doesn't exist yet | Parked — invariants 1, 2 |
