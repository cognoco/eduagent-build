...
The star is **computed at read time**, never stored — `topic_notes` is not touched. When rendering a note for topic `T`, profile `P`:

- Load `concept_mastery` for the concepts under `(P, T)` **whose `supersededAt` is `NULL`** (the latest round's authoritative set; stale replaced labels are excluded).
...
Presence-only by rule: the star's *presence* is the only signal. Its absence is a neutral "there's a tutor addition here if you want it," never a "you were wrong" mark — this honors the no-struggle copy rule. Today this is *approximately* equivalent to reading `assessments.masteryChallengeVerifiedAt` (also all-solid-gated), but the `supersededAt` filter additionally stops a stale prior-round concept from suppressing a star the latest round earned; v1 derives from `concept_mastery` so the signal is concept-aware and ready for finer per-concept note attribution later.
...
| Note exists, no concepts captured | Note from a session predating this layer | No star, no tutor addition (neutral) | Acceptable — pre-layer notes simply have no concept signal; nothing breaks. |
...
- **Star derivation:** all-solid topic → star; mixed → no star + neutral correction affordance surfaced; a stale superseded weak concept does **not** block the star. Negative path: a non-owner/other-profile read never sees another profile's concept mastery (scoped-read break test).
