...
- `continue/unclear` commits no notice-lifecycle transition at any learner response, including the third. Attempt lifecycle is separate from notice status: on reaching the three-response cap after a valid `continue`, the server ends the current re-check attempt and detaches that attempt's bookkeeping, leaving the notice unresolved and eligible for a later re-offer under ordinary eligibility and cooldown rules — a subsequent attempt runs its own full response cycle.
- An unresolved evaluation commits no transition before the cap. On the third learner response, an unresolved evaluation — and only an unresolved evaluation — deterministically terminalizes as `not_yet/insufficient`. Unresolved covers, for example, unavailable or malformed judging, a referenced answer event the server cannot find, and a verdict whose reason does not match its outcome; the list is illustrative, and every member fails in the same safe direction.
...
- Re-check tests cover both transports, every exact verdict/reason pair, attempt end and re-offerability at the cap after a valid continue, unresolved-only terminalization at the cap, retry idempotency, and tutor-prompt snapshots; the judge prompt also passes a live eval.
