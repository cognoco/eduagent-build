...
- **R2 — a Windows-only tooling bug normalized `--no-verify`.** `@nx/expo/plugin` stack-overflows on Windows (`project_nx_expo_plugin_bug`), and the workaround tells agents to `--no-verify`. CI is Ubuntu. ~~dev is macOS. This is dead weight that undermines the gate model.~~ **CORRECTED 2026-06-09 (WI-537, operator ruling): dev is ¾ Windows-native, not macOS — so this is NOT dead weight. `nx affected` is genuinely broken on the *primary* dev platform; the `--no-verify` escape is load-bearing for the majority and is kept until `@nx/expo/plugin` is fixed upstream (watch-item). The defect is "Windows is degraded," not "Windows is unsupported." See A2 (corrected).**
...
- **A2 — fleet is macOS (dev) + Linux (CI). Windows-specific workarounds are removed, not preserved.** ~~[original assumption]~~ **CORRECTED 2026-06-09 (WI-537, operator ruling): the premise is false. ¾ of dev machines are Windows-native (not WSL); CI is Linux. Windows is the PRIMARY dev platform.** Policy is therefore inverted — _default to making things OS-agnostic; where that's hard **and** the Windows config breaks Mac/Linux, discuss; where it's hard **and** harmless on Mac/Linux, leave it._ **Never "remove Windows support."** Disposition by the 5-category Windows-config taxonomy: **(1) incidental non-portable artifact** (hardcoded because authored on Windows) → make portable; **(2) deliberate workaround imposed on all OSes** → OS-gate it (Windows keeps it, Mac/Linux gets native); **(3) workaround for a Windows-only break** → keep — the fix is making Windows work, not removal; **(4) Windows knowledge/docs** → keep (serves the majority); **(5) already portable / dual-documented** → leave.
...
| `project_nx_expo_plugin_bug` (`--no-verify` for >20 files) | R2 (Windows) | ~~**Remove** — Windows not supported~~ → **KEEP (corrected 2026-06-09, WI-537):** Cat-3 — Windows is the *primary* dev platform; the `nx affected` break degrades it. Keep the escape + memory until `@nx/expo` is fixed upstream (watch-item). |
...
- ~~**Windows workarounds → removed.**~~ **CORRECTED 2026-06-09 (WI-537): Windows is the primary dev platform — workarounds are made OS-agnostic or kept, never blanket-removed. See A2 (corrected).**
