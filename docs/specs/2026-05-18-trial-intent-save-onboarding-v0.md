...
| Under-18 owner selects intent = child, reaches save wizard | Pre-signup intent did not gate on age (intent is routing context only) | Wizard blocks child setup with the adult-owner message; if a stale or bypassed client reaches the API, `createProfileWithLimitCheck()` rejects with `ADULT_OWNER_REQUIRED` | User completes own-learning save instead, or returns when adult; the mobile and API adult-owner gates can be disabled independently with `ADULT_OWNER_GATE_ENABLED` for emergency rollback |
