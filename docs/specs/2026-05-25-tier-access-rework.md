...
6. **Drop dead `premiumModelProfiles` field (per D3).** Verification (2026-05-25) found `premiumModelProfiles` is defined on `TierConfig` and asserted in `subscription.test.ts` but **never read** by any runtime code path (`grep` across `apps/api/src` returns zero non-test, non-config consumers). The field is genuinely dead and can be removed safely.
   ...
   - **Predicate shape:**
     ...
     isFamilyHubEligible(context)
       = isAdultOwner(activeProfile)
         ...
         && (subscription.tier === 'free' || subscription.billingGood)
     ```
     Tier is intentionally not in the eligibility check. Free is unconditionally eligible (no billing to gate). Paid tiers require `billingGood`, which the client computes from the server billing state.
   - **Schema change required for this predicate to work.** The current `NavigationSubscriptionContext` at `apps/mobile/src/lib/navigation-contract.ts:64-66` only exposes `status: 'loading' | 'ready'` (a data-load discriminator) and `tier`. The earlier draft of this predicate relied on `status === 'ready'` to gate paid-status problems — that was incorrect; `'ready'` only means "subscription data has loaded", not "billing is current". Extend `NavigationSubscriptionContext` with `billingGood: boolean | null` (true when server-side `status ∈ {'trial', 'active'}`; false when `∈ {'past_due', 'cancelled', 'expired'}`; null while loading). The mapping lives in the hook that builds the context, not in `navigation-contract.ts` (which stays pure).
   - **Why not just rely on existing billing gating elsewhere.** Today, paid-feature gating for `past_due`/`cancelled`/`expired` is enforced server-side at API call time (the client tier check is a UX accelerator, not a security control). Family Hub display is a navigation-shape decision made client-side before any API call — so the client predicate needs its own billing-state input. Adding `billingGood` keeps the predicate self-contained and avoids a Recaps tab that's visible until the user taps it and gets a 402.
...
- **`isFamilyHubEligible`:** adult Free owner with linked child → true; adult Free owner with 0 children → false; non-owner child profile → false; `past_due` Plus owner → false (requires the new `billingGood` field in `NavigationSubscriptionContext`); Free owner with linked child + billing data not yet loaded → false (during the loading flicker window).
...
6. **Client gating cleanup** — Drop tier paywall on add-child (`more/index.tsx`, `ParentHomeScreen.tsx`). Extend `NavigationSubscriptionContext` with `billingGood: boolean | null` (mapped from server status in the hook). Introduce `isFamilyHubEligible` derived flag. Replace tier checks at the four family-hub display sites (`navigation-contract.ts`, `more/index.tsx`, `profiles.tsx`, `subscription.tsx`). Leave `useFamilySubscription` family-pool gates alone.
7. **Child quota UX** — "Ask your parent" exhaustion screen + parent in-app notification when child hits cap.
