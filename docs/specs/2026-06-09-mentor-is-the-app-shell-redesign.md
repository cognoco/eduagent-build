...
Build ownership: S0 (`retention_due` candidate + the `retention.review` deep link already exist), S1 (the quick-check card copy + the "test something else" override affordance), S2 (the Subject-hub due-review entry). The standalone recall-test screen and the cross-subject interleaved-retrieval front-door remain S6 wire/keep/delete items — the engines stay regardless of the screen dispositions.
