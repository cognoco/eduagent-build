...
- ARCH-11: ~~Workers KV~~ DB-backed cache (`kind: 'home_surface_cache_v1'` discriminator on a row in the `coaching_card_cache` table) for coaching cards (write on Inngest precompute, read on app open) and subscription status (write on Stripe/RevenueCat webhook, read on metering). _Original spec said Workers KV; implementation uses DB table as conscious adaptation — acceptable at current scale._
...
Navy backgrounds (#1a1a3e) in dark mode, teal/lavender tokens centralized in `design-tokens.ts`, no hardcoded hex in components, light mode darkened variants for cream background.
...
**Stories:**
- 11.1: **Navy background + teal/lavender tokens** — dark mode bg/surface/elevated shifted to navy-tinted (#1a1a3e). Teal primary, lavender secondary. Theme wired to system setting with user override.
- 11.2: **Accent cascade fix** — colors accessed via `useThemeColors()` hook and semantic tokens, no hardcoded accent hex in components.
...
**Stories:** 12.1–12.7
Removes the `personaType` enum entirely. Profile stores `birthYear` (integer). Age is derived at runtime. Parent capabilities derived from `familyLinks`. Session intent expressed via home cards, not profile settings.
...
Honest time tracking (wall-clock for users, active-time internal), crash recovery with session resumption, milestone celebrations (celestial theme), adaptive silence detection, and graceful session close.
...
Per-message feedback, quick-action chips, topic switching, escalation visibility, homework overhaul (problem cards, OCR correction, multi-problem flow, learning extraction), and home card dismissal.
...
All phases are complete. The original 7-phase implementation sequence was: (1) Foundation cleanup — Epic 11 brand + Epic 13.1-13.2, (2) Quick wins — Epic 14 Phase A, (3) Homework overhaul — Epic 14 Phase B, (4) Celebration system — Epic 13.3-13.7, (5) Architecture refactor — Epic 12, (6) New features — Epics 7, 8, 14 Phase C, (7) Language learning — Epic 6. Epic 10 ran in parallel throughout. Epic 15 (Visible Progress) was added after the original plan and implemented last.
...
Stories 8.1-8.5 cover text/voice toggle, TTS playback, voice controls, screen-reader coexistence, and accessibility. Story 8.6 (VAD) is optional stretch.
...
| Story | Status | Notes |
|-------|--------|-------|
| 10.1 — Topic skip confirmation & undo | | `POST .../curriculum/unskip` route + `unskipTopic()` service |
| 10.2 — Child-friendly consent text | | consent-copy.ts: "Almost there! We need a grown-up's help" |
| 10.3 — Reassuring profile removal alert | | Alert title: "Profile switched", reassuring body text |
| 10.4 — Actionable error messages | | `format-api-error.ts` centralized; session streaming now surfaces reconnect guidance |
| 10.5 — Plain-language curriculum labels | | Curriculum uses Essential / Current / Cutting-edge, plus "Change my topics" + "Why this order?" |
| 10.6 — Child-friendly relearn descriptions | | Persona-keyed: "Show Me Pictures", "Walk Me Through It" |
| 10.7 — Living Book animation | | Full Reanimated page-flip + sparkle + glow animations |
| 10.8 — Session summary structured prompts | | Phase 0 instrumentation shipped; Phase 1 prompt cards remained decision-gated and unnecessary |
| 10.9 — Softer recall remediation copy | | "Not quite there yet, but that's okay!" + supportive copy |
| 10.10 — Hand-to-parent consent | | Two-phase child→parent handoff, resend path, and delivery feedback all in place |
| 10.11 — Consent deny confirmation | | 12 pass (2 new), unit tested |
| 10.12 — Subject raw input audit trail | | 455 API + 33 schema + 5 hooks pass, E2E written (not run) |
| 10.13 — Guided label tooltip | | 7 pass (all new), E2E written (not run) |
| 10.14 — App Store compliance + age-gated Sentry | | Age-gated Sentry, privacy manifests, privacy policy copy, and `privacyPolicyUrl` config are in place |
| 10.15 — Curriculum completion celebration | | Completion milestone now appears on the ranked home surface and in Library |
...
| 10.23 — Practice-for-test subject picker | | Subject picker modal with single-subject auto-select |
...
The heaviest story — touches consent pipeline, Sentry age-gating, session metadata, test factory/seed, consent-web deep links, and all profile create/read paths.
