...
adr: docs/adr/MMT-ADR-0016-llm-provider-model-selection-and-routing.md (Amendment 1)
status: draft
---

# Gemini-Exit Interactive Routing Build — Implementation Spec

**Goal:** Implement the full §1.5 / Amendment-1 interactive routing matrix and make the fallback path **never** resolve to an under-18-banned vendor (Gemini/Vertex). Concretely: free → **Mistral Small 4** (EU); paid teaching rungs 1–3 (all paid tiers) → **gpt-oss-120b @ Cerebras**; paid fallback + all paid vision → **GPT-5 mini @ low**; deep-reasoning rungs 4–5 → **gpt-5.4 @ medium** for **Plus / Pro / AI-Upgrade-entitled only** — the **Family tier has NO access to gpt-5.4** (owner ruling 2026-06-07); Family's rungs 4–5 stay on **gpt-oss-120b @ Cerebras `high`**. Sonnet 4.6 stays the rung-4–5 fallback.
...
| `router.ts` `getModelConfig` | Pin the matrix behind `LLM_ROUTING_V2_ENABLED` (free→Mistral; paid 1–3→Cerebras; vision→GPT-5 mini/Mistral; 4–5→gpt-5.4 @ medium). |
...
- [ ] **T9: Compliance-aware fallback** — rewrite `getFallbackConfig` (`router.ts:485-550`). See `## Tests T9` (HIGH-severity safety fix — red-green break test required per the Fix Development Rules). Replace the "fall back to Gemini when registered" branches with an **allow-list** selector: `const FALLBACK_FORBIDDEN = new Set(['gemini', 'vertex'])` (under-18-banned), plus a per-rung ordered preference of **compliant** providers; return the first registered, non-forbidden config, else `null` (→ caller raises the existing circuit-open error). Gemini/Vertex must be unreachable as a fallback target regardless of registration. — done when: the break test passes with the fix and fails when reverted; no path returns `provider: 'gemini'` or `'vertex'`.
...
- [ ] **T10: Pin the matrix** in `getModelConfig` behind `LLM_ROUTING_V2_ENABLED`. Flag **off** → byte-identical to today (Gemini default), proven by the existing equivalence snapshot. Flag **on**:
  ...
  - free tier, all rungs, text → `mistral` (`mistral-small-2603`, no reasoning); free does not escalate to a premium model;
  - paid tier (plus/family/pro), rungs 1–3, text → `cerebras` (`gpt-oss-120b`, `reasoningEffort: 'high'`);
  ...
  Use the **existing** `llmTier`/tier inputs already threaded into `getModelConfig` — do not invent a new tier source. — done when: `router.test.ts` gains an `LLM_ROUTING_V2_ENABLED=true` block asserting each row resolves to the stated `{provider, model, reasoningEffort}`, **including a Family-tier rung-4 case asserting `provider:'cerebras'` (NOT `openai`/`gpt-5.4`)**; the flag-off equivalence snapshot is unchanged.
...
- [ ] **T12: Fallback pairing for the new primaries** (with T9's allow-list). Ordered compliant preferences: Cerebras primary → GPT-5 mini → Sonnet; Mistral primary → GPT-5 mini → Sonnet; OpenAI primary → Anthropic (never Gemini); gpt-5.4 (rungs 4–5) → Sonnet. — done when: `router.fallback-compliance.test.ts` asserts the fallback target for each primary and that the chain terminates at `null` (circuit-open), never a forbidden vendor.
...
- **Spec coverage:** every Amendment-1 / §1.5 row maps to a task — free→Mistral (T4, T10), paid→Cerebras (T3, T10), GPT-5 mini fallback+vision (T5, T10, T12), gpt-5.4 @ medium rungs 4–5 (T5 effort plumbing, T10, T11), Sonnet fallback (T12), async (T10). Compliance fix = T9. Refusal handler = T2/T3. Vendor wiring = T6–T8. Judge/gating modes explicitly deferred to a separate spec.
