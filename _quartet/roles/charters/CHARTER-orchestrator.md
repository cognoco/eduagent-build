...
3. **Gate integrity** — Gate-1 strict-green verification where the lane runs one; the two-key
   ...
   (the container, not just its WIs). At graduation, **verify worktree hygiene** *(NEW — decision
   event 31, B-12 ruling a′)*: the lane's shepherd removed its own worktrees on close; poke a live
   shepherd to clean stragglers, and if the shepherd has been stood down, ensure they are cleaned
   (janitor script WI-1672 is the backstop). A lane graduates clean or not at all.
