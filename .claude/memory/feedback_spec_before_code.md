...
**How to apply:** If the user says "build X" and no spec exists, ask whether to create the spec first. Use BMAD commands (`/bmad-bmm-create-prd`, `/bmad-bmm-create-architecture`, etc.) for completeness. The value is in the documents produced, not the method itself — but BMAD enforces the completeness that matters.
