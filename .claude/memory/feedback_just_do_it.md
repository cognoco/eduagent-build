...
This rule is about confirmation gates, not fix scope. If a fix touches 3+ sibling locations, the sweep rule (AGENTS.md § Fix Development Rules ("When fixing a drift that has 3+ sibling locations…")) applies — sweep or track-defer in the same PR; do not ship a single-site fix and "come back later" without a tracked entry.
