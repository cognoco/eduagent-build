...
**PREREQ SATISFIED 2026-06-16** (supersedes the "staging lacked neonctl/NEON_API_KEY → PITR-only" gap, for prod): `neonctl` installed + authenticated on Ramtop as `jorn.jorgensen@zwizzly.com` (broad-admin; OAuth creds persisted in `~/.config/neonctl/`, no API key needed). Broad-admin write access to the MentoMate Neon project verified end-to-end (create+delete branch test off `production`). Snapshot target: project **`lingering-violet-30592106`** (eu-central-1), prod branch **`production`** (`br-green-pond-agpzmrwx`); staging branch `br-delicate-star-agpvtzx3`. MentoMate is ONE Neon project with per-env branches (not per-env projects).
