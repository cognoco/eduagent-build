...
There is also a separate live premium-routing runner at `scripts/premium-routing-pass.ts`. It runs hard Bayes/base-rate exchanges across Plus, Family standard, and Family advanced add-on cases, forcing rung 4 and rung 5 cases so it can verify commercial routing, Gemini-only Family boundaries, Claude/Gemini quality at rung 4, the OpenAI advanced candidate only at rung 5+, sourceAudit, and useful answer shape. Results are written under `tmp/premium-routing/results/`.
