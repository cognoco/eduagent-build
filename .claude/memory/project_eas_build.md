...
**Runtime version policy (2026-04-05):** Switched from `fingerprint` to `appVersion` policy. Fingerprint policy breaks in pnpm monorepo because `.pnpm` store paths differ between Windows and Linux (EAS). Current analysis and restore criteria live in `docs/deployment-and-secrets.md` under "Runtime Version Strategy".
