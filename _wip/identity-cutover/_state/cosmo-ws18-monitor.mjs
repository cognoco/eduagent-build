#!/usr/bin/env node
// WS-18 Cosmo Stage monitor — polls the ZDX Work Items DB filtered to Workstream=WS-18,
...
const WS18 = '3808bce9-1f7c-81a2-9ea1-ee924aeaa0a8'; // WS-18 page
...
async function poll() {
  const res = await fetch(`https://api.notion.com/v1/databases/${DB}/query`, {
    ...
    body: JSON.stringify({
      filter: { property: 'Workstream', relation: { contains: WS18 } },
      page_size: 100,
    }),
  });
...
if (ONCE) {
  poll().then((m) => {
    ...
    console.log(`WS-18 poll OK — ${rows.length} items`);
    ...
    try {
      ...
      if (prev) {
        for (const [wi, st] of cur) {
          if (!prev.has(wi)) console.log(`[${ts()}] WS-18 NEW -> ${wi}=${st}`);
          else if (prev.get(wi) !== st) console.log(`[${ts()}] WS-18 Stage change -> ${wi}: ${prev.get(wi)} => ${st}`);
        }
      }
      prev = cur;
    } catch (e) {
