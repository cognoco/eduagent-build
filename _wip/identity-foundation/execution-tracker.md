...
| WI-579 | WP-W3-pii-error-logging | minor-PII out of logs + Sentry (F-018/074/140) | WP | P2 | WI-571, WI-574 | **in-progress (fix round)** — PR #902: review wants the GC6 deferral record (commit-message block for retained gc1-allow mocks) + 2 considers; CI red on an UNRELATED mobile flake (create-profile.test.tsx timing test; diff is API-only). Executor folding all into one round |
