...
| --- | --- | --- | --- | --- | --- |
| WI-689 | CUT-A | additive identity schema extension (M-HOMES + M-RESEED2 + verify-ext + ADR + canon lockstep) | P1 | — | **review (PR #1092 open, CI pending)** 2026-06-13 — head `329154e62`, branch WI-689, 3 commits. Migrations landed next-free: `0113` baseline reconciliation (pre-existing TS↔journal drift surfaced by preflight — 4 categories, own rollback section, all 3 shepherd safety gates passed), `0114` M-HOMES, `0115` M-RESEED2. Self-review caught a genuine convergence-freeze abort (unguarded `actor_id` FK on `parent_reported` rows, invisible to all-`self_report` dev) → fixed + verify-lockstep. age_knowing supersession documented in ADR + data-model §2B; `age_method` vocab confirmed `{self_report,parent_reported}`. 5 dev verify "fails" = pre-existing 0109 dev drift (resolves at convergence when 0109 re-runs first); 6 new CUT-A checks pass |
