...
- **2026-06-15 — PRG-01 GRADUATED; WI-586 emigrated to PRG-06 (Identity Cutover).** Per the
  WI-586 code-half organization ruling (B), the cutover *completion* — WI-586 (drop-legacy /
  convergence runbook) + its 2 provenance sub-items — moved to the new **PRG-06 "Identity
  Cutover"** workstream (`3808bce9-1f7c-81a2-9ea1-ee924aeaa0a8`); WI-586 Blocked-by re-pointed
  to the PRG-06 code lane; its Description corrected (the prior "code is done" framing was
  disproven by the staging cutover). **WS-9 open-WI audit: 52/52 Closed** → this Workstream's
  Cosmo `Status` set to **Closed**. PRG-01 graduated on its residual **foundation-build**
  scope; the original "delete legacy" outcome is now delivered by PRG-06. This tracker is the
  historical record of the IF build; live cutover state lives in
  `_wip/identity-cutover/execution-tracker.md`.
