...
**Last updated:** 2026-06-13 (CUT wave WI-689..693 all merged/closed; CUT-tail preconditions WI-722/723 workstream-tagged + hard-blocked onto WI-586 + dispatched) · **Owner:** Jorn (+ runway session agents)
...
| WI | unit | What | Pri | dep | status |
| --- | --- | --- | --- | --- | --- |
| WI-722 | CUT-tail-usage | twin `getUsageBreakdownForProfile` onto CUT-B2's `guardianship.ts` reader (stop reading legacy `family_links`) | P1 | WI-692 ✓, WI-693 ✓ | **Executing 2026-06-13** — Ready+Assisted (workstream-tagged, EP=Assisted, refined), claimed `claude:vetinari:WI-722`, worktree `.worktrees/WI-722`. Executor phase-0 plan approved with 3 guardrails (mirror CUT-B3 seam exactly; verify flag-off-byte-identical with a test, not assertion; test profile-set semantic-equivalence vs legacy — not just "no family_links read"). Building. |
| WI-723 | CUT-tail-findelete | `financial_record` creation on the v2 deletion path per data-model §6.1 (CUT-B2 deferred billing-domain) | P2 | WI-692 ✓ | **Executing 2026-06-13** — Ready+Assisted (workstream-tagged, EP=Assisted, refined), claimed `claude:vetinari:WI-723`, worktree `.worktrees/WI-723`. Executor phase-0 plan approved (internal membership lookup; two provisional counsel-owned rows tax+chargeback; `retentionPeriod=NULL` §4.9; writes inside the existing tx/advisory-lock before person drop; `financial_record` has no person FK → survives drop) with 4 guardrails (RESTRICT-gap breadcrumb; exercise the non-empty subscription payload path; verify the relational-query call; PR counsel-owned callout). Building. |
