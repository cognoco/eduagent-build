...
| WI-690 | CUT-canon-intake | account-detachment ruling sec-4 deltas into identity canon (OQ-11; 5 canon deltas, docs-only) | P2 | — | **MERGED 2026-06-13 — `1c95743d4` (PR #1095, admin-merge per ruling below); resubmitting to Reviewing for close.** 2 Codex findings fixed (detached-guardian visibility Supportership-gated per ADR-0008; 13-floor reconciled §3+inv13). Reviewer bounced once on `dod.4.pr_merged` (PR not yet landed) — resolved by the merge. Follow-on: `graduation-pre-org-choice` code-state rename (deferred). |
| WI-691 | CUT-B1 | identity spine cutover — flag plumbing, maintenance gates, bootstrap graph, person-scope twins | P1 | WI-689 ✓ | **review — PR #1101 (head `13eb7eb8e`), fix round in flight.** Phase-0 plan approved (5 seams, 9-step bootstrap, full §2.3a reducer) w/ 5 shepherd guardrails; executor found+fixed a real runtime bug (min(granted_at) ISO coercion). CI green except advisory `claude-review` crash. **13 review threads** (2 Codex P1 + 11 CodeRabbit) — substantive auth/consent correctness (consent `id` tiebreak, BUG-411 email-retry, multi-membership fail-closed, onboarding TOCTOU, profile-scope escalation parity, quota/replay) — fix round dispatched. 3 full-graph integ tests gated on M-REPOINT (expected). |
