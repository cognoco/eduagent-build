...
# CHECK - should correspond to off-the-shelf/best practice standard
...
# CHECK - vs three cohorts of minors (no consent required, consent required - own device, consent required - "Netflix style" account)

# Use of eg. "13-16" as a deifining term..?
...
# CHECK - OK, but hopefully handled by Goolge/Applie directly
...
# CHECK - "never" is a very strong negation here. Do no scenarios exist where something follows from a user having a guardian role?
...
# CHECK - build vs buy is likely cost-dependent
...
# CHECK - too detailed for PRD (but likely correct direction)
...
# CHECK - way too detailed to decide in PRD but OK as example of potential approach
...
# CHECK - is it really correct that student memerhsip should be auto-created? What if someon is signing up to explicitly become a mentor (typically a parent)?
...
# CHECK - isn't this mixing guardianship with mentee? Shouldn't the 1 be a variant of 3 for underage mentees (and then have subvariants for own login + "Netflix model")

# Co-guardian: requires another set of rules if more than one guardian at any time. Isn't "guardianship" just the documentation that consent has been given at some point? Ref. also rule about 'Guardian' NEVER being a role.
...
# Here we're now enforcing that every user MUST be a student, turning all earlier role design upside down. Is this really what's intended
...
- # Enforces multi-Guardian, meaning Guardian is not just consent relationship
...
# CHECK - here the guardianship is describes as carrying consent record only (good!)

# But this is NOT the "core model"
...
# CHECK - separate into two dimensions? 1) Own device or not, 2) age group. Stop using "11-12" and "13-17" as terms
