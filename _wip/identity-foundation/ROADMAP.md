...
- [ ] **J** — **four-gate canon/memory/docs alignment**: **J0** scrub the four domain-canon docs section-by-section before any `_wip/` → `docs/canon/` move; **J1** align retained memories as pointers and cull unprovenanced orphans; **J2** reduce `CLAUDE.md`/`AGENTS.md` to pointer-layer for identity-foundation canon and scrub `docs/project_context.md` of shadow identity/routing canon; **J3** make per-file docs-tree conformance decisions under ADR-0000 §I.4. See Phase J detail above.
