...
| **N** | Sequencing                                              | the dependency map + critical path, folded into the master plan | Claude (you ratify)         | ✅ **DONE** (2026-06-09) — **N.0** ruled the Stream-2 pull-forward partition **EMPTY** (default-defer holds; all 16 cited canon artifacts exist in-tree; complete 134-finding accounting in `stream-2-backlog.md § N.0 partition`); **N.1** sequenced the in-scope set into 5 waves (critical path W1→W2→W3; identity-foundation first as dogfood; all 11 execution-blocking defects a hard pre-W1 gate; baseline reset per `MMT-ADR-0012` is the first execution step) in `2026-06-09-phase-n-sequencing.md` | M                            | **N.0** first rules the Stream-2 pull-forward partition (see *Phase N — detail*); then sequencing proper: a sequenced set of work packages with dependencies and bundles; the identity-foundation workstream is sequenced *first* (dogfood) |
| **O** | Remediation plan (the master plan)                     | a single document                   | Claude (you sign off)       | ✅ **DONE** (2026-06-09) — master plan authored + ratified (architect sign-off): N.1's waves decomposed into **17 WPs + 4 solo Items** (ZDX `Altitude` shape); full 183-finding accounting (49 in-scope→units, 134 out-of-scope **ordered by blast-radius**); `B+/C−` Cosmo-enablement interface formalized (dogfood-in/track-cautious) with **W0 patches decoupled from `WI-530`/Phase-P**; hardened by a 6-dimension adversarial verification workflow + 3 external-QA rounds (all folded; record in Appendix C). Deliverable: `2026-06-09-phase-o-master-plan.md` | N                            | scope (in-scope work packages); out-of-scope workstreams (named, with rationale); sequenced work packages; dependency map; bundle grouping; Cosmo-enablement interface (B+/C− posture; identity-foundation as first dogfood) |
| **P** | Hand off to execution (Cosmo work-package slicing)     | the Cosmo WIs (sliced from O)       | You + Claude                | 🟡 **P slicing executed 2026-06-10** — all 21 O units live in Cosmo (15 WP + 3 Item created as WI-569…WI-586, joining W0 patch WIs 549/550/551; full O §4 dependency edges; Workstream *Identity Foundation*). WI map + coarse status: `_wip/identity-foundation/execution-tracker.md` (the execution entry point from here on). **Execution start of W1+ remains gated on `WI-530`** (W0 patches decoupled, per O §7 decision 4) | O                            | every work package is a Cosmo work item (or grouped into a Cosmo work package); the Cosmo top-down process enablement is the precondition (parallel workstream) |
...
- **Baseline reset** (was "T1 revert") — **superseded by `MMT-ADR-0012` (accepted 2026-06-04)**: the prior
  forward-only-revert call is **rejected**; migration `0106` is **removed from the effective chain** (one-time
  documented baseline reset, *not* undone with a follow-up migration). Execution lands as the **first execution
  step** (post-P; see Phase N.1 sequencing W0). Do **not** delete `0106` in isolation. ⬜
...
- [x] **N** — ✅ 2026-06-09. **N.0** ruled pull-forward subset **EMPTY** (default-defer holds; cited canon all exists; 134-finding accounting in `stream-2-backlog.md § N.0 partition`). **N.1** sequenced the in-scope set into 5 waves (`2026-06-09-phase-n-sequencing.md`): critical path W1→W2→W3, identity-foundation first (dogfood), all 11 execution-blocking defects a hard pre-W1 gate, baseline reset (`MMT-ADR-0012`) the first execution step.
...
- [ ] Baseline reset (MMT-ADR-0012; remove `0106` from the effective chain + land the clean baseline migration) sequenced as the first implementation step (lands *during* the execution phase, after P).
...
- **2026-06-10** — **Phase P slicing executed — the 21 O units instantiated in Cosmo.** The 18 remaining master-plan units (15 WPs + 3 solo Items) created via headless `/cosmo:capture` as **WI-569…WI-586** (project MentoMate), joining the pre-existing decoupled W0 patch WIs **WI-549/550/551** (WI-549/550 found already Closed/Done — PRs #817/#818 merged 2026-06-10; the decoupling worked as designed). Altitude set per O (15 WP / 3 Item; WI-549 corrected Item→WP), the full **O §4 dependency edge set** written as native Blocked-by relations (W0 hard gate = WI-570's 4-entry blocked-by; envelope-router gates on W2 consent; reseed's 10-edge fan-in; the WI-551→WI-583 soft edge as Related Items only), Cosmo **Workstream "Identity Foundation"** created with all 21 units attached in wave order (`Workstream Order` 10–210). **WP-W0-baseline (WI-569) = Ready/Assisted** (front of the gated queue — Ready = DoR met, not "go"); all other gated units Backlog/Unset pending refine. **The execution entry point from here on is `_wip/identity-foundation/execution-tracker.md`** (charter / WI map / sequence / coarse status); this ROADMAP becomes the historical record of the A–P runway. **Execution start of W1+ remains gated on `WI-530`/`WI-533`** (Harness-Hygiene block below, unchanged).
