...
**The WI-2080 finding was consumed as settled fact and not reopened**, per instruction. What this slice verified — as implementation detail for the correction, not as re-investigation — is that no current surface writes a selection: `accentPresetId` initialises to `null` (`theme.ts:17`) and nothing outside the context provider calls the setter. **Corrected at review:** the first draft went further and claimed the selection is unset for every user, making cleanup a visual no-op. That is wrong — see §8 finding R5.
...
## 8 · Corrections applied at review

Five findings were raised on PR #2716 by the Codex connector (three P1, two P2). **All five were verified against the tree and all five were valid** — none was waived, and the badge severity was not the deciding factor in any case. Two of them corrected substantive factual errors in a compliance-adjacent ADR, which is exactly the failure mode this drain exists to prevent.

| # | Finding | Verified against | Resolution |
|---|---|---|---|
| R1 | `MMT-ADR-0052` reduced "below threshold" to a binary learner-or-guardian outcome | `apps/api/drizzle/0158_wi2690_country_policy_seed.sql:211` stores `joint_child_guardian` alongside `blocked` and `parental_authorization_required` | **New clause 3**: the jurisdiction resolves the required *form* of authorisation. A binary reading would let a guardian-only grant satisfy a jurisdiction requiring both parties — a permissive failure |
| R2 | Unresolved residence was described as falling back to guardian authorisation | The launch ruling makes an unresolved combination *unavailable*; `apps/api/src/services/identity-v2/country-policy.ts:76,220` returns `launchDecision: 'blocked'` on every non-positively-allowed path | **Clause 6 rewritten**: failing closed means *blocked*, not guardian-mediated. The original wording created a path that could complete on a grant obtained under no verified policy — fail-open wearing fail-closed's appearance |
| R3 | The UX-spec banners made a `Proposed` ADR the governing canon, contrary to `docs/adr/README.md:46` | The ADR is `Status: Proposed`; sign-off is a human act | **All three banners reframed** as drift notices grounded in shipped code, which stands on its own. The ADR is cited as the drafted rationale, explicitly not-yet-canon, with formal supersession deferred to sign-off |
| R4 | `MMT-ADR-0047` clause 6 claimed retention and progress are single implementations reading the mode | `packages/database/src/schema/language.ts:62` (`vocabulary_retention_cards`), `apps/api/src/services/vocabulary.ts`, `apps/api/src/routes/language-progress.ts` | **Clause 6 rewritten** to distinguish the shared session shell from deliberately separate language-specific state. As written it could have justified deleting vocabulary scheduling and CEFR history as contradicting canon |
| R5 | `MMT-ADR-0051` claimed removing the accent machinery is a visual no-op | `apps/mobile/src/app/_layout.tsx:184-204` reads a per-profile selection from device storage on profile change | **Consequence rewritten** in the ADR, the ledger (§3) and the UX-spec note: an upgraded installation can still have a stored selection, so cleanup is a persisted-key migration, not a pure deletion |

**R1 and R2 also corrected this slice's own evidence base.** Both sources available when drafting — the 2026-07-23 drain assessment and the 2026-07-30 roadmap correction — described the jurisdiction resolver as a fail-closed scaffold with no production callers and an unpopulated policy table. The review surfaced that a canonical resolver has since landed: a pure function over an effective-dated, DB-mastered policy registry that holds no embedded country list, so adding a jurisdiction is a data change. The dated note in `MMT-ADR-0052`'s Consequences was rewritten accordingly — the mechanism exists; what remains is its adoption by the live consent gate, which is still a flat jurisdiction-blind comparison. **No code was changed by this slice; the code was right and the draft ADR was wrong.**
