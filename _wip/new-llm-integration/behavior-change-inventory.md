...
### 6. IDEMPOTENCY_KV binding now declared (not yet wired)
`wrangler.toml` now declares `IDEMPOTENCY_KV` for all three environments with placeholder IDs (`__IDEMPOTENCY_KV_DEV__`, etc.). The `render-wrangler-kv.mjs` script substitutes these at deploy time. **Action required before deploy:** create the KV namespace in Cloudflare for each environment and add `CF_KV_IDEMPOTENCY_ID_DEV/_STG/_PRD` to Doppler. Without this, wrangler will fail at deploy with an unresolved placeholder unless the substitution script is updated.
