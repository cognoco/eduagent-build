...
- Epics 0-5: full API layer (~1,705 API tests + ~664 mobile tests + 16 integration test suites, all passing)
...
- Mobile: 38+ screens (76 test suites), all using real API calls via TanStack Query + Hono RPC
...
- **Epic 7: Concept Map — Advisory Prerequisite Learning (v2 "Guide, Don't Gate")** (FR118-FR127 revised, FR150-FR152 new — 13 FRs, 6 stories) — All prerequisites advisory, never blocking. `topic_prerequisites` join table with `status` enum (ACTIVE/SKIPPED) — edges never deleted, soft-skip only. Single relationship type at launch (no REQUIRED/RECOMMENDED distinction). Any learner can self-override via "I already know this" or "Prove it" quiz (3-5 questions, reuses recall test infra). Age-appropriate concept map: journey path for under-13, Sugiyama DAG for 13+ (`react-native-svg`, no WebView). Graph-aware coaching cards with celebration on topic unlock (Epic 13 `queueCelebration()`). Periodic suggestive decay quizzes (FR150) — coaching card, 7-day cooldown, never mandatory. Per-edge human feedback (FR151) — learner/parent can flag bad prerequisites. LLM context injection for skipped/weak prerequisites. SM-2 stays pure per-topic — graph logic in coaching precomputation + new `prerequisite-service.ts`. Topological sort ordering with retention urgency tiebreaker.
...
5. **Does this cost money or consume limited resources?** EAS builds, API calls to paid services, push notifications — never retry these on failure. Investigate first.
   - **Build deduplication rule:** After merging a PR or triggering any build, **wait 3 minutes** then run `gh run list --workflow="Mobile CI" --limit 3` to check if a build is already running. Only trigger `gh workflow run` if NO build exists for that commit. Merging a PR auto-triggers CI workflows — do NOT manually dispatch on top. After any build trigger, verify with `eas build:list --limit 3` that only one build was created. If duplicates appear, cancel extras immediately.
