...
- **Default to `QueryStateView` for query-shaped routes.** New routes that follow the standard `loading → optional skeleton → query error → success` flow should wrap their content in `QueryStateView` (from `components/common`) instead of hand-rolling `isLoading ? ... : isError ? <ErrorFallback /> : ...`. QSV composes `ErrorFallback` and `TimeoutLoader` with a required `retry` action, so dead-ends become structurally impossible. Pass `variant="card"` or `"centered"` to match the screen, `loadingFallback` to keep a custom skeleton, and `errorTitle` / `errorMessage` for screen-specific copy. Hand-roll only when recovery is bespoke (multi-phase flow resets, hard-timeout with timer reset, inline card-variant errors mid-screen, missing-param guards).
