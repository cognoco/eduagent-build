...
- Mobile: ~80 screens, 288 test suites, ~3269 tests
- API: 42 route groups, 187 test suites, ~4565 tests, 50 Inngest functions
...
> Counts verified 2026-05-19. Test-case totals are a heuristic grep of `it(` / `test(` line starts; jest-reported totals may be slightly higher due to `it.each(...)` expansion at runtime. Re-verify with `git ls-files | grep '\.test\.'` for suite counts.
