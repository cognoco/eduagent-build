...
1. RED: with the event-time capture callback replaced by a no-op, the targeted test `starts the body read from the response event before Chromium can release it` failed because both matching responses produced zero early reads; the settled observation then rejected with `Network.getResponseBody: No data found` after simulated release.
