...
1. RED — capture-callback mutation: replacing the event-time capture callback with a no-op left both matching responses at zero early reads; the targeted test failed at `now-refresh-observation.test.ts:101` (`expected 1, received 0`) before any fallback read.
...
- `pnpm exec jest apps/mobile/e2e-web/helpers/now-refresh-observation.test.ts --runInBand --no-cache -t "starts the body read"` — RED under the fallback-selection mutation.
