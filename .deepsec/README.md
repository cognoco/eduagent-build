...
Currently configured project: `eduagent-build` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/eduagent-build/SETUP.md` to fill in
   `data/eduagent-build/INFO.md`.
...
`--project-id eduagent-build` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/eduagent-build/`.
