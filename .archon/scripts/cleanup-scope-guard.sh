...
while IFS= read -r file; do
    ...
    # Always allow LLM eval-harness snapshot regen. CLAUDE.md requires running
    # `pnpm eval:llm` whenever an LLM-prompt file is changed, so any work order
    # that legitimately claims a *-prompts.ts file will produce diff in these
    # snapshot artifacts as a mandatory downstream consequence. The files are
    # generated output (not human-authored, not imported at runtime), so the
    # smuggle-risk of exempting them is low; adversarial review and the harness
    # schema check (pnpm eval:llm --live) remain as guards on their content.
    if [[ "$file" == apps/api/eval-llm/snapshots/* ]]; then
        continue
    fi
...
if [[ ${#violations[@]} -gt 0 ]]; then
    violation_file="${artifacts_dir}/scope-violation.md"
    {
        ...
        echo ""
        echo "Files under \`apps/api/eval-llm/snapshots/\` are always allowed (regenerated by \`pnpm eval:llm\` whenever a claimed prompt file changes; see CLAUDE.md)."
    } > "$violation_file"
