...
Add a shared API service, likely `apps/api/src/services/practice-activity-summary.ts`, that computes period-bounded testing/practice activity summaries from canonical backend data sources. Wire it into weekly report generation, monthly report generation, and the progress/library endpoint that needs an activity count.

This service introduces **direct table queries** into the report generation path. Unlike existing report metrics (derived from `ProgressMetrics` snapshots via `snapshot-aggregation.ts`), practice activity must be queried directly from `quiz_rounds`, `dictation_results`, `assessments`, and `session_events` — none of which feed into the snapshot pipeline today. Evaluate whether quiz/dictation should also be added to the snapshot pipeline for long-term consistency.
...
- Aggregate completed quizzes from `quiz_rounds` where `status = 'completed'` and `completedAt` is within the period, split by `capitals`, `guess_who`, and `vocabulary`. Vocabulary rounds must be language-aware (`languageCode` is NULL for capitals/guess_who, non-null for vocabulary — decide whether to group by language or aggregate flat).
- Aggregate completed assessments from terminal `assessments` rows. Terminal statuses are: `passed`, `failed`, `borderline`, `failed_exhausted`. Decide which terminal states count as "completed" for the summary (all four? only `passed`?).
- Aggregate completed dictations from `dictation_results`. Completion proxy is `createdAt` (no `completedAt` column exists). Calendar date is stored in `date` column.
- Aggregate quiz XP from `quiz_rounds.xpEarned` (integer, set at round completion). Note: quiz XP is NOT in the `xp_ledger` — these are two disconnected XP stores. The `xp_ledger` tracks topic/session XP only. Any "total XP earned" must query both stores.
...
- Aggregate fluency drill scores from `session_events.drillCorrect` and `session_events.drillTotal` (integer columns, populated from LLM envelope `ui_hints.fluency_drill.score` during sessions).
- Count recitation sessions via `learning_sessions.metadata->>'effectiveMode' = 'recitation'`. Note: `effectiveMode` is an untyped JSONB key (not a DB enum), with known values: `learning`, `freeform`, `homework`, `recitation`, `practice`, `review`. Decide whether `practice` and `review` mode sessions should also count as practice activity.
...
- Faking exact historical review counts if the current data model cannot support them (see Decision Gate).
...
1. **New append-only `review_events` table** — records each review attempt with a timestamp. Enables exact per-period counts. Requires a migration.
2. **Proxy from `session_events`** — if review interactions are recorded as session events with a specific `eventType`, count those. Needs investigation of whether existing event types cover all review paths.
3. **Remove `reviewsCompleted` from schema** — defer review counting entirely until an append-only source exists. Ship quiz/dictation/assessment/drill counts first.
...
- The existing `reportPracticeSummarySchema` shape (`quizzesCompleted`, `reviewsCompleted`) is likely too narrow — it will need to be expanded to cover dictation, assessments, drills, and potentially XP.
...
- Quiz XP (`quiz_rounds.xpEarned`) and topic XP (`xp_ledger`) are separate stores. Decide which to include.
- Treat review counts as **blocked** until Decision Gate resolves. `retention_cards.lastReviewedAt` is a single mutable timestamp — cannot count reviews per period. `vocabularyRetentionCards` has the same limitation.
...
| Aggregation query times out | Profile with thousands of quiz rounds over a wide period | Report generation Inngest step fails/retries | Add index on `quiz_rounds(profileId, completedAt)` if not present. Consider aggregation cap or pagination. |
...
To be completed after Decision Gates are resolved.
...
- **Schema expansion required:** `reportPracticeSummarySchema` must grow beyond `quizzesCompleted` / `reviewsCompleted` to cover dictation, assessments, drills, XP, and any new fields.
- **Decision Gate — Review Counts:** May require a new append-only `review_events` table if exact review counts are needed (migration required), or `reviewsCompleted` is removed from the schema.
...
- **Index evaluation:** Check for existing indices on `quiz_rounds(profileId, completedAt)`, `dictation_results(profileId, createdAt)`, `assessments(profileId, updatedAt)`. Add if missing.

### Testing Strategy

To be completed after Decision Gates are resolved. Must include:
...
- This service builds from zero, not extends existing infrastructure. The schema slot and read-path pass-through exist, but the computation layer (querying sources, aggregating, injecting into reports) is completely absent.
- The `progress_snapshots` pipeline does not include quiz or dictation data. This service introduces a fundamentally different query pattern (direct table queries with date-range filters) into the report generation path.
