...
jest.mock('../../hooks/use-dashboard', () => ({
  // gc1-allow: WithdrawalCountdownBanner reads dashboard data; mocking isolates banner rendering from real API calls.
  useDashboard: () => mockUseDashboard(),
}));

jest.mock('../../hooks/use-restore-consent', () => ({
  // gc1-allow: restore-consent mutation is a network side effect; mocking lets tests verify CTA wiring without real API calls.
  useRestoreConsent: () => ({
    mutate: mockMutate,
    isPending: false,
  }),
...
jest.mock('../../lib/platform-alert', () => ({
  // gc1-allow: platformAlert is a native display side effect; mocking lets the banner test assert CTA feedback safely.
