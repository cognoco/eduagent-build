...
--
-- NOTE: Renumbered from 0086 (original commit 22545aae0) to 0087 because
-- 0086_bug393_fk_indexes.sql was already applied to all environments.
--
-- Steps:
...
--
-- ## Rollback
-- Rollback is possible pre-launch only. Steps:
--   DROP INDEX IF EXISTS "memory_dedup_decisions_profile_category_idx";
--   ALTER TABLE "memory_dedup_decisions" DROP CONSTRAINT "memory_dedup_decisions_profile_id_pair_key_category_pk";
--   ALTER TABLE "memory_dedup_decisions" DROP COLUMN "category";
--   TRUNCATE TABLE "memory_dedup_decisions";
--   ALTER TABLE "memory_dedup_decisions" ADD CONSTRAINT "memory_dedup_decisions_profile_id_pair_key_pk" PRIMARY KEY ("profile_id", "pair_key");
-- Post-launch: rollback is not possible without data loss; all dedup decisions
-- recorded against the new (pair_key, category) PK would be orphaned.
