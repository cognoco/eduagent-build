...
export const consentRoutes = new Hono<ConsentRouteEnv>()
  ...
  .get('/consent/:childProfileId/status', async (c) => {
    ...
    try {
      const state = await getChildConsentForParentV2(db, childProfileId, parentProfileId);
      if (!state) {
        return c.json(
  ...
  .put('/consent/:childProfileId/revoke', async (c) => {
    ...
    try {
      const { status, revokedAt } = await revokeChildConsentV2(db, childProfileId, parentProfileId).then(
  ...
  .put('/consent/:childProfileId/restore', async (c) => {
    ...
    try {
      const state = await restoreChildConsentV2(db, childProfileId, parentProfileId);
      return c.json(
        consentActionResultSchema.parse({
