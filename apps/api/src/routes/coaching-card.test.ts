...
describe('coaching card routes', () => {
  ...
  // profileAuthorityVerifiedFor proof exists, so this case exercises the
  // route guard's own fail-closed fallback (verifyPersonOwnershipV2) — the
  ...
  // before any route runs (see profile-scope.test.ts).
  ...
  describe('[WI-2877] read-authority guard', () => {
    ...
    it('GET /coaching-card rejects a cross-profile read with 403 via the fallback when no central proof exists', async () => {
      // The caller's person holds no self/guardianship authority over the
      // installed profile — the mentor's private coaching guidance about the
      // learner must never be readable via X-Profile-Id spoofing.
      jest
