  ...
  async ({ event, step }) => {
    ...
    try {
      const result = await step.run('generate-weekly-self-report', async () => {
        ...
        const eligibleProfileIds = await listEligibleSelfReportPersonIdsV2(db, win);
    ...
    const weeklyTargets = await step.run(
      ...
      async () => {
        ...
        for (let weekOffset = 0; weekOffset < 4; weekOffset += 1) {
          ...
          const eligibleProfileIds = await listEligibleSelfReportPersonIdsV2(db, backfillWin);
