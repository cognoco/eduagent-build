# Probe Battery (pre-launch tuning) × 11yo-czech-animals · P23
...
{
  "probeId": "P23",
  "description": "11yo Czech with accommodation context — short-burst, gentle pacing",
  "category": "standard",
  "context": {
    ...
    "accommodationContext": "Learner benefits from short explanations with frequent check-ins. Prefer 2-3 sentence chunks with a question after each.",
...
<source_pack>
<source id="learner_message" kind="learner_message" reliability="learner_provided" reliable_for_facts="false" label="Current learner message" excerpt="Dobře. Řekni mi něco o zlomcích."/>
...
<source id="accommodation" kind="accommodation" reliability="memory_only" reliable_for_facts="false" label="Learner accommodation and teaching preference" excerpt="Learner benefits from short explanations with frequent check-ins. Prefer 2-3 sentence chunks with a question after each. stories"/>
</source_pack>
...
```
Dobře. Řekni mi něco o zlomcích.
```
...
```
{
  "reply": "I don't have reliable source material for that yet, so I won't invent the facts. What I can safely do now is frame your question: \"Dobře. Řekni mi něco o zlomcích.\" Share the textbook passage, worksheet, photo, or trusted source, and I'll help turn it into a clear answer with evidence.",
  "signals": {
  ...
  "private_sources": {
    ...
    "insufficient": true,
    "reason": "Server used the no-source safety fallback because no reliable factual source was available. Model reason: Model used general knowledge and current topic to define fractions and their basic components, adapting to learner's preferred style (stories/examples). Server rejected general_knowledge because private_sources.factual_confidence was missing; it must be at least 0.88."
  },
  "confidence": "high"
}
```
