...
```
{
  "reply": "I don't have reliable source material for that yet, so I won't invent the facts. What I can safely do now is frame your question: \"Vůbec tomu nerozumím. Můžeš mi to vysvětlit od začátku?\" Share the textbook passage, worksheet, photo, or trusted source, and I'll help turn it into a clear answer with evidence.",
  "signals": {
  ...
  "private_sources": {
    ...
    "insufficient": true,
    "reason": "Server used the no-source safety fallback because no reliable factual source was available. Model reason: Learner expressed confusion, asking for a fresh start. Explained the core concept of reading comprehension and its importance, using an example related to learner's interest (animals/stories), then asked a check question. Relied on general knowledge and current topic for definition and importance, and mentor memory for learner's interests and learning style. Server rejected general_knowledge because private_sources.factual_confidence was missing; it must be at least 0.88."
  },
  "confidence": "high"
}
```
