...
- **WARNING CRM03-vague.multi-question:** Active Challenge Round should ask ONE question per turn; reply contains multiple question marks.
- **ERROR CRM03-vague.no-evaluation:** Active Challenge Round answer turn must emit signals.challenge_round_evaluation with at least one item.

## Live LLM response

```
{
  "reply": "Glad to hear it's clicking!\n\nReady for a quick challenge to lock it in?\n\nHow does the meaning change between these two sentences?\n1. La manzana es verde. (using *ser*)\n2. La manzana está verde. (using *estar*)",
  "signals": {
