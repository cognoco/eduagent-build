# Review-Continuity Opener faithfulness (EU-1/EU-2/EU-4 — LLM judge) × 15yo-football-gaming · verbatim-misconception
...
> **Scenario:** `verbatim-misconception`
...
```json
{
  "scenarioId": "verbatim-misconception",
  "description": "prior answer contains a wrong idea — opener must address misconception, not reinforce it",
  "context": {
    "topicTitle": "why objects fall at the same rate",
    "consentGranted": true,
    "priorRetrieval": {
      "learnerAnswerVerbatim": "The heavier one hits first because it's got more weight pulling it down, that's just how gravity works. Like a bowling ball beats a tennis ball every time.",
  ...
  "exchangeContext": {
    "sessionId": "eval-review-opener-verbatim-misconception",
...
Current topic: <topic_title>why objects fall at the same rate</topic_title>
...
<source id="current_topic" kind="current_topic" reliability="trusted_app_content" reliable_for_facts="true" label="Loaded curriculum topic" excerpt="why objects fall at the same rate"/>
</source_pack>
...
CONTINUITY OPENER: Last time on <topic_title>why objects fall at the same rate</topic_title> the idea had not landed yet, so do NOT re-state or quote the learner's earlier attempt as if it were correct. Open with a fresh, low-stakes start on <topic_title>why objects fall at the same rate</topic_title> — a gentle re-introduction and one small, supported check. Keep it forward-looking and encouraging — name what the learner has and the next step, not past difficulty.
...
- Review-continuity opener scenario: verbatim-misconception — prior answer contains a wrong idea — opener must address misconception, not reinforce it
