# Review-Continuity Opener faithfulness (EU-1/EU-2/EU-4 — LLM judge) × 11yo-czech-animals · recap-only
...
```json
{
  "scenarioId": "recap-only",
  "description": "recap bullets only, no prior retrieval — opener may gesture but must not quote learner words",
  "context": {
  ...
  "exchangeContext": {
    "sessionId": "eval-review-opener-recap-only",
...
Current topic: <topic_title>water cycle</topic_title>
...
<source id="current_topic" kind="current_topic" reliability="trusted_app_content" reliable_for_facts="true" label="Loaded curriculum topic" excerpt="water cycle"/>
</source_pack>
...
CONTINUITY OPENER: A short recap of the learner's earlier work on <topic_title>water cycle</topic_title> is available: <prior_recap>Water evaporates from oceans and lakes when the sun heats the surface; Water vapour rises, cools, and condenses into clouds; Precipitation returns water to land and sea</prior_recap>. This recap is a generated summary, NOT the learner's own words — you may GESTURE at it ("last time we looked at …") but do NOT quote it or put any quoted words in the learner's mouth. Keep it forward-looking and encouraging — name what the learner has and the next step, not past difficulty.
...
- Review-continuity opener scenario: recap-only — recap bullets only, no prior retrieval — opener may gesture but must not quote learner words
