# Review-Continuity Opener faithfulness (EU-1/EU-2/EU-4 — LLM judge) × 12yo-dinosaurs · no-material
...
```json
{
  "scenarioId": "no-material",
  "description": "pure cold start — no retrieval, no recap; opener falls back to generic calibration block",
  "context": {
  ...
  "exchangeContext": {
    "sessionId": "eval-review-opener-no-material",
...
Current topic: <topic_title>plate tectonics</topic_title>
...
<source id="current_topic" kind="current_topic" reliability="trusted_app_content" reliable_for_facts="true" label="Loaded curriculum topic" excerpt="plate tectonics"/>
</source_pack>
...
CALIBRATION QUESTION: The UI may already have presented an opening question about <topic_title>plate tectonics</topic_title>. If the learner's latest message answers that question, do NOT ask it again — respond to what they remembered and use any gaps to guide the next teaching step.
...
- Review-continuity opener scenario: no-material — pure cold start — no retrieval, no recap; opener falls back to generic calibration block
