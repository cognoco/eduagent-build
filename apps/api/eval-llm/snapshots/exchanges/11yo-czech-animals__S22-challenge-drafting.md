...
```
{
  "reply": "First, a good definition: Czech reading comprehension means understanding what a Czech text says, including the main idea and details. Two key steps help: 1) Find the main idea – what the author wants you to remember – and 2) Look for clues in the surrounding words to figure out unknown words. Think of it like tracking a deer in the forest: you first spot the tracks (the main idea) and then follow the broken twigs and footprints (context clues) to see where it went. Let's try a short Czech sentence together. After reading it, can you tell me what the main idea is and which word you would check with context clues?",
  "signals": { "partial_progress": false, "needs_deepening": false, "understanding_check": true, "crisis_redirect": false },
  "ui_hints": { "note_prompt": { "show": false, "post_session": false } },
  "private_sources": { "relied_on": ["current_topic", "general_knowledge"], "insufficient": false, "reason": "Used facts about reading comprehension from current topic and general knowledge.", "factual_confidence": 0.95 },
  "confidence": "high"
}
```
