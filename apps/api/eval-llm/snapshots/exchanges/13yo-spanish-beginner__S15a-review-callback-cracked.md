# Exchanges (main tutoring loop) × 13yo-spanish-beginner · S15a-review-callback-cracked
...
> **Scenario:** `S15a-review-callback-cracked`
...
```json
{
  "scenarioId": "S15a-review-callback-cracked",
  "scenarioPurpose": "Warm review-callback opener (RR-1) — cracked branch: \"has it stuck?\" framing, private quote grounding (never quoted verbatim)",
  "context": {
...
WARM CALLBACK OPENER: Open as a tutor who remembers this learner — pick up the thread; do NOT announce "review mode" or "a review check". Vary your wording every session; never reuse a stock phrase. Make exactly one warm, specific reference to your shared history with this topic, then invite them back in with one light question. HONESTY: only credit a past success when explicitly told below they succeeded; never claim they got something right otherwise. Last time, Photosynthesis clicked for them — frame this as checking whether it stuck (e.g. "Last time you had Photosynthesis down — let's see if it stuck"). It has been about 16 days. For your private grounding only — do NOT quote it or attribute exact words — their last message on this topic was: <last_message>plants turn sunlight into sugar using chlorophyll</last_message>.
CALIBRATION QUESTION: The UI may already have presented an opening question about <topic_title>Spanish present tense verbs</topic_title>. If the learner's latest message answers that question, do NOT ask it again — respond to what they remembered and use any gaps to guide the next teaching step.
...
```
Start a learning session about Spanish present tense verbs.
```

## Builder notes

- Scenario: S15a-review-callback-cracked — Warm review-callback opener (RR-1) — cracked branch: "has it stuck?" framing, private quote grounding (never quoted verbatim)
...
```
{"reply":"Let's dive into Spanish present tense verbs. In Spanish, verbs are categorized based on their endings: -ar, -er, and -ir. Each category has its own set of conjugations depending on who's doing the action. For example, the verb \"hablar\" (to talk) is an -ar verb. If I say, \"yo hablo,\" it means \"I talk.\" \n\nSimilarly, for -er verbs like \"comer\" (to eat), if I say, \"tú comes,\" it means \"you eat.\" Finally, with -ir verbs such as \"vivir\" (to live), the phrase \"nosotros vivimos\" means \"we live.\" \n\nUnderstanding these patterns helps you form sentences in the present tense. Can you give me an example of a verb you remember how to conjugate in the present tense?","signals":{"partial_progress":false,"needs_deepening":false,"understanding_check":true,"crisis_redirect":false},"ui_hints":{"note_prompt":{"show":false,"post_session":false}},"private_sources":{"relied_on":["current_topic"],"insufficient":false,"reason":"Teaching basic concepts of Spanish present tense verbs is reliable and relevant to the learner's request.","factual_confidence":1.0},"confidence":"high"}
```
