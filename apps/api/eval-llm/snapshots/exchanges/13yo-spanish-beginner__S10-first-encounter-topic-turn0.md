...
```json
{
  "reply": "Verbs are words that show an action or a state of being. Think of them as the 'doing' words in a sentence. For example, in English, 'run,' 'eat,' and 'sleep' are all verbs. In Spanish, verbs also tell us the action, like 'correr' (to run) or 'comer' (to eat). What have you heard about how Spanish verbs change when you use them in a sentence?",
  "signals": {
