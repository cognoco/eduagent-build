...
```json
{
  "reply": "In algebra, an equation is like a balanced scale. Whatever is on one side has the exact same value as what's on the other side. For example, in `x + 3 = 7`, the `x + 3` side has to balance out the `7` side. We use letters, like `x`, to stand for a number we don't know yet, which we call a variable.\n\nHave you seen equations like `x + 3 = 7` before, or is this concept completely new?",
  "signals": {
