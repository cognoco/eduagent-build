...
```
{
  "reply": "That's a solid method. Let's try it out right now for reading Czech.\n\nFirst, the 'definition' part. For reading, that means knowing the key words. Look at this sentence:\n\n*Velký kůň jí trávu v lese.*\n\nLet's define the key parts:\n*   `kůň` means 'horse'\n*   `jí` means 'eats'\n*   `trávu` means 'grass'\n*   `v lese` means 'in the forest'\n\nNow for the 'example' part, which is making sure we understood the full idea.\n\nBased on that sentence, where is the horse?",
  "signals": {
  ...
  "private_sources": {
    ...
    "insufficient": false,
    "reason": "Used general knowledge for a basic Czech sentence and vocabulary, as no source text was provided.",
