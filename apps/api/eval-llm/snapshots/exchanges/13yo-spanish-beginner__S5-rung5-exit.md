...
```
JSON parse failed: Unexpected token 'I', "It can be "... is not valid JSON
```

## Live LLM response

```
It can be frustrating when a concept doesn't click. That's completely normal. Let's try walking through one last example together, step by step, with no pressure.

Think about describing a horse.

For a permanent trait, like its color, you use **ser**.
*   `El caballo **es** marrón.` (The horse **is** brown.)
That's its natural, lasting color. It's part of its identity.

For a temporary condition, like being tired after a ride, you use **estar**.
*   `El caballo **está** cansado.` (The horse **is** tired.)
The horse is only tired for a little while. It's a temporary state.

So, **ser** is for the permanent "what" and **estar** is for the temporary "how."

This is a tough one. Let's come back to it fresh later. Sometimes letting an idea sit for a bit is the best way to learn it.
```
