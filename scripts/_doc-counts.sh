#!/usr/bin/env bash
# _doc-counts.sh — Shared counting functions for AGENTS.md validation/update
...
# Extract the first number matching a pattern from AGENTS.md
# Usage: extract_claimed "API tests"  →  returns "1,300" (raw text including commas)
extract_claimed() {
  local pattern="$1"
  grep -m1 "$pattern" "$REPO_ROOT/AGENTS.md" 2>/dev/null
}
