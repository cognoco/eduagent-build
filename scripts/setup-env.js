...
const DB_READ_ONLY_VERIFIER = path.join(
  __dirname,
  '..',
  'packages',
  'database',
  'scripts',
  'verify-db-read-only.mjs',
);
...
function verifyDatabaseIsReadOnly(content, runVerifier = spawnSync) {
  const databaseUrl = dotenv.parse(content).DATABASE_URL;
  if (!databaseUrl) {
    console.log(
      '\x1b[31m[Doppler]\x1b[0m DATABASE_URL is missing from staging secrets.',
    );
    return false;
  }

  const result = runVerifier(process.execPath, [DB_READ_ONLY_VERIFIER], {
    env: { ...process.env, DATABASE_URL: databaseUrl },
    stdio: 'inherit',
  });
  if (result.error || result.status !== 0) {
    console.log(
      '\x1b[31m[Doppler]\x1b[0m Refusing to write local env files: ' +
        'the staging DATABASE_URL is not catalog-verified read-only.',
    );
    return false;
  }
  return true;
...
function main() {
  ...
  if (!verifyDatabaseIsReadOnly(content)) {
    process.exit(1);
  }
