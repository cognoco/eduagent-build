...
if [[ -z "$TS_FILES" ]]; then
  ...
  if [[ "$TS_COUNT" -gt 100 ]]; then
    ...
    #
    # Set it where a NON-INTERACTIVE shell will see it: husky hooks do not source
    # interactive rc files, so ~/.zshrc (macOS) or ~/.bashrc (Windows/Git Bash) is
    # too late. Use ~/.zshenv on macOS, or a machine-level environment variable on
    # Windows. The value is forwarded verbatim, so both forms jest accepts work —
    # an integer (2) or a percentage (50%).
