...
if [[ -n "$MOBILE_TESTS" ]]; then
  ...
  add_cmd fast  "bash scripts/record-test-receipt.sh mobile"  "Record a verification receipt (REQUIRED before push)"
  note "mobile-tests: pre-push BLOCKS the push without a fresh receipt"
  note "mobile-tests: receipt path is .test-receipts/mobile.json — commit it with your changes"
fi
