...
- **`profileSchemaShape.conversationLanguageConfirmed` / `.isCurrentUser` (response) keep `.optional()`** — a documented carve-out (WI-1556) from the "request → `.optional()`, response → `.nullable()`" canon (`docs/project_context.md`, `docs/architecture.md`). Both are **launch hints derived by the API**, not stored columns, so a profile object cached by the mobile client *before* this rollout omits the keys entirely. `.nullable()` does not help: it requires the key to be **present** with a `null` value and throws on an absent one, so every pre-rollout cached profile would fail to parse. `.nullable().optional()` is banned outright by the same canon. `.optional()` is therefore the only single Zod primitive that survives old-cache parsing. **Why `.optional()` rather than the `.default(false)` used by neighbouring booleans (`hasPremiumLlm`, `hasFamilyLinks`):** `undefined` is a meaningful third state — "the server did not report this hint" — and the consumer relies on it. `shouldRequireFirstMentorLanguageConfirmation` (`apps/mobile/src/lib/first-mentor-language.ts`) gates on strict `conversationLanguageConfirmed === false`, so an absent hint **fails open** and a stale cached profile can never raise the blocking first-Mentor gate. `.default(false)` would coerce that absence into a definitive "not confirmed" and wrongly gate users whose cache predates the rollout. The ban is docs-only (no automated checker), so no escape annotation is needed. **Does not generalize:** it is scoped to these two derived hint fields on `profileSchemaShape`. Response fields that are backed by a real column still use `.nullable()`, and a new response field must not copy this pattern without the same absent-key-plus-fail-open constraint.
