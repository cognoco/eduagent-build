...
**For the historical F1–F14 scattered-gating snapshot**, see `docs/flows/audience-matrix.md`; it is explicitly pending a V2/identity refresh and is not current implementation truth. For current UI gating, read `apps/mobile/src/lib/navigation-contract.ts` and its tests (design rationale archived at `docs/_archive/specs/Done/2026-05-21-navigation-contract.md`). The short version is below.

> **Nav mode is per-environment — check it whenever the environment under discussion changes; never assume a global default.** The flags are **build-time** (`MODE_NAV_V0_ENABLED`, `MODE_NAV_V1_ENABLED`, `MODE_NAV_V2_ENABLED`, resolved in `apps/mobile/src/lib/feature-flags.ts:30-32`) and intentionally differ by environment — as of 2026-07-14: development and preview builds enable V0+V1+V2; production enables V0+V1 with V2 off (`apps/mobile/eas.json`). These can change — read the profile values and current navigation contract before making any shell claim. The June `docs/flows/mobile-app-flow-inventory.md` matrix is retained V0/V1 evidence but is explicitly pending a V2/13+ refresh. Do not write any flag state into docs or memory as "the default."
>
...
The V1 guardian redesign replaces `own-learning` + `library` with a single `recaps` tab. V1 sets live in `apps/mobile/src/lib/navigation-contract.ts`; legacy/V0 sets in `apps/mobile/src/lib/legacy-navigation-contract.ts`. When V2 is enabled, the outer shell is Mentor/Subjects/Journal and the legacy table below is not the active tab shape. Which shell is active depends on the build profile — check, don't assume (see the mode-check note above).
